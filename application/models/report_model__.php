...
class Report_Model extends CI_Model {
	...
	function get_penjualan_laba_report($from, $to, $cond, $customer_cond, $tanggal_awal, $bulan_harga){
		...
		$query = $this->db->query("SELECT tbl_a.id, tbl_a.status_aktif, no_faktur_lengkap as no_faktur, tanggal, qty, jumlah_roll, 
			...
			tbl_a.id as data , jatuh_tempo, hpp, barang_id, warna_id, nama_barang_jual, barang_id, warna_id
				FROM (
					SELECT *, concat('FPJ', DATE_FORMAT(tanggal,'%d%m%y'),'-',$this->pre_faktur,LPAD(no_faktur,5,'0')) as no_faktur_lengkap
					FROM nd_penjualan 
					WHERE tanggal >= '$from'
					...
					LEFT JOIN (
						...
						FROM (
							(
								...
								SELECT sum(qty) as qty_beli, sum(qty*ifnull(harga_stok_awal,0)*1.1) as total_beli, stok_awal.barang_id, stok_awal.warna_id
								FROM (
	...
	function get_hpp_by_mutasi($tahun, $bulan, $bulan_qty, $bulan_harga, $tanggal_start){
			...
			SELECT t1.barang_id, t1.warna_id, sum(ifnull(total_awal,0)) / sum(ifnull(qty_awal,0)) as hpp, sum(latest_harga) as latest_harga
				FROM (
					(
						...
						SELECT barang_id, warna_id, sum(qty) as qty_awal, sum(qty * (harga_beli/1.1)), 0 as latest_harga
						FROM (
							SELECT *
							FROM nd_pembelian
...
//==================================pembelian list==========================================
	...
	function get_pembelian_report($cond_tanggal, $cond, $cond_barang, $cond_warna, $cond_gudang, $order_column){
		$query = $this->db->query("SELECT tbl_a.id, tbl_a.status_aktif, no_faktur, tanggal, qty, jumlah_roll, 
			...
			tbl_b.satuan_id, po_pembelian_batch_id, po_number, created_at, ockh
				FROM (
					SELECT *
					FROM nd_pembelian 
					WHERE status_aktif = 1
				...
				LEFT JOIN (
					...
					group_concat(concat_ws(' ',nama_jual,warna_jual) SEPARATOR '??') as nama_jual, 
					group_concat(nd_pembelian_detail.harga_beli SEPARATOR '??') as harga_beli, 
					group_concat(qty SEPARATOR '??') as qty ,
					group_concat(jumlah_roll SEPARATOR '??') as jumlah_roll, 
					group_concat((qty *nd_pembelian_detail.harga_beli) SEPARATOR '??') as total, 
					sum(qty *nd_pembelian_detail.harga_beli) as g_total, pembelian_id, 
					group_concat(nd_barang.satuan_id SEPARATOR '??') as satuan_id,
					group_concat(ockh SEPARATOR '??') as ockh
					FROM (
						SELECT qty, jumlah_roll, pembelian_id, barang_id, warna_id, harga_beli, ockh
						FROM (
							SELECT *
...
//================================buku laporan penyesuaian stok=============================
	...
	function get_penyesuaian_stok($select, $tanggal_start, $tanggal_end, $cond_gudang, $cond_barang, $cond_warna){
		...
		$query = $this->db->query("SELECT t2.nama_jual as nama_barang,t2.nama_jual as nama_jual, t3.warna_beli as nama_warna, t4.nama as nama_satuan, sum(qty_masuk) as qty_masuk, sum(qty_keluar) as qty_keluar, sum(jumlah_roll_masuk) as jumlah_roll_masuk, sum(jumlah_roll_keluar) as jumlah_roll_keluar
			FROM (
				(
...
//================================so report======================================
	...
	function get_so_report($tanggal_start, $tanggal_end, $barang_cond, $warna_cond, $gudang_cond){
		...
		$query = $this->db->query("SELECT t1.*, t2.*, concat(pre_po,'-',YEAR(t1.created_at),'/41/',LPAD(no_surat,3,'0') ) as no_so
			FROM (
				SELECT group_concat(nama_jual order by t2.created_at asc) as nama_barang, 
				group_concat(warna_jual order by t2.created_at asc) as nama_warna, 
				group_concat(stok_opname_id order by t2.created_at asc) as stok_opname_id, 
				group_concat(t2.status_aktif order by t2.created_at asc) as status_aktif, 
				group_concat(nd_gudang.nama order by t2.created_at asc) as nama_gudang, 
				group_concat(t1.qty order by t2.created_at asc) as qty_data, 	
				group_concat(t1.jumlah_roll order by t2.created_at asc) as jumlah_roll_data, 
				group_concat(t3.qty order by t2.created_at asc) as qty_data_penyesuaian, 	
				group_concat(t3.jumlah_roll order by t2.created_at asc) as jumlah_roll_data_penyesuaian, 
				stok_opname_report_id, 
				GROUP_CONCAT(DATE_FORMAT(t2.created_at,'%d/%m/%y %H:%i') order by t2.created_at asc) as tanggal_so, pre_po
				FROM (
				...
				LEFT JOIN (
					SELECT keterangan, qty, jumlah_roll
					FROM nd_penyesuaian_stok
					WHERE tipe_transaksi = 4
				) t3
