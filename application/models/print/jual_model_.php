...
class Jual_Model extends CI_Model
{
	...
	function get_header($id)
	{
		$query = $this->db->query("SELECT
								...
								CONCAT('FPJ', RIGHT(CONCAT('00', date_format(A.tanggal, '%d')), 2), RIGHT(CONCAT('00', date_format(A.tanggal, '%m')), 2), RIGHT(CONCAT('00', date_format(A.tanggal, '%y')), 2), '-',$this->pre_faktur, RIGHT(CONCAT('00000', A.no_faktur), 5)) AS no_invoice,
							...
							FROM
								...
								nd_customer B ON A.customer_id = B.id LEFT JOIN
								nd_toko C ON A.toko_id = C.id
							WHERE
								A.id = '" . $id . "'");
							...
							FROM
								...
								(
									SELECT
										...
										AB.barang_id,
										AB.warna_id,
										SUM(IF(AA.jumlah_roll = 0, 1, AA.jumlah_roll)) AS roll,
