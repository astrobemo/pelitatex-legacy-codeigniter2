...
class Transaction_Model extends CI_Model {
	...
	function __construct()
    {
		...
		$this->tahun_start_program = "2023";
		$this->tanggal_start_program = "2023-01-01";
...
//===============================po pembelian===========================================
	function get_po_pembelian_list_ajax($aColumns, $sWhere, $sOrder, $sLimit){
		$extra_cond = '';
		$tahun = $this->tahun_start_program;
		...
		$query = $this->db->query("SELECT *
			FROM (
					...
					WHERE YEAR(tbl_a.tanggal) >= $tahun
				)A
			$sWhere
            ORDER BY id desc
            $sLimit
	...
	function get_ockh($po_pembelian_batch_id, $barang_id, $warna_id){
		$query = $this->db->query("SELECT t1.*
			...
			AND t1.po_pembelian_detail_id = t3.id
			WHERE t3.id is not null
			", false);
...
//===============================pembelian===========================================
	...
	function get_pembelian_list_ajax($aColumns, $sWhere, $sOrder, $sLimit, $tanggal_start, $tanggal_end){
		// $tahun = date('Y');
		// $tahun = $this->tahun_start_program;
		$query = $this->db->query("SELECT *
			FROM (
	...
	function get_data_pembelian($id){
		...
		tbl_d.nama as nama_toko, tbl_b.telepon as telepon_supplier, LPAD(no_nota,4,'0') as no_nota_p, po_number, no_plat, jam_input
			FROM (
				...
				FROM nd_pembelian
				WHere id = $id
				AND tanggal >='$this->tanggal_start_program'
			) as tbl_a
			LEFT JOIN nd_supplier as tbl_b
			ON tbl_a.supplier_id = tbl_b.id
	...
	function get_barang_before($id, $supplier_id){
		$query = $this->db->query("SELECT t1.*
			FROM (
				SELECT *
				FROM nd_pembelian_detail
	...
	function get_pembelian_barang_by_po($po_pembelian_batch_id, $OCKH){
		...
		if(tipe_barang =1, c1.nama, nama_beli_baru) as nama , d.warna_beli as nama_warna, 
		...
		tipe_barang, po_pembelian_id, a.qty, locked_by, if(tipe_barang = 1,b.barang_beli_id, barang_beli_id_baru) as barang_beli_id
			FROM (
				SELECT t1.*, t2.nama as nama_baru, t21.nama as nama_beli_baru
				FROM (
					SELECT *
					FROM nd_po_pembelian_warna
					WHERE po_pembelian_batch_id = $po_pembelian_batch_id
	...
	function get_pembelian_other($ockh, $supplier_id){
...
//==========================================PENJUALAN================================
	...
	function get_penjualan_list_ajax($aColumns, $sWhere, $sOrder, $sLimit){
		$cond = "WHERE YEAR(tanggal) >='$this->tahun_start_program'";
		if (is_posisi_id() > 3) {
	...
	function get_kartu_stok_barang_by_satuan_2($gudang_id, $barang_id, $warna_id, $tanggal_start, $tanggal_end, $tanggal_awal, $stok_opname_id, $cond_detail, $cond_detail_lain){
	...
	function get_data_penjualan($id){
		...
		$query = $this->db->query("SELECT tbl_a.id,penjualan_type_id, tanggal, customer_id, no_faktur as no_faktur_raw, 
				...
				if(penjualan_type_id = 3,if(nama_keterangan = '','no_name', nama_keterangan), if(nama_cust_fp is null or nama_cust_fp = '', tbl_b.nama, nama_cust_fp) ) as nama_keterangan, 
			...
			FROM (
				select t1.id, t1.toko_id, t1.penjualan_type_id, t1.no_faktur, t1.po_number, t1.tanggal, 
					...
					po_penjualan_id, tipe_po,nama_cust_fp,
...
function get_data_penjualan_detail_group($id){
		...
		$query = $this->db->query("SELECT tbl_a.*, jenis_barang, 
		ifnull(nama_jual_tercetak, nama_barang) as nama_barang, nama_satuan, tbl_c.nama as nama_gudang, 
		...
		group_concat(data_qty SEPARATOR '--') as data_qty, tipe_qty
			...
			LEFT JOIN (
				SELECT sum(qty * if(jumlah_roll=0,1,jumlah_roll )) as qty, sum(jumlah_roll) as jumlah_roll, penjualan_detail_id,  
				group_concat(concat_ws('??',qty,jumlah_roll) ORDER BY id asc SEPARATOR '--') as data_qty
				FROM nd_penjualan_qty_detail
				group by penjualan_detail_id
				) as tbl_e
			ON tbl_e.penjualan_detail_id = tbl_a.id
...
//=============================pengeluaran_stok_lain================================
	...
	function get_pengeluaran_stok_lain_list(){
		$query = $this->db->query("SELECT t1.status_aktif, no_faktur, tanggal, t1.id as pengeluaran_stok_lain_id, 
			ifnull(g_total,0) as g_total, status, count_item, t1.keterangan, t1.id
				FROM nd_pengeluaran_stok_lain t1
				LEFT JOIN (
					SELECT sum(qty) as g_total, pengeluaran_stok_lain_id, sum(1) as count_item
		...
		$query = $this->db->query("SELECT tbl_a.*, if(tbl_a.nama_keterangan != '', tbl_a.nama_keterangan, if(tbl_a.retur_type_id = 1,'no name', tbl_c.nama )) as nama_customer, username, created_at, no_faktur_lengkap, group_concat(harga) as harga, group_concat(qty) as qty, group_concat(jumlah_roll) as jumlah_roll, nama_barang, group_concat(nama_gudang) as nama_gudang, group_concat(nama_barang) as nama_barang,group_concat(warna_jual) as nama_warna, group_concat(nama_gudang) as nama_gudang
			FROM (
				...
				FROM (
					SELECT *, concat('FRJ', DATE_FORMAT(tanggal,'%d%m%y'),'-',LPAD(no_faktur,4,'0')) as no_faktur_lengkap
					FROM nd_retur_jual
					WHERE tanggal >= '".$this->tanggal_start_program."'
					) t1
...
//==========================================retur beli================================
	...
	function get_retur_beli_detail($id){
		$query = $this->db->query("SELECT t1.*, nama_barang, nama_satuan, t3.nama as nama_gudang, 
		t4.warna_beli as nama_warna, qty, jumlah_roll, data_qty
			FROM (
				SELECT *
				FROM nd_retur_beli_detail
				WHERE retur_beli_id = $id
				) t1
			LEFT JOIN nd_retur_beli retur
			ON t1.retur_beli_id = retur.id
			LEFT JOIN (
				SELECT t2.po_pembelian_batch_id, if(t2.tipe_barang!=1,barang_beli_id_baru,barang_beli_id) as barang_beli_id,
				if(t2.tipe_barang !=1 ,barang_id_baru,barang_id) as barang_id, warna_id 
				FROM nd_po_pembelian_warna t2
				LEFT JOIN nd_po_pembelian_detail t3
				ON t2.po_pembelian_detail_id = t3.id
			) po
			ON po.po_pembelian_batch_id = retur.po_pembelian_batch_id
			AND t1.barang_id = po.barang_id
			AND t1.warna_id = po.warna_id
			LEFT JOIN (
				SELECT nb.id as barang_id, ndb.id as barang_beli_id, ndb.nama as nama_barang, ns.nama as nama_satuan 
				FROM nd_barang_beli ndb
				LEFT JOIN nd_barang nb
				ON ndb.barang_id = nb.id
				LEFT JOIN nd_satuan ns
				ON nb.satuan_id = ns.id
				) t2
			...
			LEFT JOIN (
				...
				retur_beli_detail_id, group_concat(concat_ws('??',qty, jumlah_roll) SEPARATOR '--') as data_qty 
	            FROM nd_retur_beli_qty
	            GROUP BY retur_beli_detail_id
	            ) t5
	...
	function get_dp_list(){
		$query = $this->db->query("SELECT tbl_a.id, nama, status_aktif , ifnull(dp_masuk,0) - ifnull(dp_keluar,0) - ifnull(dp_on_piutang,0) as saldo
			...
			LEFT JOIN (
				...
				FROM (
					SELECT amount - ifnull(dp_keluar,0) as dp_masuk, customer_id, tanggal
					FROM nd_dp_masuk
					LEFT JOIN (
...
//==========================================history list================================
	...
	function get_pembelian_history($from, $to){
		...
		if($from < $this->tanggal_start_program){
			$from = $this->tanggal_start_program;
		}
...
**/
	...
	function get_outstanding_barang_for_request_2($request_barang_id, $barang_id, $tanggal, $supplier_id){
		$query = $this->db->query("SELECT t1.barang_id, t1.warna_id, t1.barang_beli_id, 
		sum(if(po_qty - ifnull(qty,0) - ifnull(qty_before,0) > 0 , po_qty - ifnull(qty,0) - ifnull(qty_before,0) ,0))as qty_outstanding, 
		group_concat(if(po_qty - ifnull(qty,0) - ifnull(qty_before,0) > 0 , po_qty - ifnull(qty,0) - ifnull(qty_before,0) ,0)  order by batch_tanggal,batch asc) as qty_data, 
		group_concat(po_qty order by batch_tanggal,batch asc) as po_qty, 
		group_concat(po_number order by batch_tanggal,batch asc  SEPARATOR '??' ) as po_number, 
		...
		group_concat(t1.po_pembelian_batch_id order by batch_tanggal,batch asc) as po_pembelian_batch_id
		FROM (
			SELECT barang_id, barang_beli_id, warna_id, sum(qty) as po_qty, po_pembelian_batch_id, 
				concat(if(tB.tanggal >= '2019-09-27', concat(ifnull(pre_po,''),tD.kode,'/',DATE_FORMAT(tB.tanggal,'%y'),'/',LPAD(po_number,3,'0')) ,concat(LPAD(po_number,3,'0'),'/',DATE_FORMAT(tB.tanggal,'%m'),'/',DATE_FORMAT(tB.tanggal,'%y'))),'-',batch,if(revisi > 1,concat('R',revisi-1),'') ) as po_number, 
				batch_tanggal, batch, po_pembelian_warna_id, po_pembelian_id
			FROM (
				SELECT *
				FROM (
					SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, if(t1.tipe_barang!=1,barang_beli_id_baru,barang_beli_id) as barang_beli_id, warna_id, po_pembelian_batch_id, 
	...
	function get_outstanding_barang_for_request_2_bybeli($request_barang_id, $barang_beli_id, $tanggal, $supplier_id){
		$query = $this->db->query("SELECT t1.barang_id, t1.warna_id, t1.barang_beli_id, 
		sum(if(po_qty - ifnull(qty,0) - ifnull(qty_before,0) > 0 , po_qty - ifnull(qty,0) - ifnull(qty_before,0) ,0))as qty_outstanding, 
		group_concat(if(po_qty - ifnull(qty,0) - ifnull(qty_before,0) > 0 , po_qty - ifnull(qty,0) - ifnull(qty_before,0) ,0)  order by batch_tanggal,batch asc) as qty_data, 
		group_concat(po_qty order by batch_tanggal,batch asc) as po_qty, 
		...
		FROM (
			...
			FROM (
				SELECT *
				FROM (
					SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, 
					if(t1.tipe_barang!=1,barang_beli_id_baru,barang_beli_id) as barang_beli_id, warna_id, po_pembelian_batch_id, 
						t1.locked_by, t1.qty, t2.po_pembelian_id, batch, revisi, t2.tanggal as batch_tanggal, t1.id as po_pembelian_warna_id, t1.locked_date
					FROM nd_po_pembelian_warna t1
					LEFT JOIN nd_po_pembelian_batch t2
					ON t1.po_pembelian_batch_id = t2.id
			...
			ON tA.po_pembelian_id = tB.id
			LEFT JOIN (
				SELECT barang_beli_id as bbidr ,barang_id as bidr, warna_id as widr, min(tanggal) as tanggal_request 
				FROM nd_request_barang_qty t1
				LEFT JOIN nd_request_barang_batch t2
				ON t1.request_barang_batch_id = t2.id
				WHERE request_barang_id = '$request_barang_id'
				GROUP BY barang_beli_id, warna_id
			) tR
			ON tA.barang_beli_id = tR.bbidr
			AND tA.warna_id = tR.widr
			LEFT JOIN nd_toko tC
			ON tB.toko_id = tC.id
			LEFT JOIN nd_supplier tD
		...
		LEFT JOIN (
			SELECT barang_id,barang_beli_id, warna_id, sum(qty) as qty, po_pembelian_batch_id
			FROM (
				SELECT barang_id,barang_beli_id, warna_id, qty, po_pembelian_batch_id, tanggal
				FROM nd_pembelian_detail tA
				LEFT JOIN (
					SELECT *
					FROM nd_pembelian
			...
			ON result.barang_beli_id = tR.bbidr
			AND result.warna_id = tR.widr
			WHERE result.tanggal < ifnull(tanggal_request, '$tanggal') 
			GROUP BY barang_beli_id, warna_id, po_pembelian_batch_id
		) t2
		ON t1.po_pembelian_batch_id = t2.po_pembelian_batch_id
		AND t1.barang_beli_id = t2.barang_beli_id
		AND t1.barang_id = t2.barang_id
		AND t1.warna_id = t2.warna_id
		LEFT JOIN (
			SELECT po_pembelian_id, po_pembelian_batch_id, po_pembelian_warna_id, qty as qty_before
		...
		AND t1.po_pembelian_batch_id = t3.po_pembelian_batch_id
		AND t1.po_pembelian_warna_id = t3.po_pembelian_warna_id
		GROUP BY t1.barang_beli_id, t1.warna_id
		");
		return $query->result();		
	}
	...
	function get_request_barang_data($batch_id){
		$query = $this->db->query("SELECT t1.id as id, t1.tanggal as tanggal, batch, no_request,  
				...
				locked_by, locked_date, request_barang_id, attn, closed_date, tanggal_awal, supplier_id
				FROM (
					SELECT *
	...
	function get_active_request_barang_by_request_2($request_barang_id, $request_barang_batch_id, $cond_supplier, $supplier_id){
		$this->db->simple_query('SET SESSION group_concat_max_len=15000');
		$latest_date = date('Y-12-31');
		...
		$query = $this->db->query("SELECT t1.*, 
				group_concat(request_barang_detail_id ORDER BY nama_barang  SEPARATOR '??') as request_barang_detail_id, 
				group_concat(barang_id ORDER BY nama_barang ASC SEPARATOR '??') as barang_id, 
				group_concat(barang_beli_id ORDER BY nama_barang ASC SEPARATOR '??') as barang_beli_id, 
				group_concat(warna_id ORDER BY nama_barang ASC SEPARATOR '??') as warna_id, 
				group_concat(qty ORDER BY nama_barang ASC SEPARATOR '??') as qty, 
				group_concat(po_number ORDER BY nama_barang ASC SEPARATOR '??') as po_number, 
				group_concat(nama_barang ORDER BY nama_barang ASC SEPARATOR '??') as nama_barang, 
				group_concat(nama_beli ORDER BY nama_barang ASC SEPARATOR '??') as nama_beli, 
				group_concat(nama_warna ORDER BY nama_barang ASC SEPARATOR '??') as nama_warna, 
				group_concat(po_pembelian_batch_id ORDER BY nama_barang ASC SEPARATOR '??') as po_pembelian_batch_id, 
				group_concat(status_urgent ORDER BY nama_barang SEPARATOR '??') as status_urgent, 
				group_concat(qty_total ORDER BY nama_barang ) as qty_total, bulan_request
				FROM (
					SELECT rbid as request_barang_batch_id, rid as request_barang_id, 
						bid as barang_id, nd_barang_beli.nama as nama_barang, 
						blid as barang_beli_id, nd_barang_beli.nama as nama_beli, 
						...
						breq as bulan_request, sum(ifnull(qty,qty_r)) as qty_total, supplier_id
						FROM (
							SELECT barang_id as bid, warna_id as wid, 
							barang_beli_id as blid,
							request_barang_batch_id as rbid, request_barang_id as rid, 
							qty as qty_r, bulan_request as breq
							FROM (
								SELECT *
								FROM nd_request_barang_qty
								WHERE id IN (
						...
						LEFT JOIN (
							SELECT tA.id as id, if(tA.po_pembelian_batch_id != 0 ,po_number, 'PO BARU') as po_number, 
							request_barang_id, qty, tA.po_pembelian_batch_id, barang_beli_id, barang_id, warna_id, bulan_request, 
							tA.request_barang_batch_id, tanggal, if(tA.po_pembelian_batch_id != 0 ,tanggal_po_batch, '$latest_date') as tanggal_po_batch,
							status_urgent, if(is_po_baru = 1, '$supplier_id', supplier_id) as supplier_id
							FROM (
								SELECT tX.*
								FROM (
									SELECT tJ.*, tanggal, request_barang_id
								...
								ON tX.barang_beli_id = tY.barang_beli_id
								AND tX.warna_id = tY.warna_id
								AND tX.bulan_request = tY.bulan_request
								AND tX.tanggal < tY.tanggal
								WHERE tY.id is null
							...
							LEFT JOIN (
								...
								concat(if(t2.tanggal >= '2019-09-27', 
								concat(ifnull(pre_po,''),t4.kode,'/',DATE_FORMAT(t2.tanggal,'%y'),'/',LPAD(po_number,3,'0')) ,
								concat(LPAD(po_number,3,'0'),'/',DATE_FORMAT(t2.tanggal,'%m'),'/',DATE_FORMAT(t2.tanggal,'%y'))),'-',batch,
								if(revisi > 1,concat('R',revisi-1),'')) as po_number, t1.tanggal as tanggal_po_batch
								FROM nd_po_pembelian_batch t1
								LEFT JOIN nd_po_pembelian t2
								ON t1.po_pembelian_id = t2.id
								LEFT JOIN nd_toko t3
						...
						ON rs.blid = result.barang_beli_id
	...
	function get_request_barang_qty2($barang_id, $request_barang_batch_id, $request_barang_id){
		$query = $this->db->query("SELECT *
			FROM (
				(
					SELECT tA.id, request_barang_batch_id, barang_id, barang_beli_id , warna_id , qty, bulan_request, tanggal
					FROM (
						SELECT *
						FROM nd_request_barang_qty
						WHERE request_barang_batch_id = $request_barang_batch_id
					...
					SELECT tA.id, tA.request_barang_batch_id, tA.barang_id , tA.barang_beli_id, tA.warna_id , tA.qty, tA.bulan_request, tanggal
					FROM (
						SELECT *
						FROM nd_request_barang_qty
						WHERE request_barang_batch_id != $request_barang_batch_id
					...
					ON tA.barang_beli_id = tC.barang_beli_id
	...
	function get_request_barang_qty_aktif($request_barang_batch_id, $request_barang_id){
		$query = $this->db->query("SELECT t1.bulan_request, t1.barang_beli_id, t1.barang_id, t1.warna_id, t1.qty, sum(ifnull(t2.qty,0)) as qty_before, 
				nd_barang_beli.nama as nama_barang, warna_jual as nama_warna, 
				if(t1.qty != ifnull(t2.qty,0) , 1, 0) as tipe
					FROM (
						SELECT tA.*, batch
						FROM (
							SELECT bulan_request, barang_beli_id, barang_id, warna_id, qty, request_barang_batch_id
							FROM nd_request_barang_qty
							WHERE request_barang_batch_id = $request_barang_batch_id
							)tA
							LEFT JOIN nd_request_barang_batch tB
					...
					ON t1.barang_beli_id = t2.barang_beli_id
					ANd t1.warna_id = t2.warna_id
	...
	function get_request_barang_terkirim_2_bybeli($request_barang_id, $closed_date){
		$this->db->simple_query('SET SESSION group_concat_max_len=15000');
		$query = $this->db->query("SELECT t1.barang_beli_id, t1.barang_id, t1.warna_id, tanggal, tanggal_masuk, po_pembelian_batch_id as po_pembelian_batch_id, 
			qty as qty_masuk,qty_request, bulan_request, latest, qty_data
			FROM (
				SELECT barang_beli_id, barang_id, warna_id, min(tanggal) as tanggal, max(DATE_FORMAT(bulan_request,'%Y-%m-31')) as latest, 
					group_concat(DATE_ADD(bulan_request,interval 1 year) order by bulan_request asc) as bulan_request, 
					group_concat(qty  order by bulan_request asc) as qty_request
				FROM  (
					SELECT *
					...
					WHERE id IN (
						...
						WHERE request_barang_id = $request_barang_id
						AND status_aktif = 1
						GROUP BY barang_beli_id, warna_id, bulan_request
					)
				) t1
				LEFT JOIN nd_request_barang_batch t2
				ON t1.request_barang_batch_id = t2.id
			...
			LEFT JOIN (
				SELECT group_concat(qty) as qty_data, sum(qty) as qty, t1.barang_beli_id, t1.barang_id, t1.warna_id, 
				group_concat(DATE_FORMAT(t2.tanggal, '%Y-%m-%d')) as tanggal_masuk, 
				group_concat(po_pembelian_batch_id) as po_pembelian_batch_id
				FROM nd_pembelian_detail t1
				LEFT JOIN nd_pembelian t2
				ON t1.pembelian_id = t2.id
				LEFT JOIN (
					SELECT barang_beli_id, barang_id, warna_id, concat(min(tanggal),' 00:00:00') as tanggal_request,'$closed_date' as latest, 
						concat(min(brq), ' 00:00:00')  as tanggal_awal
					FROM (
						SELECT *, DATE_ADD(bulan_request, INTERVAL 1 YEAR) as brq
						FROM nd_request_barang_qty
				...
				ON t1.barang_beli_id = t3.barang_beli_id
				AND t1.warna_id = t3.warna_id
				AND t2.tanggal >= t3.tanggal_awal
				AND t2.tanggal <= t3.latest
				WHERE t3.tanggal_request is not null
	...
	function get_request_barang_qty_data($request_barang_id, $request_barang_batch_id, $barang_id){
		...
		FROM (
				(
					...
					FROM (
						SELECT request_barang_batch_id, barang_beli_id, barang_id, warna_id, bulan_request, sum(qty) as qty, group_concat(qty) as qty_data
						FROM nd_request_barang_detail
						WHERE request_barang_batch_id = $request_barang_batch_id
						GROUP BY barang_id, warna_id, bulan_request
					)tA
                    LEFT JOIN (
                        SELECT barang_beli_id, barang_id, warna_id, bulan_request, request_barang_batch_id, tY.id as other_request_batch_id
                        FROM (
                            SELECT *
                            FROM nd_request_barang_batch
                            WHERE request_barang_id = $request_barang_id
	...
	function get_request_barang_qty_keterangan($request_barang_id){
		$query = $this->db->query("SELECT barang_beli_id, barang_id, warna_id, bulan_request, keterangan
			FROM nd_request_barang_qty
			WHERE id IN (
	...
	function get_updated_locked_po_request($request_barang_id, $tanggal_start){
		$query = $this->db->query("SELECT tM.*, nama as nama_barang, warna_jual as nama_warna,
			...
			FROM (
				...
				LEFT JOIN (
					SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, warna_id, po_pembelian_batch_id, 
						t1.locked_by, t1.qty, t2.po_pembelian_id, batch, revisi, t2.tanggal as batch_tanggal, t1.locked_date, 
				...
				AND tX.warna_id = tY.warna_id
				WHERE tY.locked_by is not null
			...
			LEFT JOIN (
				...
				FROM (
					SELECT barang_id, warna_id, sum(qty) as po_qty, po_pembelian_batch_id, 
					concat(if(tB.tanggal >= '2019-09-27', concat(ifnull(pre_po,''),tD.kode,'/',DATE_FORMAT(tB.tanggal,'%y'),'/',LPAD(po_number,3,'0')) ,
					concat(LPAD(po_number,3,'0'),'/',DATE_FORMAT(tB.tanggal,'%m'),'/',DATE_FORMAT(tB.tanggal,'%y'))),'-',batch,
					if(revisi > 1,concat('R',revisi-1),'') ) as po_number, batch_tanggal, batch, tA.locked_date
					FROM (
						SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, warna_id, po_pembelian_batch_id, t1.locked_by, t1.qty, 
						t2.po_pembelian_id, batch, revisi, t2.tanggal as batch_tanggal, t1.locked_date
						FROM nd_po_pembelian_warna t1
						LEFT JOIN nd_po_pembelian_batch t2
						ON t1.po_pembelian_batch_id = t2.id
						LEFT JOIN nd_po_pembelian_detail t3
						...
						AND (
							t1.locked_date is null
							OR t1.locked_date >= '$tanggal_start'
						)
						)tA
					LEFT JOIN (
						SELECT *
	...
	function get_updated_revisi_po_request($request_barang_id, $tanggal_start){
		$query = $this->db->query("SELECT tM.*, nama as nama_barang, warna_jual as nama_warna,
			...
			LEFT JOIN (
				SELECT t1.barang_id, t1.warna_id, (if(po_qty - ifnull(qty,0) > 0 , po_qty - ifnull(qty,0) ,0))as qty_outstanding, po_qty, 
				po_number, batch_tanggal, t1.po_pembelian_batch_id, locked_date
				FROM (
					SELECT barang_id, warna_id, sum(qty) as po_qty, po_pembelian_batch_id, 
					concat(if(tB.tanggal >= '2019-09-27', concat(ifnull(pre_po,''),tD.kode,'/',DATE_FORMAT(tB.tanggal,'%y'),'/',LPAD(po_number,3,'0')) ,
					concat(LPAD(po_number,3,'0'),'/',DATE_FORMAT(tB.tanggal,'%m'),'/',DATE_FORMAT(tB.tanggal,'%y'))),'-',batch,if(revisi > 1,concat('R',revisi-1),'') ) as po_number, 
					batch_tanggal, batch, tA.locked_date
					FROM (
						SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, warna_id, 
						po_pembelian_batch_id, t1.locked_by, t1.qty, t2.po_pembelian_id, batch, revisi, t2.tanggal as batch_tanggal, t1.locked_date
						FROM nd_po_pembelian_warna t1
						LEFT JOIN nd_po_pembelian_batch t2
						ON t1.po_pembelian_batch_id = t2.id
						LEFT JOIN nd_po_pembelian_detail t3
	...
	{
		$query = $this->db->query("SELECT barang_beli_id, barang_id, warna_id, bulan_request, qty, finished_date, finished_by
			FROM nd_request_barang_qty
			WHERE id IN (
				SELECT max(tA.id)
				FROM nd_request_barang_qty tA
				...
				GROUP BY barang_beli_id, barang_id, warna_id, request_barang_id, bulan_request
			)");
		return $query->result();
	}

	function get_latest_request_qty_by_barang($request_barang_id, $barang_id, $warna_id, $bulan_request){
		$query = $this->db->query("SELECT max(tA.id) as id, barang_beli_id, barang_id, warna_id, bulan_request
			FROM nd_request_barang_qty tA
			LEFT JOIN nd_request_barang_batch tB
			ON tA.request_barang_batch_id = tB.id
			WHERE request_barang_id = $request_barang_id
	...
	{
		$query = $this->db->query("SELECT barang_beli_id, t1.barang_id, t1.warna_id, 
			sum(if(po_qty - ifnull(qty,0) - ifnull(qty_before,0) > 0 , po_qty - ifnull(qty,0) - ifnull(qty_before,0) ,0))as qty_outstanding, 
			group_concat(if(po_qty - ifnull(qty,0) - ifnull(qty_before,0) > 0 , po_qty - ifnull(qty,0) - ifnull(qty_before,0) ,0)  order by batch_tanggal,batch asc) as qty_data, 
			group_concat(po_qty order by batch_tanggal,batch asc) as po_qty, 
			group_concat(po_number order by batch_tanggal,batch asc  SEPARATOR '??' ) as po_number, 
			...
			FROM (
				SELECT barang_beli_id, barang_id, warna_id, sum(qty) as po_qty, po_pembelian_batch_id, 
					concat(if(tB.tanggal >= '2019-09-27', concat(ifnull(pre_po,''),tD.kode,'/',DATE_FORMAT(tB.tanggal,'%y'),'/',LPAD(po_number,3,'0')) ,concat(LPAD(po_number,3,'0'),'/',DATE_FORMAT(tB.tanggal,'%m'),'/',DATE_FORMAT(tB.tanggal,'%y'))),'-',batch,if(revisi > 1,concat('R',revisi-1),'') ) as po_number, 
					batch_tanggal, batch, po_pembelian_warna_id, po_pembelian_id
				FROM (
					SELECT *
					FROM (
						SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, 
							if(tipe_barang = 1, t2.barang_beli_id, barang_beli_id_baru) AS barang_beli_id, 
							warna_id, po_pembelian_batch_id, 
							t1.locked_by, t1.qty, t2.po_pembelian_id, batch, revisi, t2.tanggal as batch_tanggal, t1.id as po_pembelian_warna_id, t1.locked_date
						FROM nd_po_pembelian_warna t1
						LEFT JOIN nd_po_pembelian_batch t2
						ON t1.po_pembelian_batch_id = t2.id
	...
	function get_po_outstanding_for_new_request_beli($barang_beli_id_list)
	{

		$this->db->simple_query('SET SESSION group_concat_max_len=15000');

		$query = $this->db->query("SELECT t1.barang_beli_id, t1.barang_id, t1.warna_id, 
			sum(if(po_qty - ifnull(qty,0) - ifnull(qty_before,0) > 0 , po_qty - ifnull(qty,0) - ifnull(qty_before,0) ,0))as qty_outstanding, 
			group_concat(if(po_qty - ifnull(qty,0) - ifnull(qty_before,0) > 0 , po_qty - ifnull(qty,0) - ifnull(qty_before,0) ,0)  order by batch_tanggal,batch asc) as qty_data, 
			group_concat(po_qty order by batch_tanggal,batch asc) as po_qty, 
			group_concat(po_number order by batch_tanggal,batch asc  SEPARATOR '??' ) as po_number, 
			group_concat(batch_tanggal  order by batch_tanggal,batch asc) as batch_tanggal,
			group_concat(t1.po_pembelian_batch_id order by batch_tanggal,batch asc) as po_pembelian_batch_id,
			nd_barang_beli.nama, warna_beli, nama_jual
			FROM (
				SELECT barang_beli_id, barang_id, warna_id, sum(qty) as po_qty, po_pembelian_batch_id, 
					concat(if(tB.tanggal >= '2019-09-27', concat(ifnull(pre_po,''),tD.kode,'/',DATE_FORMAT(tB.tanggal,'%y'),'/',LPAD(po_number,3,'0')) ,concat(LPAD(po_number,3,'0'),'/',DATE_FORMAT(tB.tanggal,'%m'),'/',DATE_FORMAT(tB.tanggal,'%y'))),'-',batch,if(revisi > 1,concat('R',revisi-1),'') ) as po_number, 
					batch_tanggal, batch, po_pembelian_warna_id, po_pembelian_id
				FROM (
					...
					FROM (
						SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, 
							if(tipe_barang = 1, t3.barang_beli_id, barang_beli_id_baru) AS barang_beli_id, 
							warna_id, po_pembelian_batch_id, 
							t1.locked_by, t1.qty, t2.po_pembelian_id, batch, revisi, t2.tanggal as batch_tanggal, t1.id as po_pembelian_warna_id, t1.locked_date
						FROM nd_po_pembelian_warna t1
						LEFT JOIN nd_po_pembelian_batch t2
					...
					WHERE barang_beli_id IN ($barang_beli_id_list)
				)tA
				LEFT JOIN (
					SELECT *
					FROM nd_po_pembelian
			...
			LEFT JOIN nd_warna
			ON t1.warna_id = nd_warna.id
			GROUP BY t1.barang_id, t1.barang_beli_id, t1.warna_id");
...
//==============================================================================================
**/
	function get_po_penjualan($cond){
		// klo mau ordered by nama
		// GROUP_CONCAT(t4.penjualan_id ORDER BY nama_jual,warna_jual SEPARATOR '??') as penjualan_id, 
		// 	GROUP_CONCAT(t2.barang_id  ORDER BY nama_jual,warna_jual SEPARATOR '??') as barang_id, 
		// 	GROUP_CONCAT(t2.warna_id  ORDER BY nama_jual,warna_jual SEPARATOR '??') as warna_id, 
		// 	GROUP_CONCAT(t2.qty  ORDER BY nama_jual,warna_jual SEPARATOR '??') as qty, 
		// 	GROUP_CONCAT(t2.harga  ORDER BY nama_jual,warna_jual SEPARATOR '??') as harga_po,
		// 	GROUP_CONCAT(t4.tanggal_jual  ORDER BY nama_jual,warna_jual SEPARATOR '??') as tanggal_jual,
		// 	GROUP_CONCAT(ifnull(t4.subqty,0)  ORDER BY nama_jual,warna_jual SEPARATOR '??') as subqty,
		// 	GROUP_CONCAT(nama_jual  ORDER BY nama_jual,warna_jual SEPARATOR '??') as nama_barang,
		// 	GROUP_CONCAT(warna_jual  ORDER BY nama_jual,warna_jual SEPARATOR '??') as nama_warna

		$query = $this->db->query("SELECT t1.*, 
			nd_customer.nama as nama_customer, username,
			GROUP_CONCAT(t4.penjualan_id SEPARATOR '??') as penjualan_id, 
			GROUP_CONCAT(t2.barang_id SEPARATOR '??') as barang_id, 
			GROUP_CONCAT(t2.warna_id SEPARATOR '??') as warna_id, 
			GROUP_CONCAT(t2.qty SEPARATOR '??') as qty, 
			GROUP_CONCAT(t2.harga SEPARATOR '??') as harga_po,
			GROUP_CONCAT(t4.tanggal_jual SEPARATOR '??') as tanggal_jual,
			GROUP_CONCAT(ifnull(t4.subqty,0) SEPARATOR '??') as subqty,
			GROUP_CONCAT(nama_jual SEPARATOR '??') as nama_barang,
			GROUP_CONCAT(warna_jual SEPARATOR '??') as nama_warna,
			GROUP_CONCAT(ifnull(t2.closed_by,'-') SEPARATOR '??') as closed_by 
			FROM (
				SELECT *
			...
			LEFT JOIN (
				SELECT barang_id as bid, warna_id as wid, po_penjualan_id as pid,
					group_concat(tanggal) as tanggal_jual, 
					GROUP_CONCAT(penjualan_id) as penjualan_id,
					sum(subqty) as subqty 
				FROM nd_penjualan t3
				LEFT JOIN (
					SELECT barang_id, warna_id, sum(subqty) as subqty, penjualan_id
					FROM nd_penjualan_detail
					GROUP BY barang_id, warna_id, penjualan_id
					) t4
				ON t4.penjualan_id = t3.id
			...
			LEFT JOIN nd_user ON t1.user_id = nd_user.id
			WHERE t1.status_aktif = 1
			$cond
			GROUP BY t1.id
			ORDER BY t1.tanggal desc, po_number asc
		");
	...
	function get_po_penjualan_header($id){
		$query = $this->db->query("SELECT t1.*, nd_customer.nama as nama_customer, username as closed_by_user
			FROM (
				SELECT *
				FROM nd_po_penjualan
				WHERE id=$id
			...
			LEFT JOIN nd_customer ON t1.customer_id = nd_customer.id
			LEFT JOIN nd_user ON t1.closed_by = nd_user.id
		");
	...
	function get_po_penjualan_barang($id){
		$query = $this->db->query("SELECT tA.*, qty_invoice, username
			FROM (
				SELECT t2.id, barang_id, warna_id, qty, po_penjualan_id, harga, t2.keterangan,
				harga as harga_jual,
				nama_jual as nama_barang,
				warna_jual as nama_warna,
				nd_satuan.nama as nama_satuan, t2.closed_by, t2.closed_date
				FROM (
					SELECT *
			...
			LEFT JOIN (
				SELECT barang_id, warna_id, sum(subqty) as qty_invoice, po_penjualan_id
				FROM (
					SELECT *
					FROM nd_penjualan
					WHERE po_penjualan_id = '$id'
			...
			ON tA.po_penjualan_id = tB.po_penjualan_id
			AND tA.barang_id = tB.barang_id
			AND tA.warna_id = tB.warna_id
			LEFT JOIN nd_user
			ON tA.closed_by = nd_user.id
		");

		return $query->result();
	}

	function get_invoice_list_for_po($po_penjualan_id){
			$query = $this->db->query("SELECT t1.id AS id,t1.toko_id ,t1.penjualan_type_id ,t1.no_faktur ,t1.po_number ,t1.tanggal ,
				t1.customer_id ,t1.ppn ,t1.gudang_id,t1.diskon ,t1.jatuh_tempo ,t1.ongkos_kirim,
				t1.keterangan ,t1.nama_keterangan ,t1.alamat_keterangan ,t1.status ,t1.status_aktif ,
				t1.closed_by ,t1.closed_date ,t1.user_id ,t1.created_at ,t1.revisi , t1.fp_status ,
				if((t1.tanggal < '2022-05-01 00:00:00'),concat('FPJ',convert(date_format(t1.tanggal,'%d%m%y') using latin1),'-',ifnull(t2.pre_faktur,''),convert(lpad(t1.no_faktur,5,'0') using latin1)),concat(t2.pre_po,':PJ01/',convert(date_format(t1.tanggal,'%y%m') using latin1),'/',convert(lpad(t1.no_faktur,4,'0') using latin1))) AS no_faktur_lengkap
	...
	function get_barang_invoice_by_po($po_penjualan_id){
		$query = $this->db->query("SELECT penjualan_id, tanggal, barang_id, warna_id, subqty as qty, subjumlah_roll as jumlah_roll, no_faktur_fp
			from nd_penjualan t1 
			left join nd_penjualan_detail t2 
			on t2.penjualan_id = t1.id
			WHERE po_penjualan_id = $po_penjualan_id
	...
	function get_barang_po($po_penjualan_id) { 
		$query = $this->db->query("SELECT t1.*, nama_jual as nama_barang, warna_jual as nama_warna, satuan_id, t4.nama as nama_satuan
			FROM (
				SELECT id, barang_id, warna_id, harga as harga_jual
				FROM nd_po_penjualan_detail
				WHERE po_penjualan_id= $po_penjualan_id
			)t1
			LEFT JOIN nd_barang t2
