		...
		$query = $this->db->query("SELECT barang_id, warna_id, sum(qty_stok) as qty_stok, sum(jumlah_roll_stok) as jumlah_roll_stok, sum(qty_po) as qty_po, group_concat(qty_po_data ) as qty_po_data, sum(if(qty_sisa_po < 0, 0, qty_sisa_po) ) as qty_sisa, group_concat(qty_sisa_data ) as qty_sisa_data, group_concat(batch_id ) as batch_id, group_concat(po_pembelian_id  ) as po_pembelian_id, b.nama as nama_barang, c.warna_beli as nama_warna, d.nama as nama_satuan, group_concat(ifnull(harga,0)) as harga_po, group_concat(ifnull(OCKH,if(OCKH != '',OCKH,0)) SEPARATOR '??') as OCKH, group_concat(locked_by) as locked_by
				FROM(
					(
						...
						SELECT barang_id, warna_id, 0 ,0, sum(qty_po), group_concat(qty_po) ,sum(qty_sisa), group_concat(qty_sisa_data), group_concat(po_pembelian_id) as po_pembelian_id, group_concat(batch_id) as batch_id, group_concat(harga), group_concat(ifnull(OCKH,0)SEPARATOR '??' ), group_concat(locked_by) as locked_by
						FROM (
							...
							group_concat(if(locked_by is null,a.qty-ifnull(qty_beli,0) + ifnull(qty_retur,0) - ifnull(j.qty,0) ,0)) as qty_sisa_data, 
							...
							group_concat(ifnull(locked_by,0)) as locked_by, ifnull(j.qty,0) as qty_2019
							FROM (
								...
								FROM (
									SELECT t1.id as id, po_pembelian_detail_id, po_pembelian_batch_id, if(tipe_barang = 1, t2.barang_id, barang_id_baru) as barang_id, po_pembelian_id, t1.qty, warna_id, if(tipe_barang != 1 AND tipe_barang != 2, harga_baru, harga) as harga, OCKH, locked_by
									FROM (
										SELECT *
...
//==============================Stok & Kartu Stok======================================
	...
	function get_total_stok($select, $tanggal_start, $tanggal_awal, $select2, $new_sum){
				...
				FROM(
					...
					FROM (
						(
							...
							SELECT barang_id, warna_id, t1.gudang_id, 0 as qty_masuk, 0 as jumlah_roll_masuk, qty as qty_keluar, 
							jumlah_roll as jumlah_roll_keluar,20 as tipe, tanggal, t3.id, t2.created_at
							FROM nd_pengeluaran_stok_lain_detail t1
							LEFT JOIN (
								SELECT *
								FROM nd_pengeluaran_stok_lain
		...
		$query = $this->db->query("SELECT tbl_b.nama as nama_barang, tanggal, tbl_c.warna_beli as nama_warna, barang_id, warna_id, qty_masuk, qty_keluar, jumlah_roll_masuk, jumlah_roll_keluar, no_faktur, tipe, trx_id, tbl_a.id, qty_data, roll_data, time_stamp
				FROM(
					(
				        ...
				        SELECT barang_id, warna_id, t1.gudang_id, CAST(0 as DECIMAL(15,2)) as qty_masuk, 0 as jumlah_roll_masuk, qty as qty_keluar, jumlah_roll as jumlah_roll_keluar, tanggal, no_faktur_lengkap, 'a2' as tipe, t2.id as trx_id, t1.id, qty_data, roll_data, ifnull(closed_date, t2.created_at)
				        FROM (
				        	SELECT *
				        	FROM nd_penjualan_detail 
				        	WHERE gudang_id = $gudang_id 
			...
			sum(if(time_stamp >= tanggal_stok, 1,0))
				FROM(
				    (
				        ...
				        SELECT barang_id, warna_id, gudang_id, if(tipe_transaksi = 1,qty,0) , if(tipe_transaksi = 1,jumlah_roll,0), if(tipe_transaksi = 2,qty,0), if(tipe_transaksi = 2,jumlah_roll,0),tanggal, 7, id, created_at
				        FROM (
				            SELECT *
				            FROM nd_penyesuaian_stok
				            WHERE created_at < '$tanggal_start'
				...
				FROM((
						...
						FROM (
							(
						    	...
						    	SELECT barang_id, warna_id, gudang_id, if(tipe_transaksi = 1 || tipe_transaksi = 2,t2.qty, t3.qty) , if(tipe_transaksi = 1,t2.jumlah_roll,0), if(tipe_transaksi = 2, t2.jumlah_roll, if(tipe_transaksi = 3,t3.jumlah_roll,0)) , 'p1', t2.id, tanggal, 'penyesuaian', created_at
						    	FROM (
						    		SELECT *
						    		FROM nd_penyesuaian_stok
							    	WHERE barang_id = $barang_id
					...
					-- batas, di bawah stok_awal
						...
						FROM (
							(
						    	...
						    	SELECT barang_id, warna_id, gudang_id, t2.qty, if(t2.jumlah_roll = 0,1, t2.jumlah_roll),0,'pSp2' as tipe, t2.id, tanggal, created_at
						    	FROM (
						    		SELECT *
						    		FROM nd_penyesuaian_stok
							    	WHERE barang_id = $barang_id
				...
				LEFT JOIN (
					...
					FROM (
						(
							...
							SELECT sum(qty) as qty_beli, sum(qty*(harga_stok_awal*(1+(ppn_berlaku/100) ))) as total_beli, t1.barang_id, t1.warna_id
							FROM (
