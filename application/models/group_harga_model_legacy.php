...
class Group_Harga_Model extends CI_Model {
    ...
    function get_harga_berlaku($id)
    {
		...
		$query = $this->db->query("SELECT nd_barang.id as barang_id, tipe, ifnull(if(tipe=1,harga_cash, harga_kredit), harga_jual) as harga, 
    ...
    function get_harga_baru_launch($id){
        ...
        if(tipe=1, harga_cash_baru, harga_kredit_baru) as harga_baru, 
        if(tipe=1, harga_cash, harga_kredit) as harga_berlaku
        FROM (
            SELECT id, barang_id, harga_cash as harga_cash_baru, harga_kredit as harga_kredit_baru, group_harga_barang_id
            FROM nd_group_harga_baru
            WHERE group_harga_barang_id = $id
        ) t1
        LEFT JOIN (
            SELECT id, barang_id, harga_cash, harga_kredit
            FROM nd_group_harga_berlaku
            WHERE group_harga_barang_id = $id
        ) t2
        ...
        WHERE if(tipe=1, harga_cash_baru, harga_kredit_baru) > 0
		");
		return $query->result();
    }
}