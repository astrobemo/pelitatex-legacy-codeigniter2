...
//==========================================retur beli================================
	...
	function get_retur_beli_list(){
		$query = $this->db->query("SELECT tA.*, tC.nama as nama_supplier, username, created_at as created_date, 
		group_concat(harga) as harga, group_concat(qty) as qty, group_concat(jumlah_roll) as jumlah_roll, nama_barang, 
		group_concat(nama_gudang) as nama_gudang, group_concat(nama_barang) as nama_barang,group_concat(warna_beli) as nama_warna, 
		group_concat(nama_gudang) as nama_gudang, concat('SJ/R/',if(tC.kode is not null, concat(tC.kode,'/'),'LL/'),DATE_FORMAT(tA.tanggal,'%y'),'-',no_sj ) as no_sj_lengkap, po_number
			FROM (
				SELECT *
				FROM nd_retur_beli t1
				WHERE tanggal >= '".$this->tanggal_start_program."'
	...
	function get_outstanding_barang_for_request_2($request_barang_id, $barang_id, $tanggal, $supplier_id){
		...
		FROM (
			...
			FROM (
				SELECT *
				FROM (
					SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, if(t1.tipe_barang!=1,barang_id_baru,barang_beli_id) as barang_beli_id, warna_id, po_pembelian_batch_id, 
