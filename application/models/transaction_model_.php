...
//===============================po pembelian===========================================
		...
		$query = $this->db->query("SELECT *
			FROM (
					...
					if(tanggal >= '2019-09-27', 
						...
						concat_ws('??',tbl_a.id, toko_id, supplier_id, po_pembelian_batch_id, kode) as status_data, 
	...
	function get_data_po_pembelian_detail($po_pembelian_id, $group){
		$query = $this->db->query("SELECT id, if(barang_id != 112 AND nama_tercetak != '37280/60 WRC' AND tanggal >='2022-06-29',nama_tercetak ,'168270/60 WRC') as nama_tercetak, barang_id, warna_id, harga, nama_barang, nama_satuan, nama_show, 
		sum(qty) as qty, group_concat(nama_warna SEPARATOR '||') as nama_warna, 
		group_concat(warna_id SEPARATOR '||') as warna_id, sum(qty_warna_total) as qty_warna_total, 
		group_concat(qty_warna_total SEPARATOR '||') as qty_warna_total_data, 
		group_concat(qty_warna SEPARATOR '||') as qty_warna
			FROM (
				SELECT tbl_a.id, nama_tercetak, barang_id, qty, harga, tbl_b.nama as nama_barang, 
				tbl_c.nama as nama_satuan, if(nama_tercetak = '' || nama_tercetak is null, tbl_b.nama, nama_tercetak) as nama_show
				FROM (
					SELECT *
					FROM nd_po_pembelian_detail
	...
	function get_data_po_pembelian_detail_batch($po_pembelian_batch_id, $po_pembelian_detail_id, $tipe_barang){
		$query = $this->db->query("SELECT if(b.barang_id != 112 AND c.nama != '37280/60 WRC'  AND tanggal >='2022-06-29',c.nama ,'168270/60 WRC')as nama_barang , d.warna_beli as nama_warna, po_pembelian_detail_id, 
		b.barang_id, a.warna_id, a.qty, OCKH, a.id, e.nama as nama_satuan, nama_tercetak, if(a.barang_id_baru!=112  AND nama_baru != '37280/60 WRC'  AND tanggal >='2022-06-29', nama_baru,'168270/60 WRC' ) as nama_baru, tipe_barang, 
	...
	function get_data_barang_po($po_pembelian_id){
		$query = $this->db->query("SELECT tbl_a.*, tbl_b.nama as nama_barang, tbl_c.nama as nama_satuan, 
...
//===============================pembelian===========================================
	...
	function get_pembelian_barang_by_po($po_pembelian_batch_id, $OCKH){
		...
		if(tipe_barang =1, c.nama, nama_baru) as nama , d.warna_beli as nama_warna, 
		ifnull(if(harga_baru = 0, b.harga, harga_baru), b.harga) as harga_beli , 
		tipe_barang, po_pembelian_id, a.qty, locked_by
			FROM (
...
//==========================================PENJUALAN================================
	...
	function get_data_penjualan($id){
		...
		$query = $this->db->query("SELECT tbl_a.id,penjualan_type_id, tanggal, customer_id, no_faktur as no_faktur_raw, 
				...
				ifnull(tbl_e.amount,0) as bayar_dp, no_surat_jalan,if(penjualan_type_id = 3,ifnull(alamat_keterangan,'-') , if (alamat_keterangan != '',alamat_keterangan, alamat)) as alamat_keterangan, 
				toko_id, closed_date, f.username as username, g.username as username_close, 
				'diambil_semua' as status_ambil, alamat_bon, kecamatan, kelurahan, no_packing_list, kode_pos, ppn
			FROM (
				SELECT *, no_sj_lengkap as no_surat_jalan
				FROM vw_penjualan_data
				WHERE id = $id
				) as tbl_a
			LEFT JOIN (
				SELECT id, concat(ifnull(if(tipe_company ='','',concat(tipe_company,' ')),''),nama) as nama,kota, 
					concat(' ',alamat,' ', if(blok = '-' or blok='','',blok),' ', 
						...
						if(kecamatan='-' or kecamatan='','',concat(', ',kecamatan) )  ) as alamat,  
		...
		FROM (
			...
			FROM (
				SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, warna_id, po_pembelian_batch_id, 
					locked_by, t1.qty, t2.po_pembelian_id, batch, revisi, t2.tanggal as batch_tanggal, t1.id as po_pembelian_warna_id
				FROM nd_po_pembelian_warna t1
	...
	function get_outstanding_barang_for_request_2($request_barang_id, $barang_id, $tanggal){
		...
		FROM (
			...
			FROM (
				...
				FROM (
					SELECT if(t1.tipe_barang!=1,barang_id_baru,barang_id) as barang_id, warna_id, po_pembelian_batch_id, 
						locked_by, t1.qty, t2.po_pembelian_id, batch, revisi, t2.tanggal as batch_tanggal, t1.id as po_pembelian_warna_id, locked_date
					FROM nd_po_pembelian_warna t1
					LEFT JOIN nd_po_pembelian_batch t2
					ON t1.po_pembelian_batch_id = t2.id
					LEFT JOIN nd_po_pembelian_detail t3
