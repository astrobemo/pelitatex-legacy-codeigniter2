...
//=====================================================Stok inventory======================================
	...
	function get_batch_for_pre_po($barang_id){
		$query = $this->db->query("SELECT po_number, batch, po_pembelian_id, po_pembelian_batch_id, barang_id, tanggal
			FROM (
				SELECT (a.qty - ifnull(h.qty,0)-ifnull(qty_beli,0)) as qty_sisa, if(tipe_barang = 1,c.barang_id, a.barang_id_baru) as barang_id , a.warna_id, c.po_pembelian_id, concat(if(e.tanggal >= '2019-09-27', concat(ifnull(pre_po,''),g.kode,'/',DATE_FORMAT(e.tanggal,'%y'),'/',LPAD(po_number,3,'0')) ,concat(LPAD(po_number,3,'0'),'/',DATE_FORMAT(e.tanggal,'%m'),'/',DATE_FORMAT(e.tanggal,'%y'))),'-',batch,if(revisi > 1,concat('R',revisi-1),'') ) as batch, e.po_number, a.po_pembelian_batch_id, b.tanggal
				FROM (
					SELECT *
		...
		$query = $this->db->query("SELECT barang_id, warna_id, sum(qty_stok) as qty_stok, sum(jumlah_roll_stok) as jumlah_roll_stok, sum(qty_po) as qty_po, group_concat(qty_po_data ) as qty_po_data, sum(if(qty_sisa_po < 0, 0, qty_sisa_po) ) as qty_sisa, group_concat(qty_sisa_data ) as qty_sisa_data, group_concat(batch_id ) as batch_id, group_concat(po_pembelian_id  ) as po_pembelian_id, b.nama as nama_barang, c.warna_beli as nama_warna, d.nama as nama_satuan, group_concat(ifnull(harga,0)) as harga_po, group_concat(ifnull(OCKH,if(OCKH != '',OCKH,0))) as OCKH, group_concat(locked_by) as locked_by
				FROM((
						...
						LEFT JOIN (
							...
							GROUP BY barang_id, warna_id, gudang_id, tB.id
						) tbl_2
						...
						FROM (
							SELECT sum(a.qty-ifnull(h.qty,0)-ifnull(qty_beli,0) + ifnull(qty_retur,0) ) as qty_sisa, ifnull(a.barang_id_baru, c.barang_id) as barang_id, a.warna_id, c.po_pembelian_id, 
								group_concat(a.po_pembelian_batch_id) as batch_id, 
								group_concat(if(locked_by is null,a.qty-ifnull(qty_beli,0) + ifnull(qty_retur,0) ,0)) as qty_sisa_data, 
								group_concat(if(tipe_barang !=1 AND tipe_barang != 2, harga_baru, harga )) as harga, 
								group_concat(a.qty) as qty_po, group_concat(ifnull(OCKH,0) ) as OCKH,
								group_concat(ifnull(locked_by,0)) as locked_by
							FROM (
								SELECT *
								FROM nd_po_pembelian_warna
								WHERE locked_by is null
							...
							AND i.barang_id = c.barang_id 
							WHERE batch != 0
							AND b.id is not null
							GROUP BY ifnull(a.barang_id_baru, c.barang_id), warna_id, c.po_pembelian_id
						)t1
		...
		$query = $this->db->query("SELECT tbl_b.nama as nama_barang, tanggal, tbl_c.warna_beli as nama_warna, barang_id, warna_id, qty_masuk, qty_keluar, jumlah_roll_masuk, jumlah_roll_keluar, no_faktur, tipe, trx_id, tbl_a.id, qty_data, roll_data
				FROM(
					(
				    	...
				    	SELECT barang_id, warna_id, gudang_id, qty , jumlah_roll, CAST(0 as DECIMAL(15,2)) as qty_keluar, 0 as jumlah_roll_keluar, tanggal, '', 'z1' as tipe, a.id as trx_id,'', group_concat(qty) as qty_data, group_concat(jumlah_roll) as roll_data
				    	FROM (
				    		SELECT stok_opname_id, barang_id, warna_id, gudang_id,  sum(qty*jumlah_roll) as qty, sum(jumlah_roll) as jumlah_roll, id
				    		FROM nd_stok_opname_detail	
				    		WHERE barang_id = $barang_id
				        	AND warna_id = $warna_id
				        	AND gudang_id = $gudang_id
...
//==========================================rekap=========================================================
	...
	function get_stok_barang_list_rekap($select, $select_all, $tanggal, $tanggal_awal, $stok_opname_id){
				...
				FROM(
					...
					FROM (
						(
					        ...
					        SELECT barang_id, warna_id, gudang_id, qty, (jumlah_roll) as jumlah_roll_masuk, CAST(0 as DECIMAL(15,2)) as qty_keluar, 0 as jumlah_roll_keluar, 6, id, tanggal
					        	FROM nd_penyesuaian_stok
					        	WHERE tipe_transaksi = 0
					        	AND tanggal <= '$tanggal'
						        AND tanggal >= '$tanggal_awal'
					    )UNION(
					        SELECT  barang_id, warna_id, gudang_id, qty as qty_masuk, (jumlah_roll) as jumlah_roll_masuk, CAST(0 as DECIMAL(15,2)) as qty_keluar, 0 as jumlah_roll_keluar, 7, id, tanggal
				        	FROM nd_penyesuaian_stok
				        	WHERE tanggal <= '$tanggal'
					        AND tanggal >= '$tanggal_awal'
				        	AND tipe_transaksi = 1
					    )UNION(
					        SELECT barang_id, warna_id, gudang_id, CAST(0 as DECIMAL(15,2)) as qty_masuk, 0 as jumlah_roll_masuk, qty as qty_keluar, (jumlah_roll) as jumlah_roll_keluar, 8 , id, tanggal
				        	FROM nd_penyesuaian_stok
				        	WHERE tanggal <= '$tanggal'
					        AND tanggal >= '$tanggal_awal'
				        	AND tipe_transaksi = 2
				        	...
				        	FROM (
				        		SELECT barang_id, warna_id, gudang_id, sum(qty*if(jumlah_roll=0,1,jumlah_roll)) as qty, sum(jumlah_roll) as jumlah_roll, stok_opname_id, id
				        		FROM nd_stok_opname_detail t1
				        		GROUP BY barang_id, warna_id, gudang_id,stok_opname_id
			        		)t1
				        	LEFT JOIN nd_stok_opname t2
...
//=====================================================Stok + HPP======================================
	...
	function get_stok_barang_list_hpp($select2, $select, $tanggal_start, $kolom, $tgl_tutup_buku){
				...
				LEFT JOIN (
					...
					FROM (
						(
							...
							SELECT sum(qty) as qty_beli, sum(qty*(harga_stok_awal*1.1)) as total_beli, t1.barang_id, t1.warna_id
							FROM (
								SELECT sum(qty*jumlah_roll) as qty, barang_id, warna_id, stok_opname_id
								FROM nd_stok_opname_detail
							...
							ON t1.stok_opname_id = t2.id
							LEFT JOIN (
								SELECT $kolom as harga_stok_awal, barang_id, warna_id
								FROM nd_tutup_buku_detail tA
								WHERE tahun = '$tgl_tutup_buku'
								) t3
...
//=======================================================mutasi================================================
	...
	function get_mutasi_barang_ajax($aColumns, $sWhere/*, $sOrder*/, $sLimit, $cond){
		$query = $this->db->query("SELECT *
			FROM (
				-- @row := @row + 1 as idx, 
				SELECT a.status_aktif, tanggal, concat_ws(' ',e.nama_jual, f.warna_jual)  as nama_barang , c.nama as gudang_before, d.nama as gudang_after, a.qty, a.jumlah_roll, concat_ws('??',a.id,gudang_id_before, gudang_id_after, barang_id,warna_id, qty_data, jumlah_roll_data, detail_id) as data, username
				FROM (
					SELECT * 
					-- , (SELECT @row := 0)
					FROM nd_mutasi_barang
				...
				LEFT JOIN (
					SELECT sum(qty * if(jumlah_roll = 0,1,jumlah_roll)) as qty, sum(jumlah_roll) as jumlah_roll, group_concat(qty) as qty_data, group_concat(jumlah_roll) as jumlah_roll_data, mutasi_barang_id, group_concat(id) as detail_id
					FROM nd_mutasi_barang_qty
					GROUP BY mutasi_barang_id
					) g
...
//==================================mutasi persediaan barang==================
	...
	function mutasi_persediaan_barang($tanggal_awal, $tanggal,$tanggal_end, $gudang_id, $stok_opname_id){
		...
		$query = $this->db->query("SELECT t1.barang_id, t1.warna_id, 
			...
			LEFT JOIN (
				SELECT barang_id, warna_id,sum(qty*ROUND(harga_beli/1.1,2))/sum(qty) as hpp_beli 
				FROM (
					SELECT *
					FROM nd_pembelian
					WHERE tanggal >= '$tanggal'
	...
	function mutasi_persediaan_barang_global_no_warna($tanggal_awal, $tanggal,$tanggal_end, $stok_opname_id, $tutup_buku_id, $kolom_harga){
		...
		$query = $this->db->query("SELECT t1.barang_id, t1.warna_id, sum(qty_stock) as qty_stock, sum(jumlah_roll_stock) as jumlah_roll_stock, sum(qty_beli) as qty_beli, sum(jumlah_roll_beli) as jumlah_roll_beli, sum(qty_mutasi) as qty_mutasi, sum(jumlah_roll_mutasi) as jumlah_roll_mutasi, sum(qty_jual) as qty_jual, sum(jumlah_roll_jual) as jumlah_roll_jual, sum(qty_mutasi_masuk) as qty_mutasi_masuk, sum(jumlah_roll_mutasi_masuk) as jumlah_roll_mutasi_masuk, sum(qty_penyesuaian) as qty_penyesuaian, sum(jumlah_roll_penyesuaian) as jumlah_roll_penyesuaian, sum(qty_retur) as qty_retur, sum(jumlah_roll_retur) as jumlah_roll_retur ,hpp, hpp_beli, t3.nama as nama_barang, nama_jual, warna_jual, t3.satuan_id
			...
			ON t1.barang_id = t2.barang_id
			LEFT JOIN (
				SELECT barang_id, warna_id,sum(qty*(harga_beli/1.1))/sum(qty) as hpp_beli 
				FROM (
					SELECT *
					FROM nd_pembelian
					WHERE tanggal >= '$tanggal'
