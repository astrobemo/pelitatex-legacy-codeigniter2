...
//====================================daftar no rekam faktur pajak======================================
	...
	function get_data_rekam_faktur($rekam_faktur_pajak_id){
		$query = $this->db->query("SELECT t1.*, tanggal_start, tanggal_end
			...
			LEFT JOIN (
				SELECT min(tB.tanggal) as tanggal_start, max(tB.tanggal) as tanggal_end, rekam_faktur_pajak_id
	...
	function get_submitted_pajak_list(){
		$query = $this->db->query("SELECT rekam_faktur_pajak_id, sum(jml_submitted) as jml_submitted, sum(jml_unsubmitted) as jml_unsubmitted
			FROM (
				(
					SELECT rekam_faktur_pajak_id, count(id) as jml_submitted, 0 as jml_unsubmitted
					FROM nd_rekam_faktur_pajak_detail
					WHERE no_faktur_pajak is not null
					AND no_faktur_pajak != ''
					GROUP BY rekam_faktur_pajak_id
				)UNION ALL
				(
					SELECT rekam_faktur_pajak_id, 0 as jml_submitted, count(id) as jml_unsubmitted
					FROM nd_rekam_faktur_pajak_detail
					WHERE no_faktur_pajak is null
					OR no_faktur_pajak = ''
					GROUP BY rekam_faktur_pajak_id
				)
			)res
			WHERE rekam_faktur_pajak_id is not null
			GROUP BY rekam_faktur_pajak_id");
	...
	{
		$query = $this->db->query("SELECT t1.*, nama_cust_fp as nama_customer, t1.tanggal, no_invoice, g_total, count_item, 
		g_total_raw, npwp, nik, t1.customer_id, alamat, kota, provinsi, kode_pos, locked_status as locked_status_cust, 
		email as email_customer, ppn_berlaku_now as ppn_berlaku, alamat_cust_fp, tipe_company
			FROM (
				SELECT *
				FROM nd_rekam_faktur_pajak_detail
			...
			LEFT JOIN (
				...
				FROM (
					...
					FROM (
						...
						no_faktur_fp as  no_faktur_lengkap, nama_cust_fp, alamat_cust_fp
						from nd_penjualan t1
					) t1
					) tA
				LEFT JOIN (
					SELECT sum(subqty * nd_penjualan_detail.harga_jual) as g_total,
					sum(ROUND((subqty * nd_penjualan_detail.harga_jual),2) ) as g_total_raw, penjualan_id, 
					sum(1) as count_item
					FROM nd_penjualan_detail
					GROUP BY penjualan_id
					) tB
				ON tB.penjualan_id = tA.id
				LEFT JOIN (
					SELECT id, ifnull(npwp,'') as npwp, ifnull(nik,'') as nik, tipe_company,
	...
	{
		$query = $this->db->query("SELECT min(t2.tanggal) as tanggal_start, max(t2.tanggal) as tanggal_end
			FROM nd_rekam_faktur_pajak_detail t1
	...
	function get_faktur_to_rekam_satuan($penjualan_id){
		$query = $this->db->query("SELECT t1.*, nama_cust_fp as nama_customer, g_total as total_jual, g_total*(ppn_berlaku_now/100) as total_ppn
		FROM (
			SELECT t1.*, g_total,
				(SELECT ppn FROM nd_ppn tX WHERE tX.tanggal <= t1.tanggal ORDER BY tX.tanggal DESC LIMIT 1 ) as ppn_berlaku_now
...
//====================================data download faktur pajak=======================
	...
	function get_item_penjualan_list($penjualan_id_list){
		...
		penjualan_id,jenis_barang as nama_barang, if(t1.nama_jual_tercetak is null OR t1.nama_jual_tercetak = '' OR t1.nama_jual_tercetak = '0',t3.nama_jual, 
		nama_jual_tercetak) as kode_barang, barang_id, ppn_berlaku, t4.nama as nama_satuan
	...
	function get_data_penjualan_list($penjualan_id_list){
		$query = $this->db->query("SELECT no_faktur_pajak, MONTH(t1.tanggal) as MASA_PAJAK, YEAR(t1.tanggal) as TAHUN_PAJAK, 
		DATE_FORMAT(t1.tanggal,'%d/%m/%Y') as TANGGAL_FAKTUR, if(penjualan_type_id != 3 , t2.npwp ,'ERROR') as NPWP, 
		if(penjualan_type_id != 3 , t1.nama_cust_fp ,'ERROR') as NAMA, 
		if(penjualan_type_id != 3 , t1.alamat_cust_fp ,'ERROR') as ALAMAT_LENGKAP, 
		...
		(SELECT ppn FROM nd_ppn tX WHERE tX.tanggal <= t1.tanggal ORDER BY tX.tanggal DESC LIMIT 1 ) as ppn_berlaku
			...
			LEFT JOIN (
				SELECT id, ifnull(npwp,'') as npwp, ifnull(nik,'') as nik, concat(if(tipe_company != '' OR tipe_company != null,concat(tipe_company,' '),''),nama) as nama, 
				if(kode_pos='0' or kode_pos='',null, kode_pos) as kode_pos, kota,  provinsi, 
					concat(alamat,' ',
						concat('BLOK ', if(blok='','-',blok)),' ',
						concat('NO.',if(no='','-',no)),' ',
...
//====================================data email=============================================
	...
	function get_data_dari_no_faktur($id, $no_faktur){
		$query = $this->db->query("SELECT nama_customer as nama, email, no_npwp, no_nik, 
		group_concat(no_faktur_pajak SEPARATOR '??') as no_faktur_pajak, t2.customer_id, t2.tanggal as tanggal_invoice
			FROM (
				SELECT *
				FROM nd_rekam_faktur_pajak_detail
				WHERE rekam_faktur_pajak_id = $id
	...
	function get_rekam_faktur_pajak_email_legacy($id, $cond){
		...
		group_concat(no_faktur_pajak SEPARATOR '??') as no_faktur_pajak, t2.customer_id, t4.updated_at as waktu_kirim, 
		t4.id as kirim_id , group_concat(t2.tanggal) as tanggal_invoice, 
	...
	function get_laporan_pajak($cond){
		$query = $this->db->query("SELECT concat(DATE_FORMAT(tanggal_start,'%M %Y')) as tanggal_faktur, 
			...
			FROM(
				...
				FROM (
					SELECT sum(1) as jumlah_faktur, t2.id, t2.created_at, no_surat, MIN(t1.tanggal) as tanggal_start, max(t1.tanggal) as tanggal_end, locked_date
					FROM (
						SELECT *
						FROM nd_rekam_faktur_pajak_detail
						$cond
				...
				LEFT JOIN (
					...
					FROM (
						...
						FROM (
							SELECT t1.customer_id, rekam_faktur_pajak_id, sum(1) as jml_npwp
							FROM nd_rekam_faktur_pajak_detail t1
							LEFT JOIN nd_penjualan t2
							ON t1.penjualan_id = t2.id
	...
	function get_rekam_faktur_pajak_list_by_id_legacy($id){
		$query = $this->db->query("SELECT tA.*, tB.*, tC.no_faktur_pajak as no_fp_awal, tD.no_faktur_pajak as no_fp_akhir
			...
			LEFT JOIN (
				...
				rekam_faktur_pajak_id, min(t1.tanggal) as tanggal_start, max(t1.tanggal) as tanggal_end
				FROM (
					SELECT *
					FROM nd_rekam_faktur_pajak_detail
					) t1
				LEFT JOIN (
					SELECT tA.id, g_total
					FROM nd_penjualan tA
					LEFT JOIN (
...
function get_rekam_faktur_belum_no_fp($tanggal_start, $tanggal_end){
	$query = $this->db->query("SELECT penjualan.*
		FROM (
			SELECT *
			FROM nd_penjualan
			WHERE tanggal >= '$tanggal_start'
		...
		LEFT JOIN (
			SELECT penjualan_id, no_faktur_pajak
			FROM nd_rekam_faktur_pajak_detail detail
			LEFT JOIN nd_rekam_faktur_pajak header
			ON detail.rekam_faktur_pajak_id = header.id
			WHERE header.is_gunggung = 0
		)faktur_detail
		ON penjualan.id = faktur_detail.penjualan_id
		LEFT JOIN (
			SELECT penjualan_id, no_faktur_pajak
			FROM nd_rekam_faktur_pajak_detail detail
			LEFT JOIN nd_rekam_faktur_pajak header
			ON detail.rekam_faktur_pajak_id = header.id
			WHERE header.is_gunggung = 1
		)faktur_detail_gunggung
		ON penjualan.id = faktur_detail_gunggung.penjualan_id
		WHERE (
			faktur_detail.no_faktur_pajak is null
			OR faktur_detail.no_faktur_pajak = ''
		)AND (
			faktur_detail_gunggung.no_faktur_pajak is null
			OR faktur_detail_gunggung.no_faktur_pajak = ''
		)
