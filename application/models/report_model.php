...
class Report_Model extends CI_Model {
	...
	function get_penjualan_report($cond,$gudang_cond, $barang_id_cond, $warna_id_cond, $id_list){
		...
		$query = $this->db->query("SELECT *
			FROM (
				SELECT tbl_a.id, no_faktur as nf, tbl_a.status_aktif,no_faktur_pajak,
					...
					ON tbl_d.penjualan_id = tbl_a.id
					LEFT JOIN (
						SELECT penjualan_id, sum(amount) as total_lunas, group_concat(amount order by t1.id ASC) as data_lunas, group_concat(pembayaran_piutang_id order by t1.id ASC) as pembayaran_piutang_id, group_concat(ket_lunas order by t1.id ASC) as ket_lunas
					...
					ON tbl_e.penjualan_id = tbl_f.penjualan_id
	...
	function get_so_report($tanggal_start, $tanggal_end, $barang_cond, $warna_cond, $gudang_cond){
		...
		$query = $this->db->query("SELECT t1.*, t2.*, 
			...
			FROM (
				...
				group_concat(ifnull(t2.stok_current,0) order by nama_jual, warna_jual asc) as qty_current, 	
				group_concat(ifnull(t2.roll_current,0) order by nama_jual, warna_jual asc) as jumlah_roll_current, 
