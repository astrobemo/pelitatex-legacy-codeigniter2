		...
		$query = $this->db->query("SELECT *
			FROM (
				SELECT tbl_a.id, no_faktur as nf, tbl_a.status_aktif, no_faktur_lengkap as no_faktur, 
					...
					ifnull(ifnull(if(g_total - ifnull(amount_kontra_tanggung,0) < 1, 'lunas1',null),if(total_lunas < 1, 'belum lunas',ket_lunas)), if((ifnull(total_bayar,0) + ifnull(total_lunas,0) - (ifnull(g_total,0) - ifnull(diskon,0)) + ifnull(ongkos_kirim,0)) >= 1,'lunas1','belum lunas' ) ) as ket_lunas, 
					amount_kontra_tanggung, customer_id, closed_date
					FROM (
						SELECT group_concat(concat_ws(' ',nd_barang.nama,warna_jual) ORDER BY t1.id SEPARATOR '??' ) as nama_barang, group_concat(t1.harga_jual ORDER BY t1.id SEPARATOR '??') as harga_jual, group_concat(qty ORDER BY t1.id SEPARATOR '??') as qty ,group_concat(if(nd_barang.satuan_id != 3 , jumlah_roll,0) ORDER BY t1.id SEPARATOR '??') as jumlah_roll, 
						...
						group_concat(nd_gudang.nama ORDER BY t1.id SEPARATOR '??') as nama_gudang, group_concat(nd_barang.satuan_id ORDER BY t1.id SEPARATOR '??') as satuan_id
						FROM (
							SELECT *
							FROM nd_penjualan_detail
							$gudang_cond
...
//=======================================get PO Gantung=============================
	...
	function po_pembelian_report($cond){
		$query = $this->db->query("SELECT po_number, nama_supplier, tanggal, result.po_pembelian_id, 
