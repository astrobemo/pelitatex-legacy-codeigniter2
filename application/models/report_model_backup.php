...
class Report_Model extends CI_Model {
	...
	function get_penjualan_report($from, $to, $cond, $customer_cond, $gudang_cond, $barang_id_cond, $warna_id_cond, $tipe_penjualan_cond){
		$c='';
		if (is_posisi_id() == 1) {
		...
		$query = $this->db->query("SELECT *
			FROM (
				SELECT tbl_a.id, no_faktur as nf, tbl_a.status_aktif, no_faktur_lengkap as no_faktur, 
					tanggal, qty, jumlah_roll, nama_barang, nama_jual, harga_jual, total, diskon, 
					...
					ifnull(ifnull(if(g_total - ifnull(amount_kontra_tanggung,0) = 0, 'lunas1',null),if(total_lunas =0, 'belum lunas',ket_lunas)), if((ifnull(total_bayar,0) + ifnull(total_lunas,0) - (ifnull(g_total,0) - ifnull(diskon,0)) + ifnull(ongkos_kirim,0)) >= 0,'lunas1','belum lunas' ) ) as ket_lunas, amount_kontra_tanggung, customer_id, closed_date
					FROM (
						SELECT group_concat(concat_ws(' ',nd_barang.nama,warna_jual) ORDER BY t1.id SEPARATOR '??' ) as nama_barang, group_concat(t1.harga_jual ORDER BY t1.id SEPARATOR '??') as harga_jual, group_concat(qty ORDER BY t1.id SEPARATOR '??') as qty ,group_concat(if(nd_barang.satuan_id != 3 , jumlah_roll,0) ORDER BY t1.id SEPARATOR '??') as jumlah_roll, group_concat((qty * t1.harga_jual)  ORDER BY t1.id SEPARATOR '??') as total, sum(qty * t1.harga_jual) as g_total, penjualan_id, group_concat(concat_ws(' ',nama_jual,warna_jual)  ORDER BY t1.id SEPARATOR '??') as nama_jual, group_concat(nd_satuan.nama ORDER BY t1.id SEPARATOR '??') as nama_satuan, group_concat(nd_gudang.nama ORDER BY t1.id SEPARATOR '??') as nama_gudang, group_concat(nd_barang.satuan_id ORDER BY t1.id SEPARATOR '??') as satuan_id
						FROM (
							SELECT *
							FROM nd_penjualan_detail
							$gudang_cond
		...
		$query = $this->db->query("SELECT tbl_a.id, tbl_a.status_aktif, no_faktur_lengkap as no_faktur, tanggal, qty, jumlah_roll, nama_barang, harga_jual, total, diskon, ongkos_kirim, if(customer_id != 0, tbl_c.nama, concat(nama_keterangan, ' (non-pelanggan)')) as nama_customer, (ifnull(total_bayar,0) - (ifnull(g_total,0) - ifnull(diskon,0)) + ifnull(ongkos_kirim,0)) as keterangan, tbl_a.id as data , jatuh_tempo, hpp
				...
				LEFT JOIN (
					SELECT group_concat(concat_ws(' ',nama,warna_jual) SEPARATOR '??') as nama_barang, 
						group_concat(nd_penjualan_detail.harga_jual SEPARATOR '??') as harga_jual, 
						group_concat(qty SEPARATOR '??') as qty ,
						group_concat(jumlah_roll SEPARATOR '??') as jumlah_roll, 
						group_concat((qty *nd_penjualan_detail.harga_jual) SEPARATOR '??') as total, 
						sum(qty *nd_penjualan_detail.harga_jual) as g_total, penjualan_id, 
					...
					LEFT JOIN (
						...
						FROM (
							(
								...
								SELECT sum(qty) as qty_beli, sum(qty*ifnull(harga_stok_awal,0)) as total_beli, stok_awal.barang_id, stok_awal.warna_id
								FROM (
								...
								SELECT sum(qty) as qty, sum(qty * (ifnull(harga,0) * 1.1) ), t1.barang_id, t1.warna_id
								FROM (
									SELECT sum(qty * if(jumlah_roll = 0,1, jumlah_roll)) as qty, barang_id, warna_id
									...
									GROUP BY barang_id, warna_id, stok_opname_id
								) t1
								LEFT JOIN (
...
//==================================pembelian list==========================================
	...
	function get_pembelian_report($from, $to, $cond, $cond_barang, $cond_warna, $cond_gudang){
		$query = $this->db->query("SELECT tbl_a.id, tbl_a.status_aktif, no_faktur, tanggal, qty, jumlah_roll, nama_barang, nama_jual, harga_beli, total, diskon, ifnull(tbl_c.nama,'no name') as nama_supplier, (ifnull(total_bayar,0) - (ifnull(g_total,0) - ifnull(diskon,0))) as keterangan, tbl_a.id as data , jatuh_tempo, tbl_e.nama as nama_gudang, pembayaran_hutang_id, tanggal_bayar, supplier_id, tbl_b.satuan_id, po_pembelian_batch_id, po_number
				FROM (
					SELECT *
					FROM nd_pembelian 
					WHERE tanggal >= '$from'
				...
				LEFT JOIN (
					SELECT group_concat(concat_ws(' ',nama,warna_beli) SEPARATOR '??') as nama_barang, group_concat(concat_ws(' ',nama_jual,warna_jual) SEPARATOR '??') as nama_jual,  group_concat(nd_pembelian_detail.harga_beli SEPARATOR '??') as harga_beli, group_concat(qty SEPARATOR '??') as qty ,group_concat(jumlah_roll SEPARATOR '??') as jumlah_roll, group_concat((qty *nd_pembelian_detail.harga_beli) SEPARATOR '??') as total, sum(qty *nd_pembelian_detail.harga_beli) as g_total, pembelian_id, group_concat(nd_barang.satuan_id SEPARATOR '??') as satuan_id
					FROM (
						SELECT qty, jumlah_roll, pembelian_id, barang_id, warna_id, harga_beli
						FROM (
							SELECT *
	...
	function get_pembelian_report_excel($from, $to, $cond, $cond_barang, $cond_warna, $cond_gudang){
		$query = $this->db->query("SELECT no_faktur, tanggal, qty, jumlah_roll, nama_barang, nama_jual, harga_beli, total, diskon, ifnull(tbl_c.nama,'no name') as nama_supplier, (ifnull(total_bayar,0) - (ifnull(g_total,0) - ifnull(diskon,0))) as keterangan, tbl_a.id as data , jatuh_tempo, tbl_e.nama as nama_gudang, satuan_id
				FROM (
					SELECT group_concat(concat_ws(' ',nama,warna_beli) SEPARATOR '??') as nama_barang, group_concat(concat_ws(' ',nama_jual,warna_jual) SEPARATOR '??') as nama_jual,  group_concat(nd_pembelian_detail.harga_beli SEPARATOR '??') as harga_beli, group_concat(qty SEPARATOR '??') as qty ,group_concat(jumlah_roll SEPARATOR '??') as jumlah_roll, group_concat((qty *nd_pembelian_detail.harga_beli) SEPARATOR '??') as total, sum(qty *nd_pembelian_detail.harga_beli) as g_total, pembelian_id, group_concat(nd_barang.satuan_id SEPARATOR '??') as satuan_id
					FROM (
						SELECT *
...
//==========================================penerimaan list================================
	function get_penjualan_bayar_by_date($tanggal_start, $tanggal_end){
		$query = $this->db->query("SELECT tbl_a.status_aktif, tbl_a.tanggal, no_faktur_lengkap as no_faktur, no_faktur as faktur, group_concat(pembayaran_type_id) as pembayaran_type_id, group_concat(amount) as bayar , g_total as amount, if (penjualan_type_id = 3, nama_keterangan, concat(ifnull(tipe_company,''),' ',tbl_c.nama) ) as nama_customer, tbl_b.keterangan as keterangan_transfer, tbl_a.id, sum(ifnull(amount,0)) - g_total as kembali, fp_status
			FROM (
...
//=======================================retur beli=======================
	...
	function get_pembelian_retur($from, $to, $cond, $cond_barang, $cond_warna, $cond_gudang){
...
//==================================barang keluar list==========================================
	...
	function get_barang_keluar_report($tanggal_start, $tanggal_end, $cond, $cond2, $group_by){
		$query = $this->db->query("SELECT GROUP_CONCAT(ifnull(qty,0) SEPARATOR '??') as qty, GROUP_CONCAT(ifnull(jumlah_roll,0) SEPARATOR '??') as jumlah_roll, GROUP_CONCAT(ifnull(count ,0) SEPARATOR '??') as count, GROUP_CONCAT(ifnull(bulan,0)  SEPARATOR '??') as bulan, nama_warna, nama_jual, GROUP_CONCAT(ifnull(harga_rata,0)  SEPARATOR '??') as harga_rata
			FROM (
					(
						SELECT sum(qty) as qty, sum(jumlah_roll) as jumlah_roll, c.nama_jual as nama_jual, d.warna_jual as nama_warna, sum(1) as count,barang_id, warna_id, sum(b.harga_jual*qty) / sum(qty) as harga_rata, MONTH(a.tanggal) as bulan, 1 as tipe
						FROM (
							SELECT *
							FROM nd_penjualan 
							WHERE tanggal >= '$tanggal_start'
						...
						SELECT sum(qty) as qty, sum(jumlah_roll) as jumlah_roll, c.nama_jual as nama_jual, d.warna_jual as nama_warna, sum(1) as count,barang_id, warna_id, sum(b.harga_jual*qty) / sum(qty) as harga_rata, MONTH(a.tanggal) as bulan, 2
						FROM (
							SELECT *
...
//================================buku laporan penyesuaian stok=============================
	...
	function get_penyesuaian_stok($select, $tanggal_start, $tanggal_end, $cond_gudang, $cond_barang, $cond_warna){
		...
		$query = $this->db->query("SELECT t2.nama as nama_barang,t2.nama_jual as nama_jual, t3.warna_beli as nama_warna, t4.nama as nama_satuan, sum(qty_masuk) as qty_masuk, sum(qty_keluar) as qty_keluar, sum(jumlah_roll_masuk) as jumlah_roll_masuk, sum(jumlah_roll_keluar) as jumlah_roll_keluar
			FROM (
				(
					select barang_id, gudang_id, warna_id, qty * if(jumlah_roll=0,1,jumlah_roll) as qty_masuk, 0 as qty_keluar, jumlah_roll as jumlah_roll_masuk, 0 as jumlah_roll_keluar
					from nd_penyesuaian_stok
					...
					select barang_id, gudang_id, warna_id, 0, qty* if(jumlah_roll=0,1,jumlah_roll), 0, jumlah_roll
					from nd_penyesuaian_stok
...
//=======================================get PO Gantung=============================
	...
	function po_pembelian_report($cond){
		$query = $this->db->query("SELECT po_number, nama_supplier, tanggal, result.po_pembelian_id, result.id,supplier_id,
			...
			group_concat(locked_date ORDER BY nama_barang, nama_warna) as locked_po,
			...
			FROM (
				...
				ifnull(a.qty,0) as po_qty,
					...
					warna_beli as nama_warna, ifnull(locked_date,'-') as locked_date, a.id as po_warna_id,
					ifnull(a.harga_baru, c.harga) as harga_barang, last_datang
					 
				FROM (
					SELECT t1.*, concat(if(t2.tanggal >= '2019-09-27', concat(ifnull(pre_po,''),t4.kode,'/',DATE_FORMAT(t2.tanggal,'%y'),'/',LPAD(po_number,3,'0')) ,concat(LPAD(po_number,3,'0'),'/',DATE_FORMAT(t2.tanggal,'%m'),'/',DATE_FORMAT(t2.tanggal,'%y'))),'-',batch,if(revisi > 1,concat('R',revisi-1),'') ) as po_number, t4.nama as nama_supplier, supplier_id
					FROM (
						SELECT *
				...
				ON a.po_pembelian_detail_id = c.id
				LEFT JOIN (
					SELECT barang_id, warna_id, sum(ifnull(qty,0) ) as qty_beli, po_pembelian_batch_id, max(t2.created_at) as last_datang
					FROM nd_pembelian_detail t1
					LEFT JOIN nd_pembelian t2
					ON t1.pembelian_id = t2.id
				...
				LEFT JOIN (
					SELECT tA.id, qty_beli,qty_retur, tC.nama as nama_barang_baru, tA.warna_id, tA.tipe_barang, barang_id_baru
					FROM (
						SELECT *
						FROM nd_po_pembelian_warna
						WHERE tipe_barang != 1
	...
	function po_pembelian_detail_report($id){
			$query = $this->db->query("SELECT if(a.tipe_barang = 1, ifnull(d.qty_beli,0), ifnull(tA.qty_beli,0))  as qty_beli, 
				...
				LEFT JOIN (
					SELECT tA.*, tB.nama as nama_baru, tC.username, warna_beli
				...
				LEFT JOIN (
					SELECT tA.id, qty_beli,qty_retur, no_faktur, tanggal_beli, tanggal_datang, pembelian_id, harga_beli, barang_id_baru, tA.harga_baru as harga, tA.warna_id, last_datang
					FROM (
						SELECT *
						FROM nd_po_pembelian_warna
						WHERE tipe_barang != 1
...
//================================so report======================================
	...
	function get_so_report($tanggal_start, $tanggal_end, $barang_cond, $warna_cond, $gudang_cond){
		...
		$query = $this->db->query("SELECT t1.*, t2.*, concat(pre_po,'-',YEAR(t1.created_at),'/41/',LPAD(no_surat,3,'0') ) as no_so
			FROM nd_stok_opname_report t1
			LEFT JOIN (
				SELECT group_concat(nama_jual) as nama_barang, group_concat(warna_jual) as nama_warna, group_concat(nd_gudang.nama) as nama_gudang, group_concat(qty) as qty_data, group_concat(jumlah_roll) as jumlah_roll_data, stok_opname_report_id, GROUP_CONCAT(DATE_FORMAT(t2.created_at,'%d/%m/%y %H:%i')) as tanggal_so, pre_po
				FROM (
...
//================================po request report======================================
	...
	function get_request_barang_data($request_barang_id){
		$query = $this->db->query("SELECT t2.id as id, no_request, supplier_id, t4.nama as nama_supplier, 
		...
		FROM (
			(
				SELECT supplier_id, barang_id,nama as nama_barang,  request_barang_id,
					sum(qty) as qty_request, 
					sum(ifnull(qty_datang,0)) as qty_datang, 
					group_concat(warna_id ORDER BY warna_jual ASC) as warna_id, 
					group_concat(warna_jual ORDER BY warna_jual ASC) as nama_warna, 
					group_concat(qty ORDER BY warna_jual ASC) as qty_data_request, 
					group_concat(qty_datang ORDER BY warna_jual ASC) as qty_data_datang,
					'' as warna_id_no_request, 0 as qty_datang_no_request,  '' as qty_data_datang_no_request, '' as nama_warna_no_request
				FROM (
					SELECT t1.barang_id, t1.warna_id, t1.qty, 
					sum(ifnull(t2.qty,0)) as qty_datang, request_barang_id, closed_date, t1.tanggal_start as tanggal, 
					max(created_at) as last_datang, supplier_id
					FROM (
						SELECT t11.*, tanggal_start, tanggal_end
						FROM (
							SELECT barang_id, min(tB.tanggal) as tgl, warna_id, sum(qty) as qty, group_concat(qty ORDER BY bulan_request ASC SEPARATOR '||') as qty_data, 
							request_barang_batch_id, closed_date, tB.request_barang_id, supplier_id, 
						...
						LEFT JOIN (
							SELECT min(DATE_ADD(bulan_request,interval 1 year)) as tanggal_start, max(DATE_ADD(bulan_request,interval 1 year)) as tanggal_end,
						...
						ON t11.request_barang_id = t12.request_barang_id
					)t1
					LEFT JOIN (
					...
					AND t1.tanggal_start <= t2.created_at
					GROUP BY t1.barang_id, t1.warna_id, request_barang_id 
				)result
				LEFT JOIN nd_barang
				ON result.barang_id = nd_barang.id 
					...
					FROM (
						...
						FROM (
							SELECT t1.barang_id, t1.warna_id, t1.qty, 
									...
									group_concat(ifnull(DATE_FORMAT(tanggal_beli, '%Y-%m-%d'),'x') order by tanggal_beli ASC SEPARATOR '||') as bulan_datang, 
									request_barang_id, 
									closed_date, t1.tanggal_start as tanggal, max(tanggal_beli) as last_datang, supplier_id, qty_data, bulan_request
									FROM (
										SELECT t11.*, tanggal_start as tanggal_start, tanggal_end
										FROM (
									...
									ON t1.barang_id = t2.barang_id
									AND t1.warna_id = t2.warna_id
									AND t1.tanggal_end >= t2.tanggal_beli
									AND t1.tanggal_start <= t2.tanggal_beli
									GROUP BY t1.barang_id, t1.warna_id, request_barang_id
						)result
						LEFT JOIN nd_barang
						ON result.barang_id = nd_barang.id 
	...
	function get_po_request_report_detail_3($request_barang_id){
		$this->db->simple_query('SET SESSION group_concat_max_len=15000');
		$tgl= date('Y-m-d H:i:s');
		$query = $this->db->query("SELECT supplier_id, request_barang_id, barang_id, 
			DATE_ADD(group_concat(bulan_request_awal SEPARATOR ''),interval 1 year) as bulan_request_awal, tanggal_request_awal,
