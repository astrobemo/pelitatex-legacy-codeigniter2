...
//====================================hutang======================================
    ...
    function get_hutang_list($jatuh_tempo){
        $query = $this->db->query("SELECT a.nama as nama_supplier, b.*, amount_outstanding, jml, sisa_hutang_awal, sisa_retur
            ...
            LEFT JOIN (
                ...
                FROM (
                    (
                        SELECT tbl_a.status_aktif, supplier_id, sum(total) as total_beli, sum(total_bayar) as total_bayar, sum(ifnull(round(total,0),0)) - sum(ifnull(total_bayar,0)) as sisa_hutang
                        FROM (
    ...
    function get_hutang_list_detail($supplier_id){
        $query = $this->db->query("SELECT t1.*, t2.nama as supplier
            FROM (
                (
                    SELECT tanggal, no_faktur, supplier_id, total as total_beli, total_bayar, ifnull(total,0) - ifnull(total_bayar,0) as sisa_hutang, tbl_a.id, 1 as tipe, ockh_info as OCKH
                    FROM (
                        SELECT *
                        FROM nd_pembelian
                        WHERE supplier_id = $supplier_id
    ...
    function get_giro_keluar_belum_cair($supplier_id, $jatuh_tempo){
...
//==============================piutang=========================================================
    ...
    function get_piutang_list_by_date($tanggal_start, $tanggal_end, $toko_id, $customer_id, $cond_jt){
        $query = $this->db->query("SELECT tbl_a.status_aktif, no_faktur_lengkap as no_faktur, tbl_e.nama as customer, customer_id, total as total_jual, 0 as amount, amount_bayar, ifnull(total,0) - ifnull(amount,0) - ifnull(amount_bayar, 0) as sisa_piutang, tbl_a.id as penjualan_id, new_jatuh_tempo as jatuh_tempo, tanggal
                FROM (
                    SELECT *, 
                    if(jatuh_tempo = tanggal, DATE_ADD(jatuh_tempo, INTERVAL 60 DAY), jatuh_tempo ) as new_jatuh_tempo
                    FROM vw_penjualan_data
    ...
    function get_piutang_list_by_tgl_trx($tanggal){
        $query = $this->db->query("SELECT *
            FROM (
                SELECT outstanding, t1.customer_id, t2.nama as nama_customer, t3.nama as nama_toko, 
                sum(sisa_piutang) - ifnull(total_bayar,0) as sisa_piutang, MIN(tanggal_start) as tanggal_start, MAX(tanggal_end) as tanggal_end, 
                toko_id, sum(1) as jml_trx
                FROM (
                    (
...
//===============================piutang payment=============================
                ...
                FROM (
                    ...
                    FROM (
                        (
                            ...
                            SELECT tbl_b.id, customer_id, toko_id, 0, tanggal_transfer
                            FROM (
                                SELECT *
                                FROM nd_pembayaran_piutang_nilai
                                WHERE tanggal_transfer >= '$tanggal_start'
    ...
    function get_pembayaran_retur_detail($id){
...
//===============================daftar mutasi hutang=============================
    ...
    function get_mutasi_hutang_list_detail($supplier_id, $toko_id, $tanggal_start, $tanggal_end){
        $query = $this->db->query("SELECT *
            FROM  (
            (
                SELECT nd_pembelian.tanggal, no_faktur, ceil(amount_beli) as amount_beli, 0 as amount_bayar, if(amount_beli - bayar = 0, 1, 2) as status_lunas, nd_pembelian.id as pembelian_id, '' as detail_id,'' as data_status
                FROM (
                    SELECT *
                    FROM nd_pembelian
                    WHERE supplier_id = $supplier_id 
                ...
                SELECT tanggal_transfer,no_faktur,0, amount, 0 as status_lunas, '', t2.id, data_status
                FROM (
                    SELECT *
                    FROM nd_pembayaran_hutang_nilai 
                    WHERE tanggal_transfer >= '$tanggal_start'
                ...
                SELECT tanggal_transfer,no_faktur,0,amount_retur , 0 as status_lunas, '', t2.id, data_status
                FROM (
                    SELECT *
                    FROM nd_pembayaran_hutang_nilai 
                    WHERE tanggal_transfer >= '$tanggal_start'
                ...
                SELECT tanggal_transfer,'pembulatan',0, pembulatan, 0 as status_lunas, '', t1.id, ''
                FROM (
                    SELECT *
                    FROM nd_pembayaran_hutang
                    WHERE supplier_id = $supplier_id
                ...
                SELECT DATE(created),'pembulatan',0, pembulatan, 0 as status_lunas, '', t1.id, ''
                FROM (
                    SELECT *
                    FROM nd_pembayaran_hutang
                    WHERE supplier_id = $supplier_id
    ...
    function get_mutasi_hutang_detail_saldo_awal($supplier_id, $toko_id, $tanggal_start){
        $query = $this->db->query("SELECT sum(ifnull(amount_beli,0)) - sum(ifnull(amount_bayar,0)) as saldo_awal
            FROM  (
                (
                    SELECT t1.tanggal, no_faktur, amount_beli, 0 as amount_bayar, t1.id
                    FROM (
                        SELECT *
                        FROM nd_pembelian
                        WHERE supplier_id = $supplier_id 
                    ...
                    SELECT tanggal_transfer,t1.id,0, amount, t2.id
                    FROM (
                        SELECT *
                        FROM nd_pembayaran_hutang
                        WHERE supplier_id = $supplier_id
                    ...
                    SELECT tanggal, concat(no_faktur, '(hutang awal)') , amount, 0 as amount_bayar, t1.id
                    FROM nd_hutang_awal t1
                    WHERE supplier_id = $supplier_id
                    AND toko_id = $toko_id
                    AND tanggal < '$tanggal_start'
...
//===============================giro register list=============================
    ...
    function get_giro_register(){
        $query = $this->db->query("SELECT t1.*, lbr, ifnull(lbr_batal,0) as lbr_batal, nama_bank, no_rek_bank, tipe_trx_1, tipe_trx_2
            FROM nd_giro_list t1
            LEFT JOIN (
                SELECT sum(lbr) as lbr, giro_list_id, group_concat(lbr) as lbr_data, group_concat(tipe) as tipe, sum(if(tipe=3,lbr,0)) as lbr_batal
                FROM (
                        (
                            SELECT sum(1) as lbr, 1 as tipe, giro_list_id
                            FROM nd_giro_list_detail
                            ...
                            SELECT sum(1) as lbr, 3 as tipe, giro_list_id
                            FROM nd_giro_list_detail
                            WHERE status = 0
                            GROUP BY giro_list_id
                        )UNION(
                            SELECT sum(1) as lbr, 4 as tipe, giro_register_id as giro_list_id
                            FROM nd_pembayaran_hutang_nilai
                            WHERE pembayaran_type_id = 5
                            GROUP BY giro_register_id
                        )UNION(
                            SELECT sum(1) as lbr, 2 as tipe, giro_register_id as giro_list_id
                            FROM nd_pembayaran_hutang_nilai
                            WHERE pembayaran_type_id = 2
                            GROUP BY giro_register_id
                        )
    ...
    function get_giro_register_detail($id, $pembayaran_type_id){
        $query = $this->db->query("SELECT t1.*, username
            FROM (
                (
                    SELECT pembayaran_hutang_id, giro_register_id, amount, jatuh_tempo, tanggal_transfer as tanggal, tA.id, 1 as tipe, no_giro, tA.user_id, tA.created, concat(tC.nama, if(tA.keterangan != '',concat(' ,', keterangan),'' ) ) as keterangan, tC.nama as penerima,3 as status
                    FROM (
                        SELECT *
                        FROM nd_pembayaran_hutang_nilai 
                    ...
                    SELECT '',giro_list_id, amount, jatuh_tempo, tanggal, t1.id, 2 as tipe, no_giro, user_id, updated, keterangan, penerima,status
