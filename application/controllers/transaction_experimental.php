	...
	function pembelian_list_detail(){
        ...
        if ($id != '') {
            ...
            foreach ($data['pembelian_data'] as $row) {
                ...
                $data_other = $this->tr_model->get_pembelian_other($ockh);
            }
	...
	function penjualan_list_update(){
        ...
        if ($status_penjualan == 1) {
            ...
            if ($penjualan_type_id != 2) {
                ...
                $this->common_model->db_delete_array("nd_pembayaran_penjualan",$delete_cond);
            }
...
//==========================penjualan_detail=================================
    ...
    function penjualan_list_detail_insert_legacy(){
        // print_r($this->input->post());
        $ini = $this->input;
        $penjualan_id =  $ini->post('penjualan_id');
	...
	function penjualan_list_detail_insert(){
        ...
        $harga_raw = str_replace('.', '', $ini->post('harga_jual'));
        $penjualanId = $penjualan_id;
        $kodeBeli = ($kode_beli == '' || $kode_beli == 0 ? 'null' : $kode_beli );
        $namaJualTercetak = ($nama_jual_tercetak == '' || $nama_jual_tercetak == 0 ? 'null' : $nama_jual_tercetak);
        $barangId = $ini->post('barang_id');
        $warnaId = $ini->post('warna_id');
        $gudangId = $ini->post('gudang_id');
        $satuanId = null;
        $hargaJual = str_replace(',', '.', $harga_raw);
        $qtyInsert = $subqty;
        $rollInsert = $subjumlah_roll;


        $po_detail_id = '';
        $qty_jual = $subqty;
        if ($po_penjualan_id != '') {
            $get_podata = $this->common_model->db_select("nd_po_penjualan_detail WHERE po_penjualan_id=$po_penjualan_id AND barang_id=$barangId and warna_id=$warnaId");
            foreach ($get_podata as $row) {
                $po_detail_id = $row->id;
        ...
        foreach ($qty_list as $key => $value) {
            ...
            $jumlah_roll = (!isset($qty[1]) && $qty[1] == '' && $qty[1]  ? 0 : $qty[1]) ;
            if ($jumlah_roll != null) {
                array_push($detail, "(penjualanDetailId, ".$qty[0].", $jumlah_roll)");
            }

        }

		$collection = implode(",",$detail);
        // echo "CALL PENJUALAN_DETAIL_INSERT($penjualanId, $kodeBeli, $namaJualTercetak, $barangId, $warnaId, null, $hargaJual, $gudangId, $qtyInsert, $rollInsert,".'"'.$collection.'"'.", $po_detail_id,$qty_jual);";
		$this->common_model->db_custom_query("CALL PENJUALAN_DETAIL_INSERT($penjualanId, $kodeBeli, $namaJualTercetak, $barangId, $warnaId, null, $hargaJual, $gudangId, $qtyInsert, $rollInsert,'$collection',$po_detail_id,$qty_jual)");
	...
	{  
        ...
        if ($penjualan_type_id != 3) {
            ...
            foreach ($get_customer_data as $row) {
                $nama_cust_fp = $row->nama_customer;
            ...
            $nama_cust_fp = $nama_keterangan_history;
...
//===================================pengeluaran_stok_lain=============================================
    function pengeluaran_stok_lain_list(){
        ...
        $data = array(
            ...
            'breadcrumb_title' => 'Transaction',
            'breadcrumb_small' => 'Daftar pengeluaran_stok_lain',
            'nama_menu' => $menu[0],
            'nama_submenu' => $menu[1],
            'common_data'=> $this->data,
            'status_aktif' => $status_aktif,
...
**/
    ...
    function request_barang(){
        ...
        $barang_beli_id = $this->input->get('beli_id');
        // $tahun = date('Y') - 1;
        ...
        if (is_posisi_id()==1) {
            ...
            $data['request_barang_detail'] = $this->tr_model->get_active_request_barang_by_request_2($request_barang_id, $request_barang_batch_id);
        }else{
            $data['request_barang_detail'] = $this->tr_model->get_active_request_barang_by_request_2($request_barang_id, $request_barang_batch_id);
        }
    ...
    function new_request_insert2(){
        ...
        foreach ($get_po as $row) {
            if (isset($req_list[$row->barang_id][$row->warna_id])) {
                // echo $row->barang_id.' == '.$row->warna_id;
                // echo '<br/>';
                // echo $row->qty_outstanding;
                // echo '<br/>'.$row->po_number;
                // echo '<br/>'.$row->qty_data;
                // echo '<br/>';
                // foreach ($req_list[$row->barang_id][$row->warna_id] as $key => $value) {
                //     echo $key.'-'.$value->qty;
                // }
                // echo "<hr/>";
...
function get_po_request_number($tahun, $bulan){
    ...
    $last_number = $this->common_model->db_select("nd_po_penjualan WHERE YEAR(tanggal)='$tahun' AND MONTH(tanggal)='$bulan' ORDER BY request_index LIMIT 1");
    $new_index = 1;
...
function get_customer_po(){
    $customer_id = $this->input->get('customer_id');
    $res = $this->common_model->get_po_customer($customer_id);
    echo json_encode($res);
}
