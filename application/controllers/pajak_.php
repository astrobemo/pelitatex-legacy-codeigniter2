...
//============================rekam faktur pajak=================================================
	...
	function update_npwp_nik(){
		$rekam_faktur_pajak_id = $this->input->get('id');
		$get_data = $this->pjk_model->get_faktur_to_rekam_by_id($rekam_faktur_pajak_id);
		...
		$this->common_model->db_update_batch("nd_rekam_faktur_pajak_detail", $data, 'id' );
		redirect(is_setting_link('pajak/rekam_faktur_pajak_detail').'?id='.$rekam_faktur_pajak_id);
	...
	function rekam_faktur_pajak_detail(){
		...
		$data = array(
			'content' =>'admin/pajak/rekam_faktur_pajak_detail',
			'breadcrumb_title' => 'Pajak',
			'breadcrumb_small' => 'Daftar Faktur',
			'nama_menu' => $menu[0],
			'nama_submenu' => $menu[1],
		...
		$data['faktur_tambahan'] = $this->pjk_model->get_rekam_faktur_lain($id, $tanggal_start, $tanggal_end);
...
//====================================faktur pajak donwload=========================================
	...
	function rekam_faktur_list_export_excel(){
		...
		foreach ($get_detail_list as $row) {
			...
			$sum_jual[$row->penjualan_id] += number_format($row->harga_jual/(1 + $ppn_nilai) ,'4','.','') * $row->qty; 
			$idx++;
		}
		...
		foreach ($faktur_data_list as $row) {
			...
			$ppn_pengali[$row->penjualan_id] = $row->ppn/100;
		}
		...
		foreach ($fp_list as $row) {
			...
			$alamat = $penjualan_data_detail['data'][$row->penjualan_id]->ALAMAT_LENGKAP_LENGKAP;
			...
			$pembagi = 1+($ppn_pengali[$row->penjualan_id]);
			$nilai_raw = number_format($penjualan_data_detail['data'][$row->penjualan_id]->g_total/$pembagi,'2','.','');
			$no_ref = $no_faktur[$row->penjualan_id]; 
			$ppn = number_format($penjualan_data_detail['data'][$row->penjualan_id]->g_total/$pembagi,'2','.','')*0.1;
			$coll++;//K
			...
			foreach ($penjualan_data_detail['detail'][$row->penjualan_id] as $key => $value) {
				foreach ($value as $key2 => $value2) {
					$pembagi = 1+($ppn_pengali[$row->penjualan_id]);
					$harga_jual = number_format($value2->harga_jual/$pembagi,'4','.','');
					$row_no++;
					$coll="A";
...
//====================================faktur pajak email=========================================
	...
	function rekam_faktur_email_list(){
		...
		$data = array(
			'content' =>'admin/pajak/rekam_faktur_pajak_email',
			'breadcrumb_title' => 'Pajak',
		...
		$data['rekam_faktur_data'] = $this->pjk_model->get_rekam_faktur_pajak_list_by_id($id);
