...
class Report extends CI_Controller {
	...
	function __construct() 
	{
		...
		$this->tahun_start_program = "2020";
		$this->tanggal_start_program = "2020-01-01";
...
//=====================================penjualan report==================================================
	...
	function penjualan_list_report(){
		...
		if($tanggal_start < $this->tanggal_start_program){
			$tanggal_start = $this->tanggal_start_program;
		}
	...
	function penjualan_list_export_excel(){
		...
		foreach ($penjualan_list as $row) {
			...
			$nama_barang_data = explode('??', $row->nama_barang_data);
			$count = count($qty);
			// $g_total = 0;
			...
			if (is_posisi_id() == 1) {
				$objPHPExcel->getActiveSheet()->setCellValue("W".$row_no, $row->id);	
			...
			foreach ($harga_jual as $key => $value) {
				...
				if (is_posisi_id() == 1) {
					$x = explode("||", $nama_barang_data[$key]);
					$brg = $x[0];
					$wrn = $x[1];

					$objPHPExcel->getActiveSheet()->setCellValue("U".$row_no, $brg);
					$objPHPExcel->getActiveSheet()->setCellValue("V".$row_no, $wrn);		
	...
	// modul ini cancelled
	function penjualan_list_customer(){
		$menu = is_get_url($this->uri->segment(1)) ;
		$tanggal_start = date('Y-m-d');
		$tanggal_end = date('Y-m-d');
		$status_excel = '0';
		...
		// $data['penjualan_customer'] = $this->rpt_model->get_penjualan_customer($tanggal_start, $tanggal_end, $customer_cond, $cond_toko, $cond_barang_warna);
...
//=====================================pembelian report==================================================
	...
	function pembelian_list_export_excel(){
		...
		foreach ($pembelian_list as $row) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue($coll.$row_start, date('d-m-Y', strtotime($row->jatuh_tempo." +30 days")));
			$objPHPExcel->getActiveSheet()->getStyle($coll.$row_start)->getAlignment()->setWrapText(true);
			$objPHPExcel->getActiveSheet()->getStyle($coll.$row_start)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
			$objPHPExcel->getActiveSheet()->getStyle($coll.$row_start)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$objPHPExcel->getActiveSheet()->getColumnDimension($coll)->setWidth(30);
...
//=====================================barang keluar report==================================================	
	...
	function get_barang_rekap(){
		...
		$sum_group = "";
		foreach ($this->satuan_list_aktif as $row) {
			$sum .= " sum(if(c.satuan_id=".$row->id.", qty, 0)) as qty_".$row->id.", ";
			$sum .= " sum(if(c.satuan_id=".$row->id.", jumlah_roll, 0)) as jumlah_roll_".$row->id.", ";

			$sum_group .= " group_concat(qty_".$row->id." ORDER BY tipe asc) as qty_".$row->id.", ";
			$sum_group .= " group_concat(jumlah_roll_".$row->id." ORDER BY tipe asc) as jumlah_roll_".$row->id.", ";
		}
		...
		$data = $this->rpt_model->get_barang_keluar_report_rekap($tanggal_start, $tanggal_end, $cond, $cond2, $sum, $sum_group);
...
//=======================================laporan PO Pembelian=============================
	...
	function po_pembelian_report(){
		...
		if ($view_type == 2) {
			$cond_lock = "WHERE a.locked_date is null";
		}
		$barang_id = '';
		$barang_beli_id = '';
		$warna_id = '';
		...
		$data = array(
			...
			'barang_beli_id' => $barang_beli_id,
			'warna_id' => $warna_id,
			'po_pembelian_report' => $this->rpt_model->po_pembelian_report($cond, $cond_lock)
			);
...
//=====================================laporan pembayaran hutang==================================================
	...
	function pembayaran_hutang_report(){
		...
		$data = array(
			...
			if($tanggal_awal < $this->tanggal_start_program){
				$tanggal_awal = $this->tanggal_start_program;
			}
...
//=====================================laporan stok opname==================================================
	...
	function stok_opname_report(){
		...
		$data['so_list'] = $this->rpt_model->get_so_report($tanggal_start.' 00:00:00', $tanggal_end.' 23:59:00', $barang_cond, $warna_cond, $gudang_cond);
		$this->load->view('admin/template',$data);
	...
	function penyesuaian_stok_opname_update(){
		$penyesuaian_stok_id = $this->input->post('penyesuaian_stok_id');
...
//=======================================laporan PO Penjualan=============================
	...
	function po_penjualan_report(){
		...
		$content[1] = 'admin/report/po_penjualan_report';
		$content[2] = 'admin/report/po_penjualan_report_by_barang';
		$content[3] = 'admin/report/po_penjualan_report_by_po';
		...
		$data = array(
			...
			'warna_id' => 0,
			'view' => $view,
			// 'po_penjualan_report' => $this->rpt_model->po_penjualan_report($cond, $cond_customer)// legacy
			'po_penjualan_report' => array()
			);
	...
	function data_po_penjualan_by_customer(){
		$cond = '';
		$cond_customer = '';
		...
		if ($this->input->get('customer_id') && $this->input->get('customer_id') != '') {
			$customer_id = $this->input->get('customer_id');
			$cond_customer = " AND customer_id = '$customer_id'";
		}

		$po_penjualan_report = $this->rpt_model->po_penjualan_report_percustomer($cond, $cond_customer);

		echo json_encode($po_penjualan_report);

	}

	function data_po_penjualan_by_barang(){
		$cond = '';
		$cond_customer = '';
		...
		if ($this->input->get('tipe') && $this->input->get('tipe') != '') {
			$tipe = $this->input->get('tipe');
			$cond_tipe = " AND tipe = $tipe";
		}

		$po_penjualan_report = $this->rpt_model->po_penjualan_report_perbarang($cond, $cond_tipe);
	...
	function get_stok_barang_for_po_penjualan(){
	...
	function get_stok_for_po_jual(){
		...
		if ($item_list != '') {
			$cond_barang = "WHERE barang_id IN ($item_list)";
		}
		...
		$barang_list_selected = $this->inv_model->get_stok_for_pre_po('', $tanggal.' 23:59:59', $tanggal_awal, $stok_opname_id, $cond_barang, $tahun_tutup_buku, $bulan_qty);
		echo json_encode($barang_list_selected);

	}

//=====================================mutasi barang==================================================

	function mutasi_barang_report(){
		$menu = is_get_url($this->uri->segment(1)) ;
		...
		if($this->input->get('warna_id') != ""){
			$barang_id = $this->input->get('warna_id');
			$cond_warna = " AND warna_id=$warna_id";
		}

		if($this->input->get('gudang_id') != ""){
			$barang_id = $this->input->get('gudang_id');
			$cond_gudang = " AND gudang_id=$gudang_id";
			$cond_gudang_masuk = " AND gudang_id_after = $gudang_id";
			$cond_gudang_keluar = " AND gudang_id_before = $gudang_id";
		}
		
		$data = array(
			'content' =>'admin/report/mutasi_barang_report',
			'breadcrumb_title' => 'Laporan',
			'breadcrumb_small' => 'Laporan Mutasi Barang',
			'nama_menu' => $menu[0],
			'nama_submenu' => $menu[1],
			'common_data' => $this->data,
			'tanggal_start' => is_reverse_date($tanggal_start),
		...
		$data['mutasi_list'] = $this->rpt_model->get_mutasi_barang_list_report($tanggal_start, $tanggal_end, $cond_barang, $cond_warna, $cond_gudang, $cond_gudang_masuk, $cond_gudang_keluar);
...
//=====================================laporan penjualan dpp==================================================

	function report_penjualan_dpp_excel(){
		...
		if ($this->input->get('customer_id') != "" && $this->input->get('customer_id') != 0) {
			$customer_id = $this->input->get('customer_id');
			$customer_cond = " AND customer_id = $customer_id";
		}
		...
		$data['data_faktur'] = $this->rpt_model->get_penjualan_faktur_pajak_report($tanggal_start, $tanggal_end, $customer_cond);
		// "nd_rekam_faktur_pajak_detail WHERE tanggal >= '$tanggal_start' AND tanggal <= '$tanggal_end' $customer_cond"
		$data['tanggal_start'] = date("d_M_Y", strtotime($tanggal_start));
		$data['tanggal_end'] = date("d_M_Y", strtotime($tanggal_end));
		...
		$this->load->view('admin/report/report_penjualan_dpp_excel', $data);
