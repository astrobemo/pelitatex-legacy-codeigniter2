...
//============================giro register=================================================
	...
	function giro_register_detail_batal(){
		...
		if ($tipe == 1) {
			if($id != ''){
				$this->common_model->db_update("nd_giro_list_detail", $data,'id',$id);
				$this->common_model->db_delete("nd_pembayaran_pembayaran_hutang_nilai", 'id', $id);
			}
			$this->common_model->db_insert("nd_giro_list_detail", $data);			
		}elseif ($tipe==3) {
			$this->common_model->db_insert("nd_giro_list_detail", $data);
...
//============================hutang section=================================================
	...
	function hutang_list_detail(){
		...
		if ($tipe_supplier == 1) {
			$data['hutang_list_detail'] = $this->fi_model->get_hutang_list_detail($supplier_id, $tanggal); 
			$data['retur_list_detail']  = $this->fi_model->get_retur_list_detail($supplier_id, $tanggal);
		}else{
	...
	function outstanding_hutang_list_detail(){
		...
		if (is_posisi_id() == 1) {
			$this->output->enable_profiler(TRUE);
			// print_r($data['outstanding_list_detail']);
		}
	}
...
//============================piutang section=================================================
	...
	function pembayaran_piutang_remove_async()
	{
		
		// echo json_encode($this->input->server('REQUEST_METHOD'));
		if ($this->input->server('REQUEST_METHOD') == "DELETE") {
			# code...
			$id = $this->input->get('id');
			$this->common_model->db_delete('nd_pembayaran_piutang','id', $id);
			$this->common_model->db_delete('nd_pembayaran_piutang_detail','pembayaran_piutang_id', $id);
			$this->common_model->db_delete('nd_pembayaran_piutang_nilai','pembayaran_piutang_id', $id);
			echo json_encode("OK");
		}else{
		...
		// $id = $this->input->get('id');
		// $this->common_model->db_delete('nd_pembayaran_hutang','id', $id);
		// $this->common_model->db_delete('nd_pembayaran_hutang_detail','pembayaran_hutang_id', $id);
		// $this->common_model->db_delete('nd_pembayaran_hutang_nilai','pembayaran_hutang_id', $id);
...
//==============================mutasi hutang=================================================
	...
	function mutasi_hutang_excel_all(){
		...
		$start = new DateTime($tanggal_start);
		$start->modify($tanggal_start);
		...
		$bayar_all = $this->fi_model->get_mutasi_hutang_bayar_all($toko_id, $tanggal_start, $tanggal_end);
		
		foreach ($period as $dt) {
			$tanggal_start = $dt->format("Y-m-01");
			$tanggal_end = $dt->format("Y-m-t");
			$bl = $dt->format('Y-m');
			$get_mutasi = $this->fi_model->get_mutasi_hutang($tanggal_start, $tanggal_end, $toko_id);
			foreach ($get_mutasi as $row) {
				$mutasi_list[$bl][$row->supplier_id] = $row;
			}
		...
		// foreach ($mutasi_list as $row) {
		// 	$hutang_awal[$row->supplier_id] = $row->amount;
		// 	foreach ($period as $dt) {
		// 		$waktu = $dt->format("Y-m");
		// 		$bayar_list[$waktu][$row->supplier_id] = null;
		// 	}
		// }

		foreach ($bayar_all as $row) {
			// $bayar_list[$row->tahun.'-'.$row->bulan][$row->supplier_id] = array("pembayaran_type_id"=>$row->bayar, "bayar"=>$row->bayar,"nama_supplier"=>$row->nama_supplier);
			$bayar_list[$row->tahun.'-'.$row->bulan][$row->supplier_id][$row->pembayaran_type_id] = $row;
		// print_r($row);
		...
		foreach ($period as $dt) {
			...
			$objPHPExcel->getActiveSheet()->setTitle(($sheet+1).'. '.substr($namaBulan.' '.$tahun, 0,20));
			
			$objPHPExcel->getActiveSheet()->mergeCells("A1:F1");
			$objPHPExcel->getActiveSheet()->mergeCells("A2:F2");
	
			$objPHPExcel->getActiveSheet()->setCellValue('A1', ' Mutasi Hutang ');
			$objPHPExcel->getActiveSheet()->setCellValue('A2', 'Bulan : '.$namaBulan);
			...
			// print_r($bayar_list[$tahun.'-'.$bulan]);
			foreach ($mutasi_list[$tahun.'-'.$bulan] as $key => $row) {
				// echo $key;
				// print_r($row);
				$coll = "A";
				...
				if (isset($bayar_list[$tahun.'-'.$bulan][$row->supplier_id])) {
					# code...
					// print_r($bayar_list[$tahun.'-'.$bulan][$row->supplier_id]);
					// echo "<hr/>";
					foreach ($bayar_list[$tahun.'-'.$bulan][$row->supplier_id] as $row2) {
						$total_bayar += $row2->bayar;
						if ($row2->pembayaran_type_id == 1) {
							$bayar1 = $row2->bayar;
						}
		...
		$data_toko = $this->common_model->db_select("nd_toko where id=$toko_id");
		$nama_toko = "";
		...
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header("Content-Disposition: attachment;filename=mutasi_hutang_$nama_toko".$tanggal_start."__".$tanggal_end.".xls");
		header('Cache-Control: max-age=0');
		$objWriter->save('php://output');
	}
