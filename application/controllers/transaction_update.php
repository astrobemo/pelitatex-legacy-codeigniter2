...
//===================================pembelian=============================================
	...
	function pembelian_list_detail(){
        ...
        if ($id != '') {
            ...
            $warna_list = array();
            if ($po_pembelian_batch_id != '' && $po_pembelian_batch_id != 0 && $data['barang_id_before'] == '') {
                $cond_ockh = $OCKH;
                if ($is_multiple_item) {
                    $cond_ockh = '';
                }
                ...
                if ($data['barang_beli_id_before'] != '' &&  $po_pembelian_batch_id ) {
                    $warna_list = $this->tr_model->get_pembelian_warna_by_po($po_pembelian_batch_id, $data['barang_beli_id_before']);
                }
            }
...
//===================================penjualan=============================================
	...
	function penjualan_list_insert(){
        ...
        if ($po_penjualan_id == 'manual') {
            $po_penjualan_id = 0;
        }else if($po_penjualan_id == ''){
...
//==========================penjualan_detail=================================
	...
	function penjualan_list_detail(){
        ...
        // }else{
            ...
            $is_custom_view = 0;
            $po_penjualan_id = '';
            $data['po_penjualan_header'] = array();
    		...
    		if ($id != '') {
                ...
                if ($po_penjualan_id != '') {
                    $data['po_penjualan_header'] = $this->tr_model->get_po_penjualan_header($po_penjualan_id);
                    $data['po_penjualan_barang'] = $this->tr_model->get_po_penjualan_barang($po_penjualan_id);
                }
            ...
            // if (is_posisi_id() != 1) {
                if ($is_custom_view && ( is_posisi_id() <= 3 || is_posisi_id() == 6) ) {
                    ...
                    $data['content'] = 'admin/transaction/penjualan_list_detail_new6';
                    $this->load->view("admin/template",$data);
                }
            // }
	...
	function get_qty_stock_by_barang(){
        ...
        // }else{
            $stok_barang_by_satuan = $this->tr_model->get_kartu_stok_barang_by_satuan_2($gudang_id, $barang_id, $warna_id, date('Y-m-d'), date('Y-m-d'), $tanggal_awal, $stok_opname_id, $cond_detail); 
            $result = $this->tr_model->get_qty_stok_by_barang_2($gudang_id, $barang_id,$warna_id, $tanggal_awal, $stok_opname_id, $cond_detail);
        // }
...
//===================================Retur=============================================
	...
	function penjualan_list_retur(){
		...
		foreach ($data_jual as $row) {
			$data = array(
                ...
                'penjualan_id' => $id,
                'po_penjualan_id' => ($row->po_penjualan_id == "" ? null : $row->po_penjualan_id),
                'po_number' => ($row->po_number == "" ? null : $row->po_number),
				'tanggal' => date('Y-m-d'),
				'no_faktur' => $no_faktur,
				'customer_id' => $row->customer_id ,
				'nama_keterangan' => $this->input->post('nama_keterangan') ,
	...
	function retur_jual_detail(){
		...
		if ($id != '') {
            ...
            $get_de = $this->common_model->db_select("nd_penjualan_detail WHERE penjualan_id = '$penjualan_id'");
            $res_detail_id = [];
            foreach ($get_de as $rw) {
                array_push($res_detail_id, $rw->id);
            }
            ...
            $cond_d = (count($res_detail_id) > 0 ? implode(',',$res_detail_id) : "0");
            $data['penjualan_list_detail'] = $this->tr_model->get_data_penjualan_detail($penjualan_id, $cond_d);
...
**/
    ...
    function po_penjualan_list(){
        ...
        $data['po_penjualan_list'] = $this->tr_model->get_po_penjualan($tanggal_start, $tanggal_end, $customer_id, $cond);
