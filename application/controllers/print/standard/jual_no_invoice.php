...
$font_size = 10;
            ...
            for ($i = $for; $i <= $until; $i++) {
                  if (isset($barang[$i][4])) {
                        ...
                        $pdf->Cell($this->col_barang['kode_beli'], $frame_detail_height, $barang[$i][3], $frame_detail, 0, 'C');
                        $pdf->Cell($this->col_barang['barang'], $frame_detail_height, $barang[$i][4], $frame_detail, 0, 'L');
                        $pdf->Cell($this->col_barang['space'], $frame_detail_height, '', $frame_detail, 0, 'L');
                        $pdf->Cell($this->col_barang['harga'], $frame_detail_height, $this->valuta . format_angka($barang[$i][5]), $frame_detail, 0, 'R'); //harga
                        $pdf->Cell($this->col_barang['total'], $frame_detail_height, $this->valuta . format_angka($barang[$i][6]), $frame_detail, 0, 'R'); //total harga
