<?php
class Jual_Invoice_And_Detail extends CI_Controller
{
      ...
      function index()
      {
            ...
            //********************************************************* */
            $profile = [
                  'nama' => $this->session->userdata['hameyean_profile']['profile_nama'],
                  'alamat1' => $this->session->userdata['hameyean_profile']['profile_alamat'],
                  'alamat2' => $this->session->userdata['hameyean_profile']['profile_kota'] . ', ' . $this->session->userdata['hameyean_profile']['profile_kode_pos'] . ' | TELP. : ' . $this->session->userdata['hameyean_profile']['profile_telepon'],
                  'npwp' => 'NPWP : ' . $this->session->userdata['hameyean_profile']['profile_npwp'],
                  'kota' => $this->session->userdata['hameyean_profile']['profile_kota'],
            ];
            ...
            if (isset($header_me)) {
                  ...
                  $nama_customer = $header_me->nama;
                  $alamat_customer = $header_me->alamat . ' RT' . $header_me->rt . ' RW' . $header_me->rw . ', ' . $header_me->kota . ', ' . $header_me->provinsi;
            ...
            //********************************************************* */
            //setup pdf
            $pdf = new FPDF('L', 'mm', array(216, 141));
            // $pdf = new FPDF('P', 'mm', 'A4');

            $pdf->addfont('Calibri', '', 'calibri-light.php');
            $pdf->addfont('Calibri', 'B', 'calibri-light-bold.php');
            $pdf->addfont('Calibri', 'I', 'calibri-light-italic.php');
      ...
      function print_header($pdf, $judul, $profile, $header)
      {
            ...
            $pdf->Cell(23, 6, strtoupper($judul), 1, 0, 'C');

            $pdf->SetFont('Arial', '', $font_tanggal);
            $pdf->Cell(177, 6, strtoupper($profile['kota'] . ' ' . $header['tanggal']), 0, 1, 'R');
