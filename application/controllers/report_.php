...
//=====================================penjualan report==================================================
	...
	function penjualan_list_report(){
		...
		if($this->input->get('gudang_id') && $this->input->get('gudang_id') != 0){
			...
			$gudang_cond = "WHERE gudang_id = ".$gudang_id;
		}

		if ($this->input->get('barang_id') && $this->input->get('barang_id') != '') {
			$barang_id = $this->input->get('barang_id');
			$barang_id_cond = ($gudang_id == 0 || $gudang_cond == '' ?' WHERE' : ' AND')." barang_id = ".$barang_id;
		}

		if ($this->input->get('warna_id') && $this->input->get('warna_id') != '') {
			$warna_id = $this->input->get('warna_id');
			$warna_id_cond = ($gudang_cond == 0 && $barang_id == '' ? 'where' : 'and')." warna_id = ".$warna_id;
		}
	...
	function penjualan_list_export_excel_split_month(){
		...
		$data['penjualan_list'] = $this->rpt_model->get_penjualan_report($tanggal_start, $tanggal_end, $cond, $customer_cond, $gudang_cond,$barang_id_cond, $warna_id_cond, $tipe_penjualan_cond);
...
//=====================================pembelian report==================================================
	...
	function pembelian_list_report(){
		...
		if ($supplier_id == 0 || $tipe_supplier == 1) {
			$data['pembelian_list'] = $this->rpt_model->get_pembelian_report($cond_tanggal, $cond, $cond_barang, $cond_warna, $cond_gudang, $order_by);
		}
		...
		$data['pembelian_retur']= $this->rpt_model->get_pembelian_retur($cond_tanggal, $cond, $cond_barang, $cond_warna, $cond_gudang);
