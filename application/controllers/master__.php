...
//================================barang list================================================
	...
	private function barang_validate($tipe = 'new', $id = 0)
	{
		...
		if ($nama == '') {
			...
			//cek apakah double 
			if ($tipe == 'new') {
				$hasil_nama = $this->common_model->db_select_num_rows("nd_barang where nama = '" . $nama . "' AND status_aktif = 1");
			} else {
				$hasil_nama = $this->common_model->db_select_num_rows("nd_barang where nama = '" . $nama . "' AND status_aktif = 1 AND id <> '" . $id . "'");
			}
	...
	function barang_planner()
	{
		...
		$data = array(
			'content' => 'admin/master/barang_planner',
			'breadcrumb_title' => 'Master',
