		...
		$('#customer_id_select, #form_add_data [name=penjualan_type_id]').change(function(){
			...
			if (penjualan_type_id == 1 || penjualan_type_id == 2) {
				...
				if (customer_id == '') {
					...
					getPOList(customer_id,"#po_penjualan_select");
...
function getPOList(customer_id, selectId){
	const inputPO = document.querySelector("#inputPOAdd");
	...
	opt2.text = "Manual";
	selectEl.add(opt2)
    fetch(baseurl+`transaction/get_customer_po?customer_id=${customer_id}`)
	.then((response) => response.json())
	.then((data) => {
		console.log(data);
		data.forEach(list => {
...
function setBarangPO(tipe){
	...
	const data_brg = barang_po.filter(item=>{
		if (item.id == sel.value) {
			// console.log('slct',item);
			return item;
		}
	});
	...
	if (sel.value !== '') {
		...
		const sisa_qty = barang.qty - barang.qty_invoice;
	
		barangInput.value=barang_id;
		warnaInput.value=warna_id;
		satuanInput.value=satuan;
		if (tipe==1) {
			const info = document.querySelector("#stok-info");
			info.querySelector(".po-qty").innerHTML = sisa_qty;
		}else{
			const info = document.querySelector("#stok-info-edit");
			info.querySelector(".po-qty").innerHTML = sisa_qty;
		}
...
function submitFormPO(){
	const tanggal = document.querySelector("#tanggal_po_add").value;
	const po_number = document.querySelector("#po_number_add").value;
	const formPOAdd =  document.querySelector("#form_po_add_data");
	
	if (tanggal != '' && po_number != '') {
		formPOAdd.submit();
	}else{
		bootbox.alert("PO Number dan Tanggal tinggal perlu diisi !");
	}
}
