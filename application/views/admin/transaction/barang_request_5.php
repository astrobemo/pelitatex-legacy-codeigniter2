	...
	<div class='container'>
		...
		foreach ($barang_terkirim as $row) {
			...
			foreach($bReq as $key => $value){
				...
				$total_terkirim_brg_warna[$row->barang_id][$row->warna_id] = 0;
			...
			for ($i=0; $i < count($tanggal_masuk) ; $i++) {
				...
				$total_terkirim_brg_warna[$row->barang_id][$row->warna_id] += $qData[$i];
				...
				if ($b_now != '' && strtotime($tanggal_masuk[$i]) < strtotime($bReq[0]) ) {
					...
					if (is_posisi_id() == 1 && $row->warna_id == 1) {
						// echo $terkirim[$row->barang_id][$row->warna_id][$b_now].' '.$i.' '.$qData[$i];
					}
					if ($pers <= 10 && $i == (count($tanggal_masuk)-1)  && $terkirim[$row->barang_id][$row->warna_id][$b_now] == 0  ) {
		...
		foreach ($barang_terkirim_pertoday as $row) {
			...
			foreach($bReq as $key => $value){
				//inisialisasi dl spya bisa di "+="
				$terkirim_now[$row->barang_id][$row->warna_id][$value] = 0;
			...
			for ($i=0; $i < count($tanggal_masuk) ; $i++) {
				...
				if (!isset($terkirim_now[$row->barang_id][$row->warna_id][$b_now])) {
					$terkirim_now[$row->barang_id][$row->warna_id][$b_now] = 0;
					$req[$b_now] = 0;
				}
				...
				if ($b_now != '' && strtotime($tanggal_masuk[$i]) < strtotime($bReq[0]) ) {
					$terkirim_now[$row->barang_id][$row->warna_id][$bReq[0]] += $qData[$i];

					if ($req[$bReq[0]] < ($terkirim_now[$row->barang_id][$row->warna_id][$bReq[0]] + $qData[$i])) {
						$pers = round($terkirim_now[$row->barang_id][$row->warna_id][$bReq[0]]/$req[$bReq[0]] * 100);
						
						$sisa_next = $terkirim_now[$row->barang_id][$row->warna_id][$bReq[0]] - $req[$bReq[0]];
						$terkirim_now[$row->barang_id][$row->warna_id][$bReq[0]] = $req[$bReq[0]];

						$b_next = date('Y-m-01', strtotime("+1 month", strtotime($bReq[0])));
						if (!isset($terkirim_now[$row->barang_id][$row->warna_id][$b_next])) {
							$terkirim_now[$row->barang_id][$row->warna_id][$b_next] = 0 ;
						}
						
						$terkirim_now[$row->barang_id][$row->warna_id][$b_next] += $sisa_next;
						// if (is_posisi_id() == 1 && $row->barang_id == 10) {
						// 	echo '1 ',$sisa_next.'<br/>';
						// }
					}
				}else if ($b_now != '' && $req[$b_now] < ($terkirim_now[$row->barang_id][$row->warna_id][$b_now] + $qData[$i]) ) {
					$b_next = date('Y-m-01', strtotime("+1 month", strtotime($b_now)));
					$sisa_next = $terkirim_now[$row->barang_id][$row->warna_id][$b_now] + $qData[$i] -  $req[$b_now] ;
					$pers = 0;
					if ($req[$b_now] > 0) {
						$pers = round($sisa_next/$req[$b_now] * 100, 2);
					}
					...
					// cek klo barang masuk sisa nya < 10% dari request
					if ($pers <= 10 && $i == (count($tanggal_masuk)-1) && $terkirim_now[$row->barang_id][$row->warna_id][$b_now] == 0  ) {
						if (count($tanggal_masuk) == 1) {
							$terkirim_now[$row->barang_id][$row->warna_id][$b_now] = $qData[$i];
						}else if (!isset($terkirim_now[$row->barang_id][$row->warna_id][$b_next])) {
							$terkirim_now[$row->barang_id][$row->warna_id][$b_now] += $qData[$i];
						}else{
							$terkirim_now[$row->barang_id][$row->warna_id][$b_now] += $qData[$i];
						}
						...
						if ($isNextAvai) {
							...
							$terkirim_now[$row->barang_id][$row->warna_id][$b_next] += $terkirim_now[$row->barang_id][$row->warna_id][$b_now] + $qData[$i] -  $req[$b_now] ;
							$terkirim_now[$row->barang_id][$row->warna_id][$b_now] = $req[$b_now];
						}else{
							$terkirim_now[$row->barang_id][$row->warna_id][$b_now] += $qData[$i];
						}
					}
					...
					if (is_posisi_id() == 1 && $row->barang_id == 10) {
						// echo '3 ',$req[$b_now] .'<'. $terkirim_now[$row->barang_id][$row->warna_id][$b_now] .'+'. $qData[$i].'<br/>';
					}
					$terkirim_now[$row->barang_id][$row->warna_id][$b_now] += $qData[$i];
		...
		foreach ($request_barang_qty_data as $row) {
			// if (is_posisi_id() == 1) {
			// 	echo $barang_id.' '.$row->warna_id;
				if (!isset($total_terkirim_brg_warna[$barang_id])) {
					$total_terkirim_brg_warna[$barang_id] = array();
				}

				if (!isset($total_terkirim_brg_warna[$barang_id][$row->warna_id])) {
					$total_terkirim_brg_warna[$barang_id][$row->warna_id] = 0;
				}
			// }
			if ($row->barang_id == $barang_id) {
				$t_request_all[$row->warna_id] -= $total_terkirim_brg_warna[$barang_id][$row->warna_id];
			}
		}
																...
																<?if ($idx == 0) {?>
																	...
																	<td rowspan="<?=count($request_barang_detail_id_b)+ count($warna_id_count);?>"  
																		...
																		<?}else{
																			...
																			<a style="max-width:120px; display:inline-block;word-wrap:break-word;" class='btn btn-md default' href="<?=base_url().is_setting_link('transaction/request_barang');?>?id=<?=$value;?>&request_barang_batch_id=<?=$request_barang_batch_id;?>">
																				<?=$nb;?>
																			</a>
																		<?}?>
																	</td>
