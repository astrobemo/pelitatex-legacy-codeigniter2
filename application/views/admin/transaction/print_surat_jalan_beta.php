...
function print_header_surat_jalan(printer_name, idx){
	<?
	if ($penjualan_type_id != 3) {?>
		data_header[0] = ['\x1B' + '\x40'+          // init
	...
	foreach ($customer_alamat_kirim as $row) {?>
		data_header[<?=$row->id?>] = ['\x1B' + '\x40'+          // init
			   	'\x1B' + '\x21' + '\x39'+ // em mode on
			   	<?="'".sprintf('%-20.18s','SURAT JALAN')."'";?>+
			   	'\x1B' + '\x21' + '\x15'+
			   	'\x09'+
			   	...
			   	'\x09'+
			   	'\x1B' + '\x21' + '\x18'+ // em mode on
			   	<?="'".sprintf('%47.47s', ${'alamat_kirim'.$row->id}[1])."'";?> + '\x0A'+
			   	...
			   	'\x09'+
			   	'\x1B' + '\x21' + '\x18'+ // em mode on
			   	<?="'".sprintf('%42.42s',${'alamat_kirim'.$row->id}[2] )."'";?> + '\x0A'+
	...
	<?}?>
	console.log(data_header[idx]);
	webprint.printRaw(data_header[idx], printer_name);
}

function print_surat_jalan_beta(printer_name){
			...
			var data = ['\x1B' + '\x40'+          // init
			   	...
			   	foreach ($penjualan_print as $row) {
			   		...
			   		$total_roll += $row->jumlah_roll;?>
					   	...
					   	<?="'".sprintf('%4.4s', $row->jumlah_roll)."'";?>+'\x09'+
