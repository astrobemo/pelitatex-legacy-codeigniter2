					...
					<div class="portlet-body">
						...
						<div>
							...
							if ($status_aktif != -1) {
								...
								<?if (is_posisi_id() == 1) {?>
									<a <?if($disabled_status == ''){ ?> href="<?=base_url();?>transaction/penjualan_print_kombinasi?penjualan_id=<?=$penjualan_id;?>"<? }else{ echo $disabled_status; } ?> target='_blank' class='btn btn-lg green hidden-print'><i class='fa fa-eye'></i> Faktur + Detail </a>
								<?}?>
								<?if (is_posisi_id() != 6) {?>
									...
									<?$display = '';?>
                                                      <button   type="button" tabindex='-1' class="btn btn-lg blue btn-print" onclick="printInvoice()">FAKTUR</button>
                                                      <button   type="button" tabindex='-1' class="btn btn-lg blue btn-print" onclick="printInvoiceDetail()">FAKTUR + DETAIL</button>
                                                      <button   type="button" tabindex='-1' class="btn btn-lg green btn-print" onclick="printSJ()">SURAT JALAN</button>
                                                      <button   type="button" tabindex='-1' class="btn btn-lg green btn-print" onclick="printSJNon()">SJ NO HARGA</button>
                                                <?}else if(is_posisi_id() == 6 || is_posisi_id() == 1){?>
									<a <?if($disabled_status == ''){ ?> href="<?=base_url();?>transaction/penjualan_print_kombinasi?penjualan_id=<?=$penjualan_id;?>"<? }else{ echo $disabled_status; } ?> target='_blank' class='btn btn-lg green hidden-print'><i class='fa fa-eye'></i> Faktur + Detail </a>
								<?}?>
				            	<?if (is_posisi_id() == 1) {?>
...
jQuery(document).ready(function() {
	...
	<?}?>
		...
		<?if($penjualan_id != '' && $status==0 && is_posisi_id() !=6 ){?>
        ...
        $.each(JSON.parse(data_respond), function(k,v){
            if (k==0) {
            	...
            	for (var i = 0; i < v.length; i++) {
            		...
            		// console.log(v[i].qty+':', sisa_roll);
            		if (sisa_roll > 0) {
            			qtyListDetail[`s-${parseFloat(Math.round(v[i].qty*100)/100)}`] = sisa_roll;
            			for (var j = 0; j < sisa_roll; j++) {
            				if (v[i].qty != '100' && v[i].qty != 100) {
		            			qtyTblDetail.push(parseFloat(Math.round(v[i].qty*100)/100 ));
            				};
            			};
            		}else if(v[i].qty == 100){
...
function printInvoiceDetail() {
      let penjualan_id = "<?= $penjualan_id ?>";
      let url = '<?= base_url('print/jual_invoice_and_detail/index/'); ?>';
      url += `/${penjualan_id}`;
