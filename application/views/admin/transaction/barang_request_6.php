...
<div class="page-content">
	<div class='container'>
		...
		foreach ($request_barang_detail as $row) {
			$barang_beli_id_get = explode("??", $row->barang_beli_id);
			$barang_id_get = explode("??", $row->barang_id);
			$warna_id_get = explode("??", $row->warna_id);
			$qty = explode("??", $row->qty);
			if (is_posisi_id()==1) {
				echo '<hr/>--wig';
				print_r($warna_id_get);
				echo '<hr/>';

			}
			// $print_w = array_unique(($warna_id_get))



			foreach ($barang_beli_id_get as $key => $value) {
				if ($value == $barang_beli_id) {
					$warna_id_b = explode(",", $warna_id_get[$key]);
					$qty_b = explode(",", $qty[$key]);
					foreach ($warna_id_b as $k2 => $v2) {
						if (is_posisi_id()==1) {
							// echo $k2.' == ';
							// echo $v2.'<br/>';
						}
						if (!isset($t_request_all[$v2])) {
							$t_request_all[$v2] = 0;
						}
						$t_request_all[$v2] += $qty_b[$k2];
					}
		...
		$total_terkirim_brg_warna = array();
		$total_terkirim_now = array();
		...
		foreach ($barang_terkirim as $row) {
			...
			foreach($bReq as $key => $value){
				...
				$terkirim[$row->barang_beli_id][$row->warna_id][$value] = 0;
				$total_terkirim_brg_warna[$row->barang_beli_id][$row->warna_id] = 0;
			...
			for ($i=0; $i < count($tanggal_masuk) ; $i++) {
				...
				$total_terkirim_brg_warna[$row->barang_beli_id][$row->warna_id] += $qData[$i];
				...
				if (!isset($terkirim[$row->barang_beli_id][$row->warna_id][$b_now])) {
					$terkirim[$row->barang_beli_id][$row->warna_id][$b_now] = 0;
					$req[$b_now] = 0;
				}
				...
				if ($b_now != '' && strtotime($tanggal_masuk[$i]) < strtotime($bReq[0]) ) {
					$terkirim[$row->barang_beli_id][$row->warna_id][$bReq[0]] += $qData[$i];

					// if (is_posisi_id() == 1 && $row->barang_beli_id == 10) {
					// 	echo '0 ',$qData[$i].'<br/>';
					// }
					if ($req[$bReq[0]] < ($terkirim[$row->barang_beli_id][$row->warna_id][$bReq[0]] + $qData[$i])) {
						$pers = round($terkirim[$row->barang_beli_id][$row->warna_id][$bReq[0]]/$req[$bReq[0]] * 100);
						
						$sisa_next = $terkirim[$row->barang_beli_id][$row->warna_id][$bReq[0]] - $req[$bReq[0]];
						$terkirim[$row->barang_beli_id][$row->warna_id][$bReq[0]] = $req[$bReq[0]];

						$b_next = date('Y-m-01', strtotime("+1 month", strtotime($bReq[0])));
						if (is_posisi_id() == 1 && $row->barang_beli_id == 26 && $row->warna_id == 8 ) {
							// echo '<h1>'.$b_next.'</h1><br/>';
						}

						if (!isset($terkirim[$row->barang_beli_id][$row->warna_id][$b_next])) {
							$terkirim[$row->barang_beli_id][$row->warna_id][$b_next] = 0 ;
						}
						
						$terkirim[$row->barang_beli_id][$row->warna_id][$b_next] += $sisa_next;
						// if (is_posisi_id() == 1 && $row->barang_beli_id == 10) {
						// 	echo '1 ',$sisa_next.'<br/>';
						// }
					}
				}else if ($b_now != '' && $req[$b_now] < ($terkirim[$row->barang_beli_id][$row->warna_id][$b_now] + $qData[$i]) ) {
					// if (is_posisi_id() == 1 && $row->barang_beli_id == 26 ) {
					// 	// echo '<h1>TEST</h1>';
					// 	echo $row->warna_id.' '.$req[$b_now] .'<'. ($terkirim[$row->barang_beli_id][$row->warna_id][$b_now] .'+'. $qData[$i]).'<br/>';
					// }
					if (is_posisi_id() == 1 && $row->barang_beli_id == 10) {
						// echo '2 ',$req[$b_now] .'<'. $terkirim[$row->barang_beli_id][$row->warna_id][$b_now] .'+'. $qData[$i].'<br/>';
					}
					$b_next = date('Y-m-01', strtotime("+1 month", strtotime($b_now)));
					$sisa_next = $terkirim[$row->barang_beli_id][$row->warna_id][$b_now] + $qData[$i] -  $req[$b_now] ;
					$pers = 0;
					if ($req[$b_now] > 0) {
					...
					if (is_posisi_id() == 1 && $row->warna_id == 1) {
						// echo $terkirim[$row->barang_beli_id][$row->warna_id][$b_now].' '.$i.' '.$qData[$i];
					}
					if ($pers <= 10 && $i == (count($tanggal_masuk)-1)  && $terkirim[$row->barang_beli_id][$row->warna_id][$b_now] == 0  ) {
						if (count($tanggal_masuk) == 1) {
							$terkirim[$row->barang_beli_id][$row->warna_id][$b_now] = $qData[$i];
						}else if (!isset($terkirim[$row->barang_beli_id][$row->warna_id][$b_next])) {
							$terkirim[$row->barang_beli_id][$row->warna_id][$b_now] += $qData[$i];
						}else{
							$terkirim[$row->barang_beli_id][$row->warna_id][$b_now] += $qData[$i];
						}
						...
						if ($isNextAvai) {
							...
							$terkirim[$row->barang_beli_id][$row->warna_id][$b_next] += $terkirim[$row->barang_beli_id][$row->warna_id][$b_now] + $qData[$i] -  $req[$b_now] ;
							$terkirim[$row->barang_beli_id][$row->warna_id][$b_now] = $req[$b_now];
						}else{
							$terkirim[$row->barang_beli_id][$row->warna_id][$b_now] += $qData[$i];
						}
					}
					...
					$isTercapai[$row->barang_beli_id][$row->warna_id][$b_now] = 1;
				}else if($b_now != ''){
					if (is_posisi_id() == 1 && $row->barang_beli_id == 10) {
						// echo '3 ',$req[$b_now] .'<'. $terkirim[$row->barang_beli_id][$row->warna_id][$b_now] .'+'. $qData[$i].'<br/>';
					}
					$terkirim[$row->barang_beli_id][$row->warna_id][$b_now] += $qData[$i];
		...
		foreach ($barang_terkirim_pertoday as $row) {
			// breakdown data
			$bReq = explode(",", $row->bulan_request);
			$qReq = explode(",", $row->qty_request);
			$req = array_combine($bReq, $qReq);
			...
			foreach($bReq as $key => $value){
				//inisialisasi dl spya bisa di "+="
				$terkirim_now[$row->barang_beli_id][$row->warna_id][$value] = 0;
				$total_terkirim_now[$value] = 0;
			}
			...
			for ($i=0; $i < count($tanggal_masuk) ; $i++) {
				...
				if (!isset($terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now])) {
					$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] = 0;
					$req[$b_now] = 0;
				}
				
				// kalau tanggal masuk < dari bulan request, 
				// tambah ke data terkirim_now // ini harus dicek ulang
				if ($b_now != '' && strtotime($tanggal_masuk[$i]) < strtotime($bReq[0]) ) {
					$terkirim_now[$row->barang_beli_id][$row->warna_id][$bReq[0]] += $qData[$i];

					if ($req[$bReq[0]] < ($terkirim_now[$row->barang_beli_id][$row->warna_id][$bReq[0]] + $qData[$i])) {
						$pers = round($terkirim_now[$row->barang_beli_id][$row->warna_id][$bReq[0]]/$req[$bReq[0]] * 100);
						
						$sisa_next = $terkirim_now[$row->barang_beli_id][$row->warna_id][$bReq[0]] - $req[$bReq[0]];
						$terkirim_now[$row->barang_beli_id][$row->warna_id][$bReq[0]] = $req[$bReq[0]];

						$b_next = date('Y-m-01', strtotime("+1 month", strtotime($bReq[0])));
						if (!isset($terkirim_now[$row->barang_beli_id][$row->warna_id][$b_next])) {
							$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_next] = 0 ;
						}
						
						$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_next] += $sisa_next;
						// if (is_posisi_id() == 1 && $row->barang_beli_id == 10) {
						// 	echo '1 ',$sisa_next.'<br/>';
						// }
					}
				}else if ($b_now != '' && $req[$b_now] < ($terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] + $qData[$i]) ) {
					$b_next = date('Y-m-01', strtotime("+1 month", strtotime($b_now)));
					$sisa_next = $terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] + $qData[$i] -  $req[$b_now] ;
					$pers = 0;
					if ($req[$b_now] > 0) {
						$pers = round($sisa_next/$req[$b_now] * 100, 2);
					}
					...
					// cek klo barang masuk sisa nya < 10% dari request
					if ($pers <= 10 && $i == (count($tanggal_masuk)-1) && $terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] == 0  ) {
						if (count($tanggal_masuk) == 1) {
							$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] = $qData[$i];
						}else if (!isset($terkirim_now[$row->barang_beli_id][$row->warna_id][$b_next])) {
							$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] += $qData[$i];
						}else{
							$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] += $qData[$i];
						}
						...
						if ($isNextAvai) {
							...
							$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_next] += $terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] + $qData[$i] -  $req[$b_now] ;
							$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] = $req[$b_now];
						}else{
							$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] += $qData[$i];
						}
					}
					...
					if (is_posisi_id() == 1 && $row->barang_beli_id == 10) {
						// echo '3 ',$req[$b_now] .'<'. $terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] .'+'. $qData[$i].'<br/>';
					}
					$terkirim_now[$row->barang_beli_id][$row->warna_id][$b_now] += $qData[$i];
		...
		foreach ($request_barang_qty_data as $row) {
			if ($row->barang_beli_id == $barang_beli_id) {
			...
			$status_request[$row->bulan_request][$row->barang_beli_id][$row->warna_id] = $row->tipe;
		}

		foreach ($request_barang_qty_data as $row) {
			// if (is_posisi_id() == 1) {
			// 	echo $barang_beli_id.' '.$row->warna_id;
				if (!isset($total_terkirim_brg_warna[$barang_beli_id])) {
					$total_terkirim_brg_warna[$barang_beli_id] = array();
				}

				if (!isset($total_terkirim_brg_warna[$barang_beli_id][$row->warna_id])) {
					$total_terkirim_brg_warna[$barang_beli_id][$row->warna_id] = 0;
				}
			// }
			if ($row->barang_beli_id == $barang_beli_id) {
				if (is_posisi_id()==1) {
				...
				$t_request_all[$row->warna_id] -= $total_terkirim_brg_warna[$barang_beli_id][$row->warna_id];
			}
		}

		foreach ($request_barang_qty_keterangan as $row) {
			$keterangan_qty[$row->barang_beli_id][$row->warna_id][$row->bulan_request] = $row->keterangan;
		}

		foreach ($request_barang_status as $row) {
			$isFinished[$row->barang_beli_id][$row->warna_id][$row->bln_request] = array('id'=>$row->id,'status'=>($row->username != '' ? true : false) );
		}
		...
		<div class="modal fade bs-modal-full" id="portlet-config-new" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-full">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('transaction/new_request_insert_bybeli')?>" class="form-horizontal" id="form_add_data" method="post">
                			...
                			<input name='barang_id' value="<?=$barang_beli_id;?>" hidden>
			                ...
			                <div class="form-group keterangan-request" hidden>
		...
		<div class="modal fade bs-modal-lg" id="portlet-config-new2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('transaction/new_request_insert2_bybeli')?>" class="form-horizontal" id="form_add_data2" method="post">
							...
							<div class="form-group keterangan-request">
			                    ...
			                    <div class="col-md-6">
									<!-- <input type="text" name="bulan_awal_request" value = "<?=$tanggal_start_forecast?>"> -->
									<?
										// generate today + 3 months
										$this_month = date("Y-m-d");
										...
										if (is_posisi_id() == 1) {
											...
											// echo $periode_months[0]->format('Y-m-01');
										}
									?>
			                    	<select name='bulan_awal_request' class='form-control'>
			                    		<?foreach ($periode_months as $date) {?>
											<option value="<?=$date->format('Y-m-01')?>"><?=$date->format('F').' '.($date->format('Y')+1)?></option>
										<?}?>
			                    	</select>
	                    		</div>
			                </div> 
		...
		<div class="row margin-top-10">			
					...
					<div class="portlet-body">
						...
						<table style='font-size:1.2em'>
							<form method='get'>
								...
								<tr>
									<td>
										Nama Barang <?=(is_posisi_id()==1 ? $barang_beli_id : '');?>
									</td>
									<td class='padding-rl-25'>
										:
									</td>
									<td>
										<select name='barang_beli_id' id="barang-beli-list-select" style='width:325px'>
											<option value="">Pilih</option>
											<?foreach ($barang_beli_list as $row2) {
												if ($row2->id == $barang_beli_id) {
													$nama_barang_terpilih = $row2->nama;
												}?>
												<option data-barang="<?=$row2->barang_id;?>" value="<?=($row2->id)?>" <?=($barang_beli_id == $row2->id ? 'selected' : '');?> ><?=$row2->nama?></option>
											<?}?>
										</select>

										<select name='id' id="barang-list-select" hidden>
											<option value="">Pilih</option>
											<?foreach ($this->barang_list_aktif as $row2) {?>
												<option value="<?=($row2->id)?>" <?=($barang_id == $row2->id ? 'selected' : '');?> ><?=$row2->nama_jual?></option>
											<?}?>
										</select>
								...
								<tr>
									...
									<td>
										<select name='supplier_id_filter' id="supplier-list-select" style='width:325px'>
											<option value="">Pilih</option>
											<?foreach ($this->supplier_list_aktif as $row2) {?>
												<option value="<?=($row2->id)?>" <?=($supplier_id_filter == $row2->id ? 'selected' : '');?> ><?=$row2->nama?></option>
											<?}?>
										</select>
										<span hidden style='font-size:1.3em'><?=$nama_beli;?></span>
									</td>
			...
			<!-- ===============================health planner========================= -->
			<div class="col-md-12">
				<div class="portlet light hidden-print" <?=($barang_beli_id == 0 ? 'hidden' : '');?> >
					<div class="portlet-title">
						<div class="caption caption-md">
							<i class="icon-bar-chart theme-font hide"></i>
						...
						<table class='table-qty table-qty-border' id='general-table' width='100%'>
								...
								foreach ($data_warna as $row) {
									...
									<tr>
										...
										<?
											...
											$total_request_all = 0;
											if (isset($t_request_all[$row->warna_id])) {
												$total_request_all = $t_request_all[$row->warna_id];
											}
											$sisa_stok_now = $qty_stok[$row->warna_id];
											$sisa_outstanding_now = $qty_outstanding[$row->warna_id];
											$sisa_request_now = $total_request_all;

											foreach ($periode as $date) {

												$perf_bar_width++;
												if ($first_date == '') {
													$first_date = ($date->format('Y')+1).'-'.$date->format('m');
												}
												$i = $date->format("Y-m");
												$key = $i;
												$tahun_now = ($date->format('Y')+1).'-'.$date->format('m');
												...
												$total_forecast += $forecast_now;
												$t_forecast[$row->warna_id] += $forecast_now;
										...
										foreach ($periode as $date) {
											...
											if (isset($qty_request_warna[$row->warna_id])) {
												...
												$qty_request_warna_before[$row->warna_id][(int)date('m', strtotime($i))-1] = 0;
												if ($request_bar_width > 1) {
													$qty_request_warna_before[$row->warna_id][(int)date('m', strtotime($i))-1] -= 1;
												} 
											...
											<!-- <div class='health-bar'>
													...
													<?}?>
													<?if ($request_bar_width > 0) {?>
														<div class='request-bar' style='text-align:center;background:<?=$month_color[(int)date('m', strtotime($i))-1];?>;width:<?=$request_bar_width*100;?>%'><?=$request_bar_width*100;?>%</div>
													<?}?>
													<?if ($stok_bar_width > 0) {?>
											...
											<td class='data-container' id='cell-<?=$key?>-<?=$row->warna_id?>' data-old="">
												<div style="width:100%: height:100%;" onclick="showHistory('<?=$row->warna_id?>','<?=$key?>', '<?=(isset($qty_data[$key][$row->warna_id]) ? (float)$forecast_now : 0);?>')" 
													...
													<?if ($tahun_now == $first_date) {
														...
														<div class='perf-bar-container' style="width:<?=$perf_bar_width*90?>px">
															<div class='stok-bar' style='text-align:left;width:<?=$total_stok_width*90;?>px'><?=$total_stok_width;?></div>
															<div class='outstanding-bar' style='left:<?=$total_stok_width*90;?>px;width:<?=$total_outstanding_width*90;?>px'><?=$total_outstanding_width;?></div>
															<div class='request-bar' style='left:<?=$total_stok_width*90;?>px;width:<?=$total_request_width*90;?>px'><?=$total_request_width;?></div>
															<div class='empty-bar' style="left:<?=($total_stok_width*90) + ($left_empty *90) ;?>px;width:<?=$total_empty_width*90;?>px"><?=$total_empty_width;?></div>
														</div>
													<?}?>
							...
							<tfooter>
								<tr>
									...
									<?}?>
									<th style="font-size:1.1em; text-align:center"><?=number_format($grand_total,'0',',','.')?></th>
								</tr>
							</tfooter>
						</table>
					</div>
				...
				<div class="portlet light">
					...
					<div class="portlet-body">
													...
													<td>
														<select name="id" class='form-control' style="width:250px; display:inline-block" id="select-barang" >
															...
															<?foreach ($request_barang_detail as $row) {
																...
																$barang_beli_id_get = explode("??", $row->barang_beli_id);
																$nama_beli = explode("??", $row->nama_beli);
																for ($i=0; $i < count($barang_beli_id_get)  ; $i++) {?> 
																	<option  <?=($barang_beli_id == $barang_beli_id_get[$i] ? 'selected' : '')?> value="<?=$barang_beli_id_get[$i]?>"><?=$nama_beli[$i];?></option>
																<?}
															}?>
														</select>
													</td>
							...
							<div style='margin-bottom:15px'>
								Keterangan : <a onclick="toggleKeterangan()" class='btn btn-xs default'>show <i class='fa fa-caret-down'></i> </a> 
								<button class='btn btn-xs blue' onclick="toggleTerkirim()">SHOW terkirim per hari ini</button>
								<br>
							...
							<div class='row'>
										...
										<form action="" id="form-search-barang" hidden>
											...
											<select name="id" class='form-control' style="width:250px; display:inline-block" id="select-barang" >
												...
												<?foreach ($request_barang_detail as $row) {
													...
													$nama_barang = explode("??", $row->nama_barang);
													for ($i=0; $i < count($barang_beli_id_get)  ; $i++) {?> 
														<option  <?=($barang_beli_id == $barang_beli_id_get[$i] ? 'selected' : '')?> value="<?=$barang_beli_id_get[$i]?>"><?=$nama_barang[$i];?></option>
													<?}
												}?>
											</select>
											<button class='btn default' style="display:inline-block" ><i class='fa fa-search'></i></button>
								...
								foreach ($request_barang_qty_all as $row) {
									$status_beda[$row->barang_beli_id][$row->warna_id][$row->bulan_request] = ($no_batch == 1 ? 0 : $row->tipe );
								}
													...
													<?foreach ($warna_id_count as $key2 => $value2) {
														...
														foreach ($widList[$key2] as $key3 => $value3) {
															...
															if ($tp == 1) {
																...
																$classStatus .= (isset($isTercapai[$value][$key2][$bln_up]) && $no_batch > 1 ? 'pre-finished' : '' );
															}
															// $classUrgent = ($classFinished != 'finished' ? ($urg != '' ? 'urgent' : '') : '');
															?>
															<tr style="<?=($idx == 0 ? $brdr_first : '')?>; <?=($idx2 == 0 && $idx != 0 ? $brdr_warna : '')?>; <?=($barang_beli_id != '' && $barang_beli_id != $value ? 'display:none' : '')?>" class="<?=($barang_beli_id != '' && $barang_beli_id != $value ? 'row-hidden' : '')?>" >
																<?if ($idx == 0) {?>
																	<!-- kolom nama barang -->
																	<td rowspan="<?=count($request_barang_detail_id_b)+ count($warna_id_count);?>"  
																		...
																		<?}else{
																			...
																			<a style="max-width:120px; display:inline-block;word-wrap:break-word;" class='btn btn-md default' href="<?=base_url().is_setting_link('transaction/request_barang');?>?id=<?=$barang_id_get[$key];?>&barang_beli_id=<?=$value?>&request_barang_batch_id=<?=$request_barang_batch_id;?>&supplier_id_filter=<?=$supplier_id_filter?>">
																				<?=$nb;?>
																			</a>
																		<?}?>
																	</td>
																...
																<!-- qty terkirim -->
																	<td  class='<?=$classStatus;?> <?=$classStatusInit;?>' ondblclick="urgentByWarna()" data-info="req-<?=$request_barang_qty_id;?>" rowspan="<?=$value2?>" style="position:relative;cursor:pointer;;<?=$brdr;?>;vertical-align:middle; text-align:center; font-size:1em" data-request="req-<?=$value3['request_barang_detail_id'];?>" data-status="<?=$value3['status_urgent'];?>" >
																		<div class="po-warna-batch" data-toggle="popover" data-placement='top' data-trigger='click' data-reqqtyid="<?=$request_barang_qty_id?>" data-detailId="<?=$value3['request_barang_detail_id'];?>" data-container="body" title="<?=$nama_barang[$key].' <b>'.$value3['nama']?></b>" data-html='true' data-content="<?=$popover_content;?>" style="z-indez:-1;position:absolute; padding:3px 5px;; top:0px; left:0px; height:100%;width:100%;" >
																			...
																			<div class='info-terkirim-now' style='display:none; color:blue' >
																				<?if (isset($terkirim_now[$value][$key2][$bln_up]) && $terkirim_now[$value][$key2][$bln_up] > 0 ) {?>
																					<?if($subtotal[$key2] >=$terkirim_now[$value][$key2][$bln_up] ){
																						// echo '=='.$subtotal[$key2] .'>='.$terkirim_now[$value][$key2][$bln_up].'==';
																						$total_terkirim_now[$bln_up] += $terkirim_now[$value][$key2][$bln_up];
																						// echo '--'.$terkirim_now[$value][$key2][$bln_up].'--';
																					}else{
																						$total_terkirim_now[$bln_up] += $subtotal[$key2];
																						// echo '--'.$subtotal[$key2].'--';
																					}?>
																					<?=str_replace(",00","", number_format($terkirim_now[$value][$key2][$bln_up],2,",",".")) ;?>
																				<?}?>
																			</div>
																	</td>
																<?}?>
															...
															<?if (($idx2+1) == $value2 ) {
																...
																<tr 
																	class="<?=($barang_beli_id != '' && $barang_beli_id != $value ? 'row-hidden' : '')?> " 
																	style="border-top:1px solid #555;
																		<?=($barang_beli_id != '' && $barang_beli_id != $value ? 'display:none' : '')?>"
																	>
						...
						<div class='text-right hidden-print'>
							...
							<?}else if($request_barang_id != '' && $locked_by != ''){?>
								<a type="button" class="btn btn-lg yellow-gold btn-print" target="_blank" href="<?=base_url()?>transaction/generate_request_barang_pdf?request_barang_batch_id=<?=$request_barang_batch_id?>&supplier_id=<?=$supplier_id_filter;?>">PDF</a>
...
<script>
	...
	var barang_beli_id = "<?=$barang_beli_id;?>";
	...
	foreach ($request_barang_qty as $row) {
		...
		if ($row->request_barang_batch_id == $request_barang_batch_id && $row->barang_beli_id == $barang_beli_id) {?>
		...
		<?}elseif($row->barang_beli_id == $barang_beli_id){?>
	...
	jQuery(document).ready(function() {
		...
		$("#barang-beli-list-select, #select-barang").select2();

		$("#barang-beli-list-select").change(function(){
			const option = $(this).find('option:selected', this).attr('data-barang');
			$("#barang-list-select").val(option).change();
			console.log(option);
		})
	...
	function populateBarang(){
		...
		// console.log('bpqc',barangPoQtyC);
		// console.log('bq',barangQty);
	...
	function submitRequest(){
		...
		data['barang_beli_id'] = "<?=$barang_beli_id;?>";
		data['request_barang_batch_id'] = "<?=$request_barang_batch_id;?>";
		data['data'] = dataSend;
		var url = "transaction/request_barang_submit_beli";
		console.log('DS',dataSend);
		ajax_data_sync(url,data).done(function(data_respond  ,textStatus, jqXHR ){
			console.log(data_respond);
				// alert(data_respond)
				if(data_respond == 'OK'){
	...
	function getPOBaru(){
		...
		console.log('get po baru', po_baru_list.length);
		if (po_baru_list.length > 0) {
			var data = {};
	...
	function toggleTerkirim(){
		$('.info-terkirim').toggle();
		$('.info-terkirim-now').toggle();
	}
</script>