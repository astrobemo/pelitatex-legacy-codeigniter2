<?
		...
		foreach ($brs as $periode => $value) {
			...
			foreach ($value as $index => $data) {
				...
				//=====index nya nama object
				$yAxisBefore = $yAxisAfter;
				
				if (strlen($data['nama']) > 7 && $data['jumlah_baris'] > 2 ) {
					$yNow = $pdf->getY();
					
					$pdf->setXY($setXStart, $yNow+(($tinggi*$data['jumlah_baris'])/2) - 2 );
					$pdf->Multicell(15,$tinggi, strtoupper($data['nama']),0,'C');
					
					$pdf->setXY($setXStart,$yNow);
					$pdf->Multicell(15,($tinggi*($data['jumlah_baris'])), '',$garis,'C');
					$yAfter = $pdf->getY();
					$pdf->setY($yAxisBefore + ($tinggi*($data['jumlah_baris']/2)));
				}else if (strlen($data['nama']) > 7 && $data['jumlah_baris'] == 2 ){
					$pdf->SetFont( $font_name, '', 8 );
					$pdf->Multicell(15,$tinggi, strtoupper($data['nama']),$garis,'C');
					$pdf->SetFont( $font_name, '', $general_font_size );
				}else{
					$pdf->Multicell(15,($tinggi*($data['jumlah_baris'])), strtoupper($data['nama']),$garis,'C');
				}
				...
				$pdf->Multicell(15,($tinggi*$data['jumlah_baris']), str_replace(',00','',number_format($data['total_request'],'2',',','.')),$garis,'C');				
				...
				foreach ($data['warna'] as $index2 => $data_warna) {
					...
					$pdf->Multicell(44,($tinggi*$data_warna['jumlah_baris']),$data_warna['nama'],'B','L',$status_fill);
