...
function print_kombinasi_3(printer_name){
...
foreach ($data_penjualan_detail_group as $row) {
	...
	<?
		if ($baris_idx > 20 || $total_roll_all > 20  ) { ?>
			...
			var data = ['\x1B' + '\x40'+          // init
							...
							'\x1B' + '\x21' + '\x14'+ // em mode on
						   	...
						   	<?="'".sprintf('%-1.1s', '|')."'";?>+
						   		...
						   		<?for ($n=0; $n < 10; $n++) { 
						   			$k = 10 * $m + $n;
						   			?>
									<?="'".sprintf('%6.6s', (isset($qty_c[$k]) ? ($row->satuan_id == 1 ? is_qty_general($qty_c[$k]) : $qty_c[$k]) : '' ) )."'";?>+ '\x09'+
						   		<?}?>
				...
				'\x0C'+          // cut paper
			   	'\x0D',  // Generate Pulse to kick-out cash drawer**
