							...
							<div class="form-group">
			                    ...
			                    * </span>
			                    ...
			                    <div class="col-md-6">
			                    	...
			                    	<select class='input1 form-control supplier-input' style='font-weight:bold' name="supplier_id" id="supplier_id_edit" onchange="checkPOSupplier()">
			                    		<?foreach ($this->supplier_list_aktif as $row) { ?>
...
var po_pembelian_batch_aktif = [];

<?if ($pembelian_id != '') {?>
	selected_supplier = "<?=$supplier_id;?>";
	selected_po = "<?=$po_pembelian_batch_id?>";
	po_pembelian_batch_aktif = <?=json_encode($this->po_pembelian_batch_aktif);?>;
<?}?>
...
jQuery(document).ready(function() {
    ...
    <?if ($pembelian_id != '' && $uniq_blessing == true) { ?>
		...
		$("#form_add_data input").keydown(function(e){
			if (e.keyCode == 13) {
				e.preventDefault();
			}
		});
...
function saveBrg(){
	...
	const po_batch_id = "<?=$po_pembelian_batch_id?>";
	const barang_beli_id_before = "<?=$barang_beli_id_before?>";
	const is_multiple_item = "<?=$is_multiple_item?>";

	console.log('po_batch_id',po_batch_id);
	console.log('barang_beli_id_before',barang_beli_id_before);
	console.log('is_multiple_item',is_multiple_item);

	if (po_batch_id != '' && po_batch_id != 0 && barang_beli_id_before !== '' && is_multiple_item == "") {
		$("#btn-save-brg").attr('disabled',false);
		if($('#warna_id_select').val() == ''){
...
function checkSupplier(){
	selected_po = $("#po_list_edit").val();
	for (const key in po_pembelian_batch_aktif) {
		if (selected_po == po_pembelian_batch_aktif[key].id) {
			const newSupp = po_pembelian_batch_aktif[key].supplier_id;

			if (newSupp != selected_supplier) {
				$("#supplier_id_edit").val(newSupp);
				selected_supplier = newSupp;
				bootbox.alert("Supplier berhasil dirubah")
			}
		}
	}
	// po_pembelian_batch_aktif.each((i,v)=>{
	// 	console.log(i,v);
	// })
} 

function checkPOSupplier(){
	selected_supplier = $("#supplier_id_edit").val();
	for (const key in po_pembelian_batch_aktif) {
		if (selected_po == po_pembelian_batch_aktif[key].id) {
			const newSupp = po_pembelian_batch_aktif[key].supplier_id;

			console.log(newSupp, selected_supplier, newSupp != selected_supplier)
			if (newSupp != selected_supplier) {
				bootbox.alert("Supplier dan PO tidak match");
			}
		}
	}
}
