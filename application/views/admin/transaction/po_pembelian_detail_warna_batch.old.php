				...
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('transaction/po_pembelian_warna_insert')?>" class="form-horizontal" id="form_add_barang" method="post">
							...
							<div class="form-group">
			                    <label class="control-label col-md-3">Barang (PO Besar)<span class="required">
			                    * </span>
			                    </label>
			                    <div class="col-md-8">
			                    	<input name='po_pembelian_id' value='<?=$po_pembelian_id;?>' hidden>
			                    	...
			                    	<select name="po_pembelian_detail_id" class='form-control input1' id='barang_id_select'>
		                				...
		                				foreach ($data_barang_po as $row) { 
		                					...
		                					?>
			                    			<option value="<?=$row->id?>" <?=($po_pembelian_detail_last == $row->id ? 'selected' : '');?> ><?=$row->nama_barang;?> <b><?=$row->harga;?></b> | Kuota : <?=number_format($row->sisa_kuota,'0',',','.');?></option>
			                    		<? } ?>
			                    	</select>
			                    	...
			                    	<select id='barang_id_copy' hidden>
		                				<?foreach ($data_barang_po as $row) { 
		                					...
		                					?>
			                    			<option value="<?=$row->id?>" ><?=$row->harga;?></option>
			                    		<? } ?>
			                    	</select>
			                    </div>
			                </div>
			                    ...
			                    <div class="col-md-8">
			                    	<div class="radio-list">
										<label class="radio-inline">
					                    	<input name='tipe_barang' type='radio' value='1' <?=($tipe_barang_last == '' ? 'checked' : '')?> <?=($tipe_barang_last == 1 ? 'checked' : '');?> >Reguler</label>
										<label class="radio-inline">
					                    	<input name='tipe_barang' type='radio' value='2' <?=($tipe_barang_last == 2 ? 'checked' : '');?> >Rename</label> <a id='rename-a' href="#portlet-config-info" data-toggle="modal"><sup>info</sup></a>
										<label class="radio-inline">
					                    	<input name='tipe_barang' type='radio' value='3'  <?=($tipe_barang_last == 3 ? 'checked' : '');?>>Split</label> <a id="split-a" href="#portlet-config-info" data-toggle="modal"><sup>info</sup></a>
					                    <label class="radio-inline">
					                    	<input name='tipe_barang' type='radio' value='4'  <?=($tipe_barang_last == 4 ? 'checked' : '');?>>Split, Rename</label> <a id='splitrename-a' href="#portlet-config-info" data-toggle="modal"><sup>info</sup></a>
			                    	</div>
			                    </div>
			                </div>
			                ...
			                <div class="form-group barang-id-baru" <?=($tipe_barang_last == 1 || $barang_id_baru_last == '' ? 'hidden' : ''  );?>  >
			                    ...
			                    <div class="col-md-8">
			                    	<select name="barang_id_baru" class='form-control' id='barang_id_baru_select'>
			                    		<option value=''>Pilih</option>
		                				<?foreach ($this->barang_list_aktif_beli as $row) { 
		                					if (!isset($nama_occupied[$row->nama])) {?>
				                    			<option <?=($barang_id_baru_last == $row->id ? 'selected' : '' );?>  value="<?=$row->id?>" ><?=$row->nama;?></option>
		                					<?}
		                					?>
			                    		<? } ?>
			                    	</select>
			                    ...
			                    * </span>
			                    ...
			                    <div class="col-md-8 form-field-tipe-4" <?=($tipe_barang_last == 4 ? '' : 'hidden'  );?> >
			                    	<select name="barang_id_baru_rename" class='form-control' id='barang_id_baru_select_rename'>
			                    		<option value=''>Pilih</option>
		                				<?foreach ($this->barang_list_aktif_beli as $row) { 
		                					if (!isset($nama_occupied[$row->nama])) {?>
				                    			<option <?//=($barang_id_baru_last == $row->id ? 'selected' : '' );?>  value="<?=$row->id?>" ><?=$row->nama;?></option>
			                ...
			                <div class="form-group harga-baru" >
			                    ...
			                    * </span>
			                    ...
			                    <div class="col-md-8">
			                		<input type="text" class="form-control amount-number" name="harga_baru" value="<?=$harga_baru_last;?>" />
			                    </div>
			                </div>
				...
				<div class="portlet light">
					...
					<div class="portlet-body">
						...
						<div id="po-pembelian-header" style='margin-bottom:15px'>
							...
							<?}else{?>
								<h1 class='text-center'><span style='color:blue'>DETAIL BARANG & WARNA <?=($revisi_count > 1 ? '(R)' : '');?></span>
								</h1>
								<?if ($revisi_count > 1) {?>
									...
									foreach ($data_barang_po as $row2) {
										...
										foreach ($po_pembelian_data_warna[$row2->id] as $row) { 
											...
											<tr >
												<?if ($i == 1) {?>
													...
													<td rowspan='<?=$merge_baris;?>'><?=$row->harga;?></td>
												<?}?>
												...
												<td class='text-right <?=$class_bg?>' style='border-right:none'>
													...
													<?if ($batch_status == 1 && $class_bg == '') {?>
														<input class='free-input-sm qty qty-input amount_number' style='border:1px solid #ddd' value="<?=str_replace(',00', '', number_format($row->qty,'2',',','.'));?>">
													<?}else{?>
										...
										<?if (isset($po_pembelian_data_warna[$row2->id]) && count($po_pembelian_data_warna[$row2->id]) > 0) {?>
											<tr>
												...
												<td class='text-right' style='border-right:0px' id='batch-qty-total'><?=str_replace(',00', '', number_format($subtotal,'2',',','.'));?> </td>
										...
										foreach ($po_pembelian_data_warna_split[$row2->id] as $isi) {
											if ($isi->tipe_barang == 3) {
												$po_split[$row2->id][$isi->barang_id_baru][$isi->nama_warna] = $isi;
											}elseif($isi->tipe_barang == 4){
												$po_split[$row2->id][$isi->barang_id_baru_rename][$isi->nama_warna] = $isi;
											}
										}
										...
										if (count($po_pembelian_data_warna_split[$row2->id]) > 0) {
											...
											foreach ($po_split[$row2->id] as $key => $value) {
												...
												$total_row += $merge_baris;
												foreach ($po_split[$row2->id][$key] as $row) { 
													$tgl_beli = explode(',', $row->tanggal_beli);
													$qty_beli = explode(',', $row->qty_beli);
													$no_faktur = explode(',', $row->no_faktur);
													$total_kirim = 0;
													...
													<tr>
														<?if ($i == 1) {?>
															...
															<td rowspan='<?=$merge_baris;?>'><?=number_format($row->harga,'0',',','.');?></td>
														...
														<td class='text-right <?=$class_bg;?> ' style='border-right:none'>
															<?$subtotal += $row->qty?>
															<?if ($batch_status == 1) {?>
																<input class='free-input-sm qty qty-input amount_number' value="<?=str_replace(',00', '', number_format($row->qty,'2',',','.'));?>">
															<?}else{?>
																<span class='free-input-sm qty' ><?=str_replace(',00', '', number_format($row->qty,'2',',','.'));?> </span>
															<?}?>
														</td>
...
<?foreach ($history_harga as $key => $value) {?>
	history_harga["<?=$key?>"] = "<?=$value;?>";
<?}?>
...
jQuery(document).ready(function() {
	...
	$('#barang_id_select, #barang_id_baru_select, #barang_id_baru_select_rename').select2({
        placeholder: "Pilih...",
        allowClear: true
    });
    ...
    $('#general_table').on('click', '.btn-detail-edit', function(){
    	...
    	form.find("[name=qty]").val(ini.find('.qty').val());
    	...
    	form.find("[name=harga_baru]").val(reset_number_format(ini.find('.harga_baru').html()) );
    	
    });

    $("[name=tipe_barang]").change(function(){
    ...
    $('#barang_id_select').change(function(){
    	...
    	var harga = $("#barang_id_copy [value='"+barang_id+"']").text();
    	$("#form_add_barang [name=harga_baru]").val(harga);
    });

    $("#barang_id_baru_select_rename, #barang_id_baru_select").change(function(){
    	let barang_id = $(this).val();
    	let harga = $("#barang-id-baru-copy [value='"+barang_id+"']").text();
		$('#form_add_barang [name=harga_baru]').val(harga);
		<?if (is_posisi_id() == 1) {?>
			// alert(harga);
		<?}?>
    ...
    $("#general_table").on("change",".qty-input", function(){
    	...
    	var data = {};
    	data['id'] =  id;
    	data['qty'] = reset_number_format($(this).val());
    	var url = 'transaction/po_pembelian_batch_qty_update';
    	// update_table(ini);
    	ajax_data_sync(url,data).done(function(data_respond  /*,textStatus, jqXHR*/ ){
			if (data_respond == 'OK') {
				update_total_qty();
...
function update_total_qty(){
	...
	$("#general_table .qty-input").each(function(){
		total += parseFloat(reset_number_format($(this).val()));
	});

	$("#batch-qty-total").html(change_number_format(total));
