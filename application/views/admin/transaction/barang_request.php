	...
	<div class='container'>
		...
		foreach ($barang_terkirim as $row) {
			if ($row->qty > 0 && $row->qty != '' ) {
				$terkirim_now = $row->qty;
				if (isset($terkirim_by_warna[$row->barang_id][$row->warna_id][$row->bulan_request.'-01'])) {
					$terkirim_now += $terkirim_by_warna[$row->barang_id][$row->warna_id][$row->bulan_request.'-01'];
				}
				$terkirim[$row->warna_id][$row->bulan_request.'-01'][$row->po_pembelian_batch_id] = $row->qty; 
				$bulan_next = date('Y-m-d', strtotime("+1 month", strtotime($row->bulan_request.'-01')));
				if ($terkirim_now > $row->qty_request) {
					$terkirim_by_warna[$row->barang_id][$row->warna_id][$bulan_next] = $terkirim_now - $row->qty_request;
					$terkirim_by_warna[$row->barang_id][$row->warna_id][$row->bulan_request.'-01'] = $row->qty_request;
				}else{
					$terkirim_by_warna[$row->barang_id][$row->warna_id][$row->bulan_request.'-01'] = $terkirim_now;
				}
			}
		}
			...
			<div class="col-md-12">
						...
						<?//if (is_posisi_id() == 1 ) {?>
							<div class="actions hidden-print">
								<?if ($request_barang_id != '' && $locked_by == '') {?>
									...
									<a onclick="populateBarang()" class="btn btn-default btn-sm"> Daftar PO </a>
								<?}?>
									...
									<tr>
										...
										// for ($i=$bulan_berjalan; $i <= 12 ; $i++) {
											...
											if (isset($qty_request[$row->warna_id][$i.'-01'])) {
												$request_bar_width = ($qty_request[$row->warna_id][$i.'-01'] / $forecast_now );
				...
				<div class="portlet light">
					...
					<div class="portlet-body">
						...
						<table class='table table-bordered' id='table-request-print'>
						<?
						// print_r($terkirim);
						$status_beda = array();
						$total_request = array();
						...
						$month_color = ['lightSalmon','orange','lightGreen','thistle','lightGray'];
						$idx_color = 0;
						foreach ($request_barang_detail as $row) {
							...
							<thead>
								<tr style="border-top:2px solid #aaa">
									<th colspan='7' class='text-center nama-bulan' style='background:<?=$month_color[$idx_color];?>'><span style='font-size:1.2em'><?=date('F Y', strtotime("+1 year", strtotime($bulan_request)));?> </span> </th>
								</tr>
								<tr>
							...
							<?foreach ($barang_id_get as $key => $value) {
											...
											<tr style="<?=($idx == 0 ? 'border-top:2px solid #aaa' : '')?>">
												<?if ($idx == 0) {?>
													<td rowspan="<?=count($request_barang_detail_id_b)?>"  style="font-size:1.2em; vertical-align:middle" class="text-center">
														...
														<?}else{?>
															<a class='btn btn-md default' href="<?=base_url().is_setting_link('transaction/request_barang');?>?id=<?=$value;?>"><?=$nama_barang[$key]?></a>
														<?}?>
													</td>
												<?}?>
												<?if ($idx2 == 0) {
													$x = implode(',', $po_by_warna[$key2]);?>
													<td rowspan="<?=$value2?>" ondblclick="urgentByWarna('<?=$x?>')"  style="cursor:pointer;<?=$classTipe?>;border-bottom:1px solid #333; font-size:1.2em; vertical-align:middle" class="text-center"><?=$value3['nama']?></td>
													<td rowspan="<?=$value2?>" ondblclick="urgentByWarna('<?=$x?>')" style="cursor:pointer;<?=$classTipe?>;border-bottom:1px solid #333; font-size:1.2em; vertical-align:middle" class="text-center" ><?=number_format($subtotal[$key2],'0',',','.')?></td>
												<?}?>

												<?if ($idx == 0) {?>
													<td rowspan="<?=count($request_barang_detail_id_b)?>"  style="font-size:1.2em; vertical-align:middle" class="text-center">
														<?=number_format($total_request[$value],'0',',','.');?>
													</td>
												<?}?>
												<td style="cursor:pointer;<?=$urg;?>;<?=$classTipe?>;<?=$brdr;?>" data-request="req-<?=$value3['request_barang_detail_id'];?>" data-status="<?=$value3['status_urgent'];?>" ondblclick="urgentByPO(<?=$value3['request_barang_detail_id']?>)">
												...
												<?$bg_tipe = $status_request[$bulan_request][$value][$key2];?>
												<td style="cursor:pointer;<?=$urg;?>;text-align:right;<?=$classTipe?>;<?=$brdr;?>" data-request="req-<?=$value3['request_barang_detail_id'];?>" data-status="<?=$value3['status_urgent'];?>" ondblclick="urgentByPO(<?=$value3['request_barang_detail_id']?>)"><?=number_format($value3['qty'],'0',',','.');?></td>
												<?if($idx2==0){?>
													<td rowspan="<?=$value2?>" style="cursor:pointer;<?=$urg;?>;<?=$classTipe?>;<?=$brdr;?>;vertical-align:middle; text-align:center; font-size:1.2em" data-request="req-<?=$value3['request_barang_detail_id'];?>" data-status="<?=$value3['status_urgent'];?>" ondblclick="urgentByPO(<?=$value3['request_barang_detail_id']?>)">
														<?$bln_up = date('Y-m-01', strtotime("+1 year", strtotime($bulan_request)));
														if (isset($terkirim_by_warna[$value][$key2][$bln_up])) {?>
															<?=str_replace(",00","", number_format($terkirim_by_warna[$value][$key2][$bln_up],2,",",".")) ;?>
														<?}?>
													</td>
												<?}?>
												<?if ($idx2 == 0) {/*?>
													<td rowspan="<?=$value2?>" style="<?=$classTipe?>;border-bottom:1px solid #333; font-size:1.2em; vertical-align:middle" class="text-center" ><?=number_format($subtotal[$key2],'0',',','.')?></td>
												<?*/}?>
											</tr>
										<?$idx2++; $idx++; }?>
									<? }?>
								...
								<?}?>
							<?}?>
							<tr style='background-color:#aaa'><td colspan='7'></td></tr>
							</tbody>
						</table>
...
function urgentByPO(request_barang_detail_id){

	let status_urgent = $(`[data-request='req-${request_barang_detail_id}']`).attr('data-status');
	status_urgent = (status_urgent-1)*-1;

	$("#table-request-print").find(`[data-request='req-${request_barang_detail_id}']`).css({'color':'#ddd','cursor':'no-drop'});
	...
	ajax_data_sync(url,data).done(function(data_respond  ,textStatus, jqXHR ){
		if (data_respond == 'OK') {
			if (status_urgent == 1) {
				$("#table-request-print").find(`[data-request='req-${request_barang_detail_id}']`).css({'color':'red','cursor':
					'pointer'});
			}else{
				$("#table-request-print").find(`[data-request='req-${request_barang_detail_id}']`).css({'color':'black','cursor':
					'pointer'});
			};

			$(`[data-request='req-${request_barang_detail_id}']`).attr('data-status').val(status_urgent);
