...
jQuery(document).ready(function() {
	...
	$("#general_table").DataTable({
   		"fnCreatedRow": function( nRow, aData, iDataIndex ) {
            ...
            var kode = status[4];
            var batch_list = [3];
            var locked_master = status[5].split('||');
            var locked_batch = status[6].split('||');
	        var barang_list = '';
			...
			const batch_lock = locked_batch[0].split(',');
			const batch_release = locked_batch[1].split(',');
			...
			<li style="width:40px"><div class='po-box'>${po_batch[index]}</div></li>
							...
							<li style="width:${w_st}px"><i class="fa  ${(batch_release[index] != '-' ? 'fa-circle' : 'fa-circle-o' )}"></i></li>
			<li style="width:40px"><div class='po-box'>${po_batch[index]}</div></li>
							<li style="width:40px;"><i class="fa fa-lock ${(batch_lock[index] != '-' ? '' : 'disabled' )}"></i></li>
							<li style="width:40px"><i class="fa fa-send ${(batch_release[index] != '-' ? '' : 'disabled' )}"></i></li> */
