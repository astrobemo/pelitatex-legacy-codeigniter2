				...
				<div class="portlet light" >
					<div class="portlet-title">
						...
						<div class="actions hidden-print">
							<?if ($view_type != 2) {?>
								<a href="#portlet-config" data-toggle='modal' class="btn btn-default btn-sm btn-form-add" onclick="cek_last_input()">
					...
					<div class="portlet-body">
							...
							<table id="data-header">
								<tr>
									<td>
										...
										<?}else if($status_po != 1 && $view_type != 2){?>
						...
						<div style="margin-bottom: 15px;">
							Show Terkirim : <input type="checkbox" id="infoSwitch" 
								<?=($status_po == 1 ? '' : 'checked');?> 
						...
						<table id='table-barang' >
							<thead>
								<tr>
									...
									<th>Barang
										<?if ($status_po == 1) {?>
											<a href="#portlet-config-detail" data-toggle="modal" class='btn btn-xs blue' id="btnAddBarang"  onclick="cek_last_input()"><i class="fa fa-plus"></i></a>
										<?}?>
									</th>
									<th>Warna</th>
									<th>QTY</th>
...
const qtyAdd = document.querySelector("#qty_add");
const qtyAddInfo = document.querySelector("#info_qty_add");
const qtyEdit = document.querySelector("#qty_edit");
const qtyEditInfo = document.querySelector("#info_qty_edit");
...
const barangIdLast = "<?=$barang_id_last?>";
const statusPo = "<?=$status_po;?>"
var tipePO = "<?=$tipe;?>";
...
jQuery(document).ready(function() {
	...
	$(document).keydown(function(e) {
		if (statusPo == 1) {
			...
			if(e.key=='Alt'){
				...
				if (isInfoShown) {
					$(".invoice-info").show();
					$("#infoSwitch").bootstrapSwitch("state",true);
				}else{
					$(".invoice-info").hide();
					$("#infoSwitch").bootstrapSwitch("state",false);
				}
	...
	qtyAdd.addEventListener("change",function(e){
		qtyAddInfo.innerHTML = (``);
		if (e.target.value != '') {
			if (e.target.value.includes(",")) {
				qtyAddInfo.innerHTML = `Untuk pecahan desimal mohon gunakan <span style="color:red">titik(.)</span> `;
			}
		}
	})

	qtyEdit.addEventListener("change",function(e){
		qtyEditInfo.innerHTML = (``);
		if (e.target.value != '') {
			if (e.target.value.includes(",")) {
				qtyEditInfo.innerHTML = `Untuk pecahan desimal mohon gunakan <span style="color:red">titik(.)</span>`;
			}
		}
	})
});
...
function cek_last_input(){
	if(barangIdLast != ''){
		setTimeout(function(){
			$('#barang_id_select').val(barangIdLast).change();
		},700);
	}else{
		setTimeout(function(){
