				...
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('transaction/new_request_insert')?>" class="form-horizontal" id="form_add_data" method="post">
                			...
                			<input name='type' >
                			<input name='request_barang_id' value="<?=$request_barang_id;?>" >
					...
					<div class="portlet-body">
						...
						foreach ($request_barang_detail as $row) {
							...
							<tr  style="border-top:2px solid #aaa">
								<td colspan='6' class='text-center nama-bulan' style='background:<?=$month_color[$idx_color];?>'><span style='font-size:1.2em'><?=date('F Y', strtotime("+1 year", strtotime($bulan_request)));?> </span> </td>
							</tr>
							<?$idx_color++;?>
							<?foreach ($barang_id_get as $key => $value) {?>
									...
									<?foreach ($warna_id_count as $key2 => $value2) {
										...
										foreach ($widList[$key2] as $key3 => $value3) {
											...
											<tr style="<?=($idx == 0 ? 'border-top:2px solid #aaa' : '')?>">
												...
												<?if ($idx2 == 0) {?>
													<td rowspan="<?=$value2?>"  style="<?=$classTipe?>;border-bottom:1px solid #333; font-size:1.2em; vertical-align:middle" class="text-center"><?=$value3['nama']?></td>
												<?}?>
												<td style="<?=$classTipe?>;<?=$brdr;?>"><?=$value3['po_number']?></td>
												<td style="<?=$classTipe?>;<?=$brdr;?>"><?=number_format($value3['qty'],'0',',','.');?></td>
												<td style="<?=$classTipe?>;<?=$brdr;?>">
													<?$bln_up = date('Y-m-01', strtotime("+1 year", strtotime($bulan_request)));
													if (isset($terkirim[$key2][$bln_up][$widList[$key2][$key3]['po_pembelian_batch_id']])) {?>
														<?=$terkirim[$key2][$bln_up][$widList[$key2][$key3]['po_pembelian_batch_id']];?>
													<?}?>
												</td>
												<?if ($idx2 == 0) {?>
								...
								<?}?>
							<?}?>
							<tr style='background-color:#aaa'><td colspan='6'></td></tr>
							</tbody>
						</table>
