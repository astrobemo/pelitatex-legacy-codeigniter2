		...
		<?
			...
			foreach ($po_pembelian_data_batch as $row) {
				if ($row->id == $batch_id) {
					...
					$release_batch_person = $row->release_person;
					$release_batch_date = $row->released_date;
					$keterangan_batch = $row->keterangan_batch;
					...
					<div class="portlet-body">
						...
						<div id="po-pembelian-header" style='margin-bottom:15px'>
							<div class='text-center'>
								<?if ($release_batch_person !='') {?>
									<h4 class='note note-warning hidden-print' style='margin:40px 0px;'>
										PO released oleh <b><?=$release_batch_person?></b> pada <b><?=date('d M Y', strtotime($release_batch_date));?></b>
									</h4>
								<?}?>
						...
						<div>
			                ...
			                <button style="float:right"  class="btn btn-lg red default button-remove hidden-print"><i class='fa fa-warning'></i> HAPUS</button>
							<?if ($batch_id != '' && $batch_status == 2 && $release_batch_person == '') {?>
								<button class="btn btn-lg purple default hidden-print" id="btn-release" onclick="releasePOBatch('<?=$batch_id;?>')"><i class='fa fa-send'></i> Release</button>
...
jQuery(document).ready(function() {
    ...
    <?if ($po_pembelian_id != '' && is_posisi_id() != 6 && $batch_status == 1 ) { ?>
    	var map = {220: false};
...
function releasePOBatch(){
	bootbox.confirm("Yakin untuk merelease PO ini ? ", function(res){
		if(res){
			...
			fetch(baseurl+`transaction/po_pembelian_batch_release?id=${batch_id}`)
				.then((response) => response.json())
				.then((data) => {
					dialog.find('.bootbox-body').html('PO Released...refreshing page');
					setTimeout(() => {
