						...
						<table width='100%'>
							...
							<tr>
								<td style='vertical-align:top'>
			                    	...
			                    	$dt_po = array();
									if (is_posisi_id() == 1) {
										echo $po_pembelian_batch_id  .' == '. $po_pembelian_batch_id  .' == '. $barang_id_before .' == '. $supplier_id .' == '. $is_multiple_item;
										// print_r($barang_list);
									}
			                    	if ($po_pembelian_batch_id != '' && $po_pembelian_batch_id != 0 && $barang_id_before == ''
									 && !$is_multiple_item) {?>
			                    		<div class="form-group">
			                    	...
			                    	<?}else{?>
										<div class="form-group">
						                    ...
						                    * </span>
						                    ...
						                    <div class="col-md-6">
						                    	<?$harga_beli_b = '';
						                    	if ($barang_id_before != '' && $ockh_info != '-' && $supplier_id != 4) {?>
							                    	<input name='barang_id' hidden <?//=(is_posisi_id() != 1 ? 'hidden' : '');?> value="<?=$barang_id_before?>">
						                    	<?}?>
						                    	<select name="barang_id" class='form-control input1' id='barang_id_select' <?=($barang_id_before != ''  && $ockh_info != '-' && $supplier_id != 4 && !$is_multiple_item ? 'disabled' : '')?> >
					                				<option value=''>Pilih</option>
					                				<?foreach ($this->barang_list_aktif_beli as $row) { ?>
						                    			<option <?=($barang_id_before == $row->id && !$is_multiple_item ? 'selected' : '' )?> value="<?=$row->id?>"><?=$row->nama;?></option>
						                    		<? } ?>
						                    	</select>
						                    	<select name='data_barang' hidden>
						                    		<?foreach ($this->barang_list_aktif_beli as $row) { 
						                    			if ($barang_id_before != '' && $harga_id_before == '') {
						                    				if ($barang_id_before == $row->id) {
    ...
    <?if ($po_pembelian_batch_id != '' && $po_pembelian_batch_id != 0){?>
    	$('#barang_id_select').change(function(){
			...
			if (pdid == '' && typeof data_brg[1] !== 'undefined') {
				$('#form_add_barang [name=harga_beli]').val(change_number_format(data_brg[1]));
			}
