			...
			<div class="col-md-12">
				<div class="portlet light">
					<div class="portlet-title">
						<div class="caption caption-md">
							<i class="icon-bar-chart theme-font hide"></i>
							<span class="caption-subject theme-font bold uppercase"><span id='forecast-title'>Forecast</span><span id='ori-title' style='color:red' hidden>Penjualan Lalu</span><span id='berjalan-title' style='color:orange' hidden>Penjualan Berjalan</span> </span>
						</div>
					...
					<div class="portlet-body">
                        <b>NOTES : </b>
                        <b style='width:100px; height:100px;  background:lightgreen'>Bulan Berjalan</b>
								...
								<?foreach ($data_warna as $row) {?>
									<tr id="row-<?=$row->warna_id;?>" class='row-active'>
										...
										foreach ($periode as $date) {
											...
											<td style='<?=$bgn?>' class='data-container <?=($key_now == date('Y-m') ? "bj-view" : '');?> ' title='double click to edit' onclick="focusOut('<?=$row->warna_id?>','<?=$key?>-01')" id="container-<?=$date->format('Y-m-01')?>-<?=$row->warna_id?>" >
										...
										<th style="font-size:1.1em; text-align:right" id="totalWarna-<?=$row->warna_id;?>">
											<span class='total-forecast'><?=number_format($total_trx_warna_all[$row->warna_id],'0',',','.');?></span><span class='qty-ori' hidden><?=number_format($total_ori,'0',',','.')?></span>
                                        </th>
									</tr>
								<?}?>
							</tbody>
