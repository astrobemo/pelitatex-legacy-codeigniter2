...
<style>
    ...
    */
    ...
    #table-harga-add thead tr th { position: sticky; top: 0; }
                    ...
                    <div class="modal-body">
                        <form action="<?= base_url('masters/group_harga_customer/daftar_insert') ?>" autocomplete='off' class="form-horizontal" id="form_add_data" method="post" onkeydown="return event.key != 'Enter';">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <select name="customer_id" class='form-control' id="customer_id_select_add" style='border:none; font-size:1.1em; font-weight:bold;' >
                            ...
                            <div style='max-height:400px; overflow:auto'>
                                <table class='table' id='table-harga-add'>
                                    <thead>
                                        <!-- <span style="font-weight:0.7em">(optional)</span> -->
                                        <tr>
                                            <th class="text-center" rowspan='2' style="vertical-align:middle" >BARANG</th>
                                            <th class="text-center" >
                                                CASH<br> 
                                                <select name='tipe_cash' class='form-control' id="hargacashSelectAdd" onchange="showHargaTambah(`1`)" style="border:none; border-bottom:1px solid;">
                                                    <option value="">Pilih</option>
                                                    <?foreach ($harga_list as $row) {?>
                                                        <option value="<?=$row->id?>">
                                                            <?=$row->nama;?>
                                            ...
                                            <th class="text-center" >KREDIT <br> 
                                            <select name='tipe_kredit' class='form-control' id="hargakreditSelectAdd"  onchange="showHargaTambah(`2`)" style="border:none; border-bottom:1px solid">
                                                    <option value="">Pilih</option>
                                                    <?foreach ($harga_list as $row) {?>
                                                        <option value="<?=$row->id?>">
                                                            <?=$row->nama;?>
                                    ...
                                    <tbody>
                                        <?foreach ($this->barang_list_aktif as $row) {?>
                                            <tr>
                                                <td><?=$row->nama_jual;?></td>
                                                <td id='div-cash-<?=$row->id?>' class='kolom-cash'></td>
                                                <td id='div-kredit-<?=$row->id?>' class='kolom-kredit'></td>
                                            </tr>
                                        <?}?>
                                    </tbody>
                                </table>
                                    ...
                                    <tr style="background-color:lightgreen">
                                        ...
                                        <th style='width:150px;' class='text-center'>
                                            <select name="tipe_kredit" class='form-control text-center' style='background:transparent; border:none; font-weight:bold; font-size:1em' id="hargaKreditSelectEdit" onchange="showHarga(2, 'edit')">
                                                <option value="">Tidak Boleh Kredit</option>
                                                <?foreach ($harga_list as $row) {?>
                                                        <option value="<?=$row->id;?>"><?=$row->nama;?> </option>
                                                <?}?>
                                            </select>
                            ...
                            <tbody>
                                <?foreach ($harga_customer as $key => $row) {?>
                                    <tr id="baris-<?=$row->customer_id;?>">
                                        ...
                                        <td class='text-center'><?=($row->nama_kredit == "" ? '-' : $row->nama_kredit)?></td>
                                        <td>
...
function showHarga(tipe, table, harga_id){
    ...
    if (group_harga_barang_id.length > 0) {
        ...
        if (typeof harga_berlaku[`ss-${group_harga_barang_id}`] === 'undefined') {
            harga_berlaku[`ss-${group_harga_barang_id}`] = {};
            fetch(baseurl+`masters/group_harga_customer/get_harga_berlaku_by_group_id?group_harga_barang_id=${group_harga_barang_id}`)
            .then((response) => response.json())
            .then((data_res) => {
                harga_berlaku[`ss-${group_harga_barang_id}`] = data_res;
                dialog.modal('hide');
                drawTable(harga_berlaku[`ss-${group_harga_barang_id}`], label, dialog);
            });
        }else{
            dialog.modal('hide');
        }
...
function showHargaTambah(tipe){
    let group_harga_barang_id = "";
    const label = (tipe == 1 ? "div-cash" : "div-kredit");
    if (tipe==1) {
        group_harga_barang_id = document.querySelector(`#hargacashSelectAdd`).value;
    }else{
        group_harga_barang_id = document.querySelector(`#hargakreditSelectAdd`).value;
    ...
    if (typeof harga_berlaku[`ss-${group_harga_barang_id}`] === 'undefined') {
        ...
        .then((data_res) => {
            ...
            harga_berlaku[`ss-${group_harga_barang_id}`].forEach(row => {
                // console.log(row);
                const el = document.querySelector(`#${label}-${row.barang_id}`);
                if (el) {
                    const harga = row.harga_berlaku;
                    el.innerHTML = `${currency.rupiah(harga)}`;
                }
            });
        });
