			...
			<div class="col-md-12">
				<div class="portlet light">
					...
					<div class="portlet-body">
						...
						<table width='100%'>
							<tr>
								...
								<td style='width:35%' class='text-right'>
									...
									<?}else{?>
										<button class='btn btn-md yellow-gold tutup-buku'><i class='fa fa-bookmark'></i> TUTUP BUKU ULANG</button>
									<?}
							...
							<?}?>
								...
								<tbody>
									...
									foreach ($this->satuan_list_aktif as $row) {
										...
										// if (is_posisi_id()==1 && $row->jumlah_roll_assembly != 0) {
										// 	echo $row->nama_jual.' '.$row->warna_jual.' -> '.$row->jumlah_roll_assembly.'<br/>';
										// }
									}
										...
										if ($row->satuan_id != '') {
											...
											//if ($row->qty_beli > 0) {?>
												<tr <?//=($row->qty_beli > 0 ? '' : 'hidden');?> >
													...
													<td><?=($row->tipe_qty != 3 ? number_format($row->jumlah_roll_stock,'0',',','.') : '');?></td>
													<td class='text-center'><?=number_format($row->qty_stock * $row->hpp,'2',',','.')?></td>
													
													<td><?=number_format($row->qty_beli,'2',',','.')?></td>
													<td><?=($row->tipe_qty != 3 ?number_format($row->jumlah_roll_beli,'0',',','.') : '');?></td>
													...
													<td><?=($row->tipe_qty != 3 ? number_format($row->jumlah_roll_jual,'0',',','.') : '')?></td>
													...
													*/?>
													...
													<td><?=($row->tipe_qty != 3 ? number_format($row->jumlah_roll_penyesuaian,'0',',','.') : '')?></td>
													<td class='text-center'><?=number_format($row->qty_penyesuaian * $hpp_all,'2',',','.')?></td>
	
													<td><?=number_format($row->qty_assembly,'2',',','.')?></td>
													<td><?=($row->tipe_qty != 3 ? number_format($row->jumlah_roll_assembly,'0',',','.') : '')?></td>
													<td class='text-center'><?=number_format($row->qty_assembly * $hpp_all,'2',',','.')?></td>
		
													<td><?=number_format($row->qty_retur,'2',',','.')?></td>
													<td><?=($row->tipe_qty != 3 ? number_format($row->jumlah_roll_retur,'0',',','.') : '')?></td>
													<td class='text-center'><?=number_format($row->qty_retur * $hpp_all,'2',',','.')?></td>
		
													<td><?=number_format($row->qty_retur_beli,'2',',','.')?></td>
													<td><?=($row->tipe_qty != 3 ? number_format($row->jumlah_roll_retur_beli,'0',',','.') : '')?></td>
													<td class='text-center'><?=number_format($row->qty_retur_beli * $hpp_all,'2',',','.')?></td>
		
													<td><?=number_format($row->qty_lain,'2',',','.')?></td>
													<td><?=($row->tipe_qty != 3 ? number_format($row->jumlah_roll_lain,'0',',','.') : '')?></td>
													<td class='text-center'><?=number_format($row->qty_lain * $hpp_all,'2',',','.')?></td>
		
													<?
													$qty_akhir = $row->qty_stock + $row->qty_beli - $row->qty_jual + $row->qty_penyesuaian+ 
													$row->qty_retur- $row->qty_retur_beli-$row->qty_lain;
													$jumlah_roll_akhir = $row->jumlah_roll_stock + $row->jumlah_roll_beli - $row->jumlah_roll_jual + $row->jumlah_roll_mutasi_masuk -  $row->jumlah_roll_mutasi + $row->jumlah_roll_penyesuaian + 
													$row->jumlah_roll_retur-$row->jumlah_roll_retur_beli-$row->jumlah_roll_lain+$row->jumlah_roll_assembly;
													
													$nilai_akhir[$row->barang_id.'--'.$row->warna_id] = $qty_akhir * $hpp_all;
													$hpp_akhir[$row->barang_id.'--'.$row->warna_id] = $hpp_all;
													$qakhir[$row->barang_id.'--'.$row->warna_id] = $qty_akhir;
													$rakhir[$row->barang_id.'--'.$row->warna_id] = $jumlah_roll_akhir;
													$show='';
													if (is_posisi_id() == 1) {
													...
													?>
													<td><?=number_format( $qty_akhir ,'2',',','.')?> <?=$show;?> </td>
													<td><?=($row->tipe_qty != 3 ? number_format($jumlah_roll_akhir,'0',',','.') : '')?></td>
													<td class='text-center'>
														<?$nilai_stok_akhir += ($qty_akhir * $hpp_all); ?>
														<?=number_format($qty_akhir * $hpp_all,'2',',','.')?>
													</td>
													<?if (is_posisi_id()==1) {?>
														<td><?=$row->barang_id;?>/<?=$row->warna_id;?>/<?=round($hpp_all,2);?></td>
													<?}?>
												</tr><?
											//}
											# code...
						...
						<?//if ($tutup_buku_id != 0 && $tutup_buku_id_now == 0) {?>
							<form id="tutup-buku-form" action="<?=base_url();?>inventory/tutup_buku_insert" method="POST" <?=(is_posisi_id() != 1 ? 'hidden' : '')?> >
								<input name="tanggal" value="<?=$tanggal_start?>">
									<br/>
								<?foreach ($mutasi_barang_list as $row) {
									if (!isset($qakhir[$row->barang_id.'--'.$row->warna_id])) {
										$qakhir[$row->barang_id.'--'.$row->warna_id] = 0;
									}?>
									...
									<input style="<?=($qakhir[$row->barang_id.'--'.$row->warna_id] < 0 ? 'border:2px solid red':'');?>" name="qty[<?=$row->barang_id?>--<?=$row->warna_id?>]" value="<?=$qakhir[$row->barang_id.'--'.$row->warna_id];?>" >
									<input name="roll[<?=$row->barang_id?>--<?=$row->warna_id?>]" value="<?=$rakhir[$row->barang_id.'--'.$row->warna_id];?>" >
									<br/>
								<?}?>
							</form>
						<?//}?>
