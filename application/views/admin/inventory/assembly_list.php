                                    ...
                                    <div id="formContainer" class='section-div'>
                                        <form action="">
                                                    ...
                                                    <div class='select-container'>
                                                        ...
                                                        <div class='select-option-container select-option-container-out'>
                                                            ...
                                                            <ul class='select-option-list'>
                                                                <li data-id="barang-id-select" data-value="" class="" >Pilih...</li>
                                                                <?foreach ($this->barang_list_aktif as $row) {?>
                                                                    <li data-id="barang-id-select" data-value="<?=$row->id;?>" class="<?=($barang_id == $row->id ? 'selected' : '')?>" ><?=$row->nama_jual;?></li>
                                                                <?}?>
                                                            </ul>
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-6">
                                                    <div class='select-container'>
                                                        <label id='warna-id-select-text' class="form-control select-container-text" >Pilih..</label>
                                                        <input id='warna-id-select' hidden />
                                                        <div class='select-option-container select-option-container-out'>
                                                            <input type="search" class='select-option-search' id='warna-id-select-search' />
                                                            <ul class='select-option-list'>
                                                                <li data-id="warna-id-select" data-value="" class="" >Pilih...</li>
                                                                <?$idx = 1;foreach ($this->warna_list_aktif as $row) {?>
                                                                    <li data-id="warna-id-select" data-value="<?=$row->id;?>" class="<?=($warna_id == $row->id ? 'selected' :'')?>"  ><?=$row->warna_jual;?></li>
                                                                <?}?>
                                                            </ul>
                                                        </div>
                                                    </div>
					...
					<div class="portlet-body">
                        <table>
                            <tr>
                                ...
                                <td>
                                    <b>
                                        <input name='tanggal_start' id='tanggalStart' readonly class='text-center date-picker' style='border:none; border-bottom:1px solid #ddd; width:86.5px;' value='<?=$tanggal_start;?>'>
                                        s/d
                                        <input name='tanggal_end' id='tanggalEnd' readonly class='text-center date-picker2 ' style='border:none; border-bottom:1px solid #ddd; width:86.5px;' value='<?=$tanggal_end;?>'> 
                                    </b>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <b>
                                        <select name='gudang_id' id='gudang-id' style='width:200px; background:none; border:none; border-bottom:1px solid #ddd'>
                            ...
                            <tr>
                                ...
                                <td>
                                    <b>
                                        <select name='barang_id'  id='barang-id' style='width:200px; background:none; border:none; border-bottom:1px solid #ddd'>
                            ...
                            <tr>
                                ...
                                <td>
                                    <b>
                                        <select name='warna_id'  id='warna-id' style='width:200px; background:none; border:none; border-bottom:1px solid #ddd'>
...
const labelBarang = document.querySelector("#barang-id-select-text");
const labelWarna = document.querySelector("#warna-id-select-text");
...
// ======================== trigger buka daftar option =============================
const openSelectList = (el) => {
    const optContainer = el.parentElement.querySelector(".select-option-container");
    el.parentElement.querySelector(".select-option-search").value="";
    const others = document.querySelectorAll(".select-option-container");
    // 1. cel kalau ada option lain yang lagi kebuka
    // 2. remove option tersebut
    const textLabel = optContainer.querySelector(".select-container-text");
    others.forEach(ele => {
        if(ele !== optContainer){
            const textLabel = ele.parentElement.querySelector(".select-container-text"); // get container nya
            textLabel.style.border = '1px solid #ddd'; // reset border nya
            ele.style.display="none"; // hide element
            ele.classList.add("select-option-container-out"); // kasih penanda out
        }
    });

    selectListToggle(optContainer);
}

const selectListToggle = (optContainer) => {
    const textLabel = optContainer.parentElement.querySelector(".select-container-text");
    const cl = optContainer.classList;
    optContainer.style.transition="transition: height 0.5s, background-color 2s, transform 2s";

    // cek kalau dia sedang posisi out (hide)
    if(cl.value.indexOf('select-option-container-out') !== -1){
        ...
        const oList = optContainer.querySelector(".select-option-list");
        filterOptions(oList.querySelectorAll('li'),'',optContainer);
        olscroll = 100;
        if(oList.querySelector(".selected") === null){
            highlightList(oList, 0);
        }
    }
    // cek kalau dalam posisi in (show)
    else{
...
const highlightList = (list, step) =>{
    const getSelected = list.querySelector('.selected');
    ...
    const li = list.querySelectorAll('li');
    let pos = Object.values(li).indexOf(getSelected);
    pos += step;
    pos = (pos < 0 ? 0 : (pos < li.length ? pos : pos - 1 ));
    const next = Object.values(li)[pos];
    if(!next.hidden){
        getSelected.classList.remove('selected');
        next.classList.add('selected');
        if (olscroll - next.offsetTop > 0) {
            olscroll += Object.values(li)[pos].offsetHeight*-1;
            list.scrollBy({top:(Object.values(li)[pos].offsetHeight+0.5)*-1});
    
        }else if(olscroll - next.offsetTop < -90){
            list.scrollBy({top:Object.values(li)[pos].offsetHeight+0.75})
            olscroll += Object.values(li)[pos].offsetHeight;
        }
    }
}
...
function resetSplitForm(){
    ...
    document.querySelector("#warna-id-select-text").textContent = "";
    setDataToForm();
    get_stok_from_db();
};
...
document.addEventListener('DOMContentLoaded',(event)=>{
    ...
    // add behaviour search nyad
    const optSearch = document.querySelectorAll('.select-option-search');
    optSearch.forEach(el => { // breakdown each element search
        const optContainer = el.parentElement;
        const select = optContainer.querySelector("ul"); 
        const options = select.querySelectorAll("li");
        el.addEventListener("keyup", (e)=>{
            if (e.keyCode === 40 || e.key === 'ArrowDown') { // klo dia pencet atas pas lagi focus
                highlightList(select, 1);
            }else if(e.keyCode === 38 || e.key === 'ArrowUp'){ // klo dia pencet bawah pas lagi focus
                highlightList(select, -1);
            }else if(e.key === 'Enter'){ // klo dia pencet Enter pas lagi focus
    ...
    document.addEventListener("click",function(e){
        if(e.target.classList.value.includes("select-container") === false &&
        e.target.classList.value.includes("select-option-container") === false &&
        e.target.classList.value.includes("select-option-search") === false
        ){
            const others = document.querySelectorAll(".select-option-container");
            others.forEach(ele => {
                const textLabel = ele.parentElement.querySelector(".select-container-text");
                textLabel.style.border = '1px solid #ddd';
                ele.style.display="none";
                ele.classList.add("select-option-container-out");
            });
        };
    });

    const selects = document.querySelectorAll('.select-option-list');
    selects.forEach(options => {
        options.querySelectorAll('li').forEach(list => {
            list.addEventListener("click", function(el){
                optionSelected(list);
                const optContainer = options.parentElement;
                selectListToggle(optContainer);
            });
...
const optionSelected = (list) => {
    const elId = list.getAttribute("data-id");
    const elVal = list.getAttribute("data-value");
    if(list !== null){
        const elText = list.textContent; 
        document.querySelector(`#${elId}-text`).textContent = elText;
        document.querySelector(`#${elId}`).value = elVal;
    }else{
        document.querySelector(`#${elId}-text`).textContent = "";
...
btnSplitSave.addEventListener("click", ()=>{
    if(evaluateSplitQty()){
        ...
        const namaBarang = labelBarang.textContent;
        const namaWarna = labelWarna.textContent;

        for (let i = 0; i < namaGudang.length; i++) {
            dataDetail.nama_gudang = namaGudang[i].label;
            dataDetail.nama_barang = namaBarang;
            dataDetail.nama_warna = namaWarna;
        }
        ...
        const q = [];
        const jR= [];
        console.log('qty', dataDetail.splitData);
...
const get_stok_from_db = () => {
    ...
    if (barang_id !== '' && warna_id !== '' && gudang_id !== '') {
            ...
            $.each(JSON.parse(data_respond),function(k,v){
                if (k==0) {
                    ...
                    for (var i = 0; i < v.length; i++) {
                        ...
                        if (sisa_roll > 0) {
                            stokan[parseFloat(v[i].qty)] = sisa_roll;
        ...
        if (gudang_id == '') {
            ...
            const el = brgSelect.parentElement.querySelector('.select-container-text');
            setTimeout(() => {
                openSelectList(el);
            }, 500);
        }else if(warna_id == ''){
            const el = wrnSelect.parentElement.querySelector('.select-container-text');
            setTimeout(() => {
                openSelectList(el);
            }, 500);
            // wrnSelect.parentElement.querySelector('.select-container-text').click();
        }
        stockContainer.style.backgroundColor="lightgray";
        disableTblSplit();
...
const removeData = (id, custom) => {
    ...
    <?if (is_posisi_id() == 9) {?>
        bootbox.confirm(`Yakin hapus data <b>${dataDelete.nama_barang} ${dataDelete.nama_warna}</b> di <b>${dataDelete.nama_gudang}</b> ?`, function (result) {
            if (result) {    
                fetch(baseurl+`inventory/penyesuaian_stok_split_remove_no_pin`,{
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
...
const setDataToForm = () =>{
    ...
    labelBarang.textContent = dataDetail.nama_barang;
    labelWarna.textContent = dataDetail.nama_warna;
...
const drawMainTable = () => {
    ...
    dataList.forEach((row,index) => {
        ...
        nC8.innerHTML = '';
        <?if (is_posisi_id() < 3  || is_posisi_id() == 9 || is_posisi_id() == 5) {?>
...
const sortTable = (prop, colIndex) =>{
    ...
    dataList.sort(function(a,b){
        if (prop === 'tanggal') {
            ...
            return (set ? ( (a[prop]+a['nama_warna']) > (b[prop]+b['nama_warna']) ? 1 : -1 )  : ((a[prop]+a['nama_warna']) < (b[prop]+b['nama_warna']) ? 1 : -1 ) );
        }else {
            return (set ? (a[prop].toLowerCase() > b[prop].toLowerCase() ? 1 : -1) : (a[prop].toLowerCase() < b[prop].toLowerCase() ? 1: -1) );
        }
    });
...
const filterOptions = (options, char, container) => {
    ...
    options.forEach((option,index) => {
        ...
        if (option.textContent.toLowerCase().includes(char.toLowerCase()) !== false) {
            if (firstOpt === null) {
