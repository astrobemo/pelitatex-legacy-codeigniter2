								...
								<?if (isset($barang_list[$baris->id])) {?>
									<?foreach ($barang_list[$baris->id] as $key => $value) {
										...
										<tr class="<?=(($idx+1) % 8 == 0 ? 'garis-bawah' : '' );?>">
											...
											<td><b><?=number_format($qty_now,'0',',','.');?></b></td>
											
											<td><?=$roll_now; //print_r($batch_set); print_r($qty_sisa_data);
											// print_r($locked_data);?>
											</td>
											...
											<td class='text-left' style='border-left:0px'><?=$value->nama_satuan?></td>
									...
									<tr style='font-size:1.2em;text-align:right'>
										<td>TOTAL</td>
										<td><?=number_format((float)$total_stok,'0',',','.');?></td>
										<td><?=$total_roll;?></td>
										<?foreach ($batch_for_pre_po as $row) {
											if (is_posisi_id() != 5) {?>
										...
										<?}?>
										<td style='border-right:0px;' ><?=number_format((float)($total_po),'0',',','.');?></td>
										<td style='border-left:0px; text-align:left'> <?=$value->nama_satuan;?> </td>
