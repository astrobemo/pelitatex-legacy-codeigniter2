			...
			<div class="col-md-12">
				<div class="portlet light">
					...
					<div class="portlet-body">
							...
							<?}?>
								...
								<tbody>
									...
									foreach ($mutasi_barang_list as $row) { 
                                        ...
                                        if ($isAwalTahun) {
                                            if (isset($qty_awal[$row->barang_id][$row->warna_id])) {
                                                ${'qty_stock_'.$row->satuan_id} += $qty_awal[$row->barang_id][$row->warna_id];
                                            }
                                        }else{
											...
											<tr <?//=($row->qty_beli > 0 ? '' : 'hidden');?> >
												...
												<?
													...
													if ($row->tipe_qty != 3) {
                                                        ...
                                                        if ($isAwalTahun) {
                                                            if (isset($qty_awal[$row->barang_id][$row->warna_id])) {
                                                                ${'roll_stock_'.$row->satuan_id} += $roll_awal[$row->barang_id][$row->warna_id];;
                                                            }
                                                        }else{
												...
												<?if ($isAwalTahun) {?>
													<?
                                                        ...
                                                        if (isset($hrg_awal[$row->barang_id][$row->warna_id])) {
                                                            $st = $qty_awal[$row->barang_id][$row->warna_id];
                                                            $ha = $hrg_awal[$row->barang_id][$row->warna_id];
                                                            $ro = $roll_awal[$row->barang_id][$row->warna_id];
                                                        }
														$total_nilai =($ha * $st) + ($row->hpp_beli * $row->qty_beli);
														$total_qty_stock = $st + $row->qty_beli;
														if ($total_qty_stock == 0) {
															$total_qty_stock = 1;
														}
                                                        
														$hpp_all = round($total_nilai / $total_qty_stock,2);?>

													<td><?=number_format($st,'2',',','.');?></td>
													<td><?=($row->tipe_qty != 3 ? number_format($ro,'0',',','.') : '');?></td>
													<td class='text-center'><?=number_format($st * $ha,'2',',','.')?></td>
						...
						<?//if ($tutup_buku_id != 0 && $tutup_buku_id_now == 0) {?>
							<form id="tutup-buku-form" action="<?=base_url();?>inventory/tutup_buku_insert" method="POST" >
