					...
					<div class="portlet-body">
						...
						<table class='table' id="general_table">
							...
							<tbody>
								...
								$filter_nama_cust = ["'","/","&"];
								$nilai_berjalan = 0; $total_faktur = 0; $total_email = 0; $total_status_1 = 0;
								$total_status_2 = 0; $total_status_3 = 0 ; $total_status_4=0; $total_pdf=0;
								$thread_list = array();
											...
											<table class='action-tbl'>
												...
												<tr <?=($status_email == 0 ? ($warning_filter != '' && $row->email_stat == 0 ? 'hidden' : $total_email++ ) : '' )?> >
													...
													<td>
														...
														<?if ($row->kirim_id != '' && $filter == 2 && $row->email_stat == 1) {?>
						...
						<?}elseif (count($fp_list_npwp) > 0){?>
							...
							<button type="button" onclick="refreshToken()" id='btn-get-token' class="btn btn-lg yellow-gold">TOKEN</button>
							<button type="button" <?=($total_complete <= 0 ? 'disabled' : '' );?>  onclick="createMailList()" id="btn-draft-mail" class="btn btn-lg green">CREATE DRAFTS</button>
					...
					<div class="modal-body">
						...
						<div class="tab-content">
							<div class="tab-pane active in" id="tab_modal_1">
								<form  action="<?=base_url('pajak/upload_dropzone_faktur_pajak'); ?> " class="dropzone" id="my-dropzone">
									<input name='id' value="<?=$rekam_faktur_pajak_id;?>" hidden>
								</form>
							</div>
...
const cekTokenBerlaku = () =>{
	...
	<?if ($status_email != 1) {?>
		...
		if (tokenBerlaku.length > 0) {
			...
			// document.querySelector("#btn-check-mail").style.display = 'inline-block';
			...
			// document.querySelector("#btn-check-mail").style.display = 'none';
...
jQuery(document).ready(function() {
	...
	// 	<?if ($total_warn > 0) {?>
	// 		bootbox.confirm("Ada <b>"+<?=$total_warn;?>+"</b> alamat email yang mungkin tidak valid, lanjutkan ? ", function(respond){
	// 			if (respond) {
	// 	<?};?>
	// 			$('#general_table .email-1').each(function(){
	// 				var data = {};
	// 				var url = "pajak/test_kirim_mail";
	// 				data['nama'] = $(this).closest('tr').find('.nama').html();
	// 				data['email'] = $(this).closest('tr').find('.email').html();
	// 				data['pdf_list'] = $(this).closest('tr').find('.pdf_list').html();
	// 				data['id'] = "<?=$rekam_faktur_pajak_id?>";
	// 				ajax_data_sync(url,data).done(function(data_respond /* ,textStatus, jqXHR */){
	// 			        console.log(data_respond);
	// 			        console.log(data);
	// 			        // var arr = Object.values(JSON.parse(data_respond));
	// 			        $('#hasil-kirim').append(data_respond);
	...
	// $("#email-send-all").click(function(){
	// 	<?if ($total_warn > 0) {?>
	// 		bootbox.confirm("Ada <b>"+<?=$total_warn;?>+"</b> alamat email yang mungkin tidak valid, lanjutkan ? ", function(respond){
	// 			if (respond) {
	// 	<?};?>
	// 			window.location.replace(baseurl+"pajak/kirim_pajak_email?id="+"<?=$rekam_faktur_pajak_id;?>");
	// 			btn_disabled_load($("#email-send-all"));
	// 	<?if ($total_warn > 0) {?>
	// 			};
	// 		});
	// 	<?};?>
	...
	// $("#general_table").on("click",".email-send-satuan", function(){
	// 	var customer_id = $(this).closest('tr').find('.customer_id').html();
	// 	<?if ($total_warn > 0) {?>
	// 		bootbox.confirm("Ada <b>"+<?=$total_warn;?>+"</b> alamat email yang mungkin tidak valid, lanjutkan ? ", function(respond){
	// 			if (respond) {
	// 	<?};?>
	// 			window.location.replace(baseurl+"pajak/kirim_pajak_email_satuan?id="+"<?=$rekam_faktur_pajak_id;?>&customer_id="+customer_id);
	// 			btn_disabled_load($("#email-send-all"));
	// 	<?if ($total_warn > 0) {?>
	// 			};
	// 		});
...
function createMailList(){
	...
	if (cekTokenBerlaku) {
		...
		bootbox.confirm(`Yakin mengirim email faktur pajak kepada customer ?<br/>
		
			<hr/>
			<div class="note note-info">
				<button type="button" class="btn btn-xs green">CREATE DRAFTS</button>
				<ol style="margin-top:15px;">
					<li>Tombol <b>CREATE DRAFTS</b> akan membuat draft email untuk daftar yang <b>belum pernah</b> dibuatkan email sebelumnya </li>
					<li>Semua Draft sudah otomatis tersimpan di GMAIL</li>
					<li>Setelah dibuatkan draft, maka otomatis kemudian akan di kirimkan</li>
					<li>Jika terjadi error / internet putus maka :
						<ul>
							<li>Refresh Halaman</li>
							<li>tekan tombol CREATE DRAFTS untuk melanjutkan draft selanjutnya </li>
							<li>jika tombol CREATE DRAFTS tidak bisa ditekan, maka semua sudah dibuat draft </li>
							<li>Jika error terjadi ketika mengirim email, maka klik tombol SEND, bukan CREATE DRAFTS </li>
						</ul> 
					</li>
				</ol>
			</div>
			<hr/>
			<div class="note note-info">
				<button type="button" class="btn btn-xs blue">SEND</button>
				<ol style="margin-top:15px;">
					<li>Tombol <b>SEND</b> akan mengirim email yang status nya di DRAFT pada tabel di atas</li>
					<li>Jika tombol send tidak muncul artinya semua draft sudah terkirim</li>
					<li>Jika terjadi error / internet putus maka :
						<ul>
							<li>Refresh Halaman</li>
							<li>tekan tombol SEND untuk melanjutkan send selanjutnya </li>
						</ul> 
					</li>
				</ol>
			</div>
			<div class="note note-warning">
				<p>
					Jika masih error, mohon menghubungi admin IT
				</p>
			<div>
			`, function(respond){
			if (respond) {
				...
				fetch(baseurl+`pajak/email_list_body?id=${rekam_faktur_pajak_id}`)
...
const createDrafts = async (email, customer) => {
	const response = await fetch('https://gmail.googleapis.com/gmail/v1/users/relay.do.bdg@gmail.com/drafts?access_token='+tokenBerlaku, {
		method: 'POST',
	...
	const resInsert = await fetch(baseurl+`pajak/email_data_status_insert`, {
		method: 'POST',
		body: `data=${JSON.stringify(res.message)}&draft_id=${res.id}&customer_id=${customer.customer_id}&rekam_faktur_pajak_email_id=${customer.rekam_email_id}&id=${rekam_faktur_pajak_id}`, // string or object
		headers: {
			'Content-Type': 'application/x-www-form-urlencoded'
		},
	});

	const resId = await resInsert.json();
	customer.rekam_email_id = resId;
...
const draftLoop = (counter) => {
	if (counter < emailList.length) {
		createDrafts(emailList[counter], customerList[counter]);
		const text = customerList[counter].nama;
		counter++;
		document.querySelector('#processDrafting .info-nama-cust').innerHTML = text;
		document.querySelector('#processDrafting .counter').innerHTML = counter +'/'+emailList.length;
	}else{
		// const dialog = bootbox.dialog({
		// 	message: `<p class="text-center mb-0"><i class="fa fa-check"></i> Berhasil membuat draft semua message... refreshing now<i class="fa fa-spin fa-cog"></i> </p>`,
		// 	closeButton: false
		// });
		// window.location.reload();
		// document.querySelector('#layer').style.display = 'none';
...
function sendMailList(){
	...
	if (cekTokenBerlaku) {
		...
		fetch(baseurl+`pajak/email_list_drafts?id=${rekam_faktur_pajak_id}`)
		.then((response) => response.json())
		.then((data) => {
			console.log(data);
...
const sendDrafts = async (draft_id, id) => {
	const response = await fetch('https://gmail.googleapis.com/gmail/v1/users/relay.do.bdg@gmail.com/drafts/send?access_token='+tokenBerlaku, {
		method: 'POST',
		body: JSON.stringify({"id":`${draft_id}`}), // string or object
		headers: {
			'Content-Type': 'application/json'
		},
	});
	...
	const resInsert = await fetch(baseurl+`pajak/email_data_status_update`, {
		method: 'POST',
		body: `id=${id}`, // string or object
		headers: {
			'Content-Type': 'application/x-www-form-urlencoded'
		},
	});
...
const sendLoop = (counter) => {
	if (counter < draftList.length) {
		sendDrafts(draftList[counter].draft_id, draftList[counter].id);
		const text = draftList[counter].nama_customer;
		counter++;
		document.querySelector('#processSending .info-nama-cust').innerHTML = text;
		document.querySelector('#processSending .counter').innerHTML = counter +'/'+draftList.length;
	}else{
		const dialog = bootbox.dialog({
			message: `<p class="text-center mb-0"><i class="fa fa-check"></i> Terkirim semua... refreshing now<i class="fa fa-spin fa-cog"></i> </p>`,
			closeButton: false
		});
		window.location.reload();
		document.querySelector('#layer').style.display = 'none';
