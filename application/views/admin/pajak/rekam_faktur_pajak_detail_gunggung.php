			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('pajak/rekam_tambah_faktur_gunggung');?>" class="form-horizontal" id="form_tambahan_faktur" method="post">
							<h3 class='block'> Tambah Faktur</h3>
							
							<input type="text" name="rekam_faktur_pajak_id" value="<?=$rekam_faktur_pajak_id?>" hidden>
							<div class="form-group">
			                    <label class="control-label col-md-3">No Faktur<span class="required">
			                    * </span>
			                    </label>
			                    <div class="col-md-6">
									<select name='penjualan_id' class="form-control" id="penjualanIdTambahan">
										<?foreach ($faktur_tambahan as $row) {
											if($row->no_faktur_fp != ''){?>
												<option value="<?=$row->id?>"><?=$row->no_faktur_fp?> : <?=$row->nama_cust_fp?></option>
											<?}?>
										<?}?>
									</select>
			                    </div>
					...
					<div class="portlet-body">
						<div style='float:right'>
							...
							<?if ($data_status == 0) {?>
								...
								<button onclick="generateXML('<?=$rekam_faktur_pajak_id;?>')" class='btn btn-lg btn-primary' id="buttonDownloadXML"><i class='fa fa-download'></i>Gunggung <span id="fakturTerpilih">All</span> </button>
						...
						<table width='100%' id="general_table">
							...
							<tbody>
								...
								foreach ($fp_list_detail as $row) { 
									...
									?>
									<tr style="<?=$bg_color;?>" class=" <?=$isRegistered?> " >
										<td><?=$idx+1;?></td>
										<td>
...
jQuery(document).ready(function() {
	...
	$("#general_table").on("click", '.btn-remove', function(){
		var ini = $(this).closest("tr");
		var id = ini.find('.id').html();
		bootbox.confirm("<b style='color:red'>Hapus</b> Invoice ini dari daftar ? ", function(respond){
			if (respond) {
	...
	$(".btn-lock").click(function(){
		if (totalFaktur > 0) {
			...
			bootbox.alert("Tidak ada data faktur pajak");
		};
	});	
	...
	$(".btn-add-faktur").click(function(){
		const penjualan_id = $("#penjualanIdTambahan").val();
		if(penjualan_id == '' || typeof penjualan_id == 'undefined'){
			alert('No faktur tidak boleh kosong');
			return;
		}
		$("#form_tambahan_faktur").submit();
	});
...
function generateXML(rekam_faktur_pajak_id){
	...
	let devUrl = 'http://localhost:3000/pajak/generate_faktur_pajak_gunggung';
	let prodUrl = 'https://api.favourtdj.com/pajak/generate_faktur_pajak_gunggung';
	...
	$.ajax({
		...
		success: function(response) {
            ...
            link.download = `fp_gunggung-${getToko.nama}-periode_${tanggal_awal}_sd_${tanggal_akhir}.xml`;
			...
			reader.onload = function(event) {
				...
				try {
					...
					for (let i = 1; i < lines.length; i++) {
						...
						if(typeof no_fp !== 'undefined' && no_fp.length > 0 && data_list[no_invoice]){
