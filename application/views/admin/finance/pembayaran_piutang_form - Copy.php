								...
								<?foreach ($piutang_list as $row) { ?>
									<tr>
										...
										<td>
											...
											<a href="<?=base_url().is_setting_link('finance/piutang_payment_form').'/?customer_id='.$row->customer_id;?>&toko_id=<?=$row->toko_id;?>&tanggal_start=<?=is_reverse_date($row->tanggal_start);?>&tanggal_end=<?=is_reverse_date($row->tanggal_end);?>" class="btn btn-xs blue"> Lihat / Pelunasan</a>
														...
														<table class='bayar-info' width='100%;' style='margin-bottom:5px;' <?=($latest_pembayaran_piutang_nilai_id != $row->id ? 'hidden' : '' );?>  >
															...
															<tr>
																...
																<td class='text-right'>
																	...
																	<?if ($row->giro_tolakan_id == '' ) {?>
																		<button type='button' style='margin:5px;' id='remove-<?=$row->id;?>' class='btn btn-xs red btn-active btn-remove-nilai'>HAPUS</button>
																		<?if ($row->pembayaran_type_id != 5) {?>
																			<button type='button' style='margin:5px;' id='update-<?=$row->id;?>' class='btn btn-xs green btn-active btn-update-nilai'>SIMPAN</button>
																		<?}?>
																	<?}?>
																</td>
															</tr>
									...
									<td style='vertical-align:top;font-size:2.5em;'  width='55%' class='text-right'>
										<table style='float:right;'>
											...
											<tr style='border:2px solid #ffd700'>
												...
												<td class='text-right padding-rl-10'>
													<b style='float:left'>Rp</b> <b><input <?=($pembayaran_piutang_id == '' ? 'readonly' :'');?> name='pembulatan' class='' value="<?=number_format($pembulatan,'0',',','.');?>" style='width:140px;border:none;text-align:right'></b>
												</td>
											</tr>
											<tr style='border:2px solid #ffea78'>
												<td class='padding-rl-25' style='background:#ffea78'>LAIN-LAIN </td>
												<td class='text-right padding-rl-10'>
													<b style='float:left'>Rp</b> <b><input <?=($pembayaran_piutang_id == '' ? 'readonly' :'');?> name='lain2' class='amount-number' value="<?=number_format($lain2,'0',',','.');?>" style='width:140px;border:none;text-align:right'></b>
												</td>
											</tr>
											<tr style='border:2px solid #ceffb4'>
												...
												<td class='padding-rl-10'>
													<?
														$sisa_bayar = ($total_bayar_piutang + $pembulatan + $lain2) - $total_piutang ;
...
//=============================================bank history click==========================
	...
	<?if ($pembayaran_piutang_id != '') { ?>
		$("[name=pembulatan], [name=lain2]").change(function(){
			var data = {};
			data['id'] = "<?=$pembayaran_piutang_id?>";
			data['nilai'] = ($(this).val() == '' ? 0 : $(this).val() );
			data['kolom'] = $(this).attr('name');
			console.log(data);
			var url = 'finance/update_pembulatan_piutang';
			ajax_data_sync(url,data).done(function(data_respond  ,textStatus, jqXHR ){
				if (textStatus == 'success') {				
					update_total_bayar();
					notific8("lime", "Pembulatan updated");
				}else{
					alert("error mohon refresh dengan menekan F5");
				}
	   		});
...
function update_total_bayar(){
	...
	var pembulatan = reset_number_format($("[name=pembulatan]").val());
	var lain2 = reset_number_format($("[name=lain2]").val());
	...
	var selisih = total_nilai_bayar - total_bayar + parseInt(pembulatan) + parseInt(lain2);
	// alert(selisih);
