	...
	<div class='container'>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('finance/hutang_awal_insert')?>" class="form-horizontal" id="form_add_data" method="post">
			                ...
			                <div class="form-group">
			                    ...
			                    * </span>
			                    ...
			                    <div class="col-md-6">
			                    	<input name='supplier_id' hidden value='<?=$supplier_id;?>'>
		...
		<div class="modal fade" id="portlet-config-edit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('finance/hutang_awal_update')?>" class="form-horizontal" id="form_edit_data" method="post">
						...
						<table class="table table-hover table-striped table-bordered" id="general_table">
							...
							<tbody>
								<?foreach ($hutang_list_detail as $row) { 
									$jatuh_tempo_giro = ($row->jatuh_tempo_giro =='' ? array() : explode(',', $row->jatuh_tempo_giro));
									$no_giro = explode(',', $row->no_giro);
									?>
									<tr>
										<td>
										...
										<td>
											...
											<td><a href="<?=base_url().is_setting_link('finance/hutang_payment_form')?>?id=<?=$row->pembayaran_hutang_id?>" target='_blank'><?=is_reverse_date($row->tanggal_bayar)?></a> </td>
											<td>
												<?if (count($jatuh_tempo_giro) > 0) {?>
													<ul>
														<?foreach ($jatuh_tempo_giro as $key => $value) {?>
															<li><?=$no_giro[$key]?> <b>[<?=is_reverse_date($value);?>]</b></li>
														<?};?>
													</ul>
												<?}?>
											</td>
