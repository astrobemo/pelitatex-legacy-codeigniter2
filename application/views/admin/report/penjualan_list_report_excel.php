<?    /** Caching to discISAM*/
		...
		foreach ($penjualan_list as $row) {
			...
			// echo $tgl_list[$index].'!='.$blnThn.'<br/>';;
			// echo ($tgl_list[$index] != $blnThn).'<br/>';
			while ($tgl_list[$index] != $blnThn ) {
				// echo $index.'==';
				foreach ($this->satuan_list_aktif as $row2) {
					${'g_total_'.$row2->id} = 0;
					${'g_returtotal_'.$row2->id} = 0;
					${'idx_'.$row2->id} = 0;
					${'yard_total_'.$row2->id} = 0;
					${'roll_total_'.$row2->id} = 0;
					${'yard_returtotal_'.$row2->id} = 0;
					${'roll_returtotal_'.$row2->id} = 0;
				}
				$idx = 1; $row_no = 5; $g_total = 0;
				$yard_total = 0;
				$roll_total = 0;
		...
		header("Content-Disposition: attachment;filename=Laporan_Penjualan_".$nama_toko."_".date("dmY",strtotime($tanggal_start))."sd_".date("dmY",strtotime($tanggal_end)).".xls");
