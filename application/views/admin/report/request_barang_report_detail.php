						...
						<div class='row'>
							...
							<div class="col-xs-12 col-md-8">
								...
								?>
									...
									<table id='table-overview' width="100%">
										<thead>
											<tr>
												<th rowspan='2' style="max-width:100px" >Barang</th>
												<th colspan='5' class='text-center'><?=date('F Y', strtotime($bulan_request_awal));?></th>
												<th colspan='3' class='text-center'>SELANJUTNYA</th>
												<th rowspan='2' class='text-center'>Non Request</th>
												<th rowspan='2' class='text-center' hidden>TOTAL DATANG</th>
											</tr>
											<tr style='font-size:1.1em; border-bottom:2px solid #777'>
												<th class='text-center'>Request</th>
												<th class='text-center bg-total'><span style='font-size:0.7em'>Pengiriman</span><br/>Total</th>
												<th class='text-center bg-total'>%</th>
												<th class='text-center bg-awal'><span style='font-size:0.7em'>Pengiriman</span><br/>Tepat</th>
												<th class='text-center bg-awal'>%</th>
												<th class='text-center bg-lain'>Request</th>
												<th class='text-center bg-lain'>Datang </th>
												<th class='text-center bg-lain'>%</th>
											</tr>
										</thead>
										<tbody>
                                            <?
											$dt_barang = array();
											foreach ($request_list as $row) {
												$persen_awal[$row->barang_id] = 0;
												if ($request_awal[$row->barang_id] != 0) {
													$persen_awal[$row->barang_id] = $datang_awal[$row->barang_id]/$request_awal[$row->barang_id];
												}
												$persen_lain[$row->barang_id] = 0;
												if ($request_lain[$row->barang_id]) {
													$persen_lain[$row->barang_id] = $datang_lain[$row->barang_id]/$request_lain[$row->barang_id];
												}

												$sub[$row->barang_id] = $datang_awal[$row->barang_id] + $datang_lain[$row->barang_id] + $datang_noreq[$row->barang_id];
											
												array_push($dt_barang, array(
													'barang_id' => $row->barang_id, 
													'nama_barang' => $row->nama_barang,
													'request_awal' => $request_awal[$row->barang_id],
													'total_datang' => $sub[$row->barang_id],
													'datang_awal' => $datang_awal[$row->barang_id],
													'request_lain' => $request_lain[$row->barang_id],
													'datang_lain' => $datang_lain[$row->barang_id],
													'datang_nr' => $datang_noreq[$row->barang_id]
												));
											}?>
											<?$idx = 1; $stotal = 0;
											foreach ($request_list as $row) {
												$stotal += $sub[$row->barang_id];
												$persen_by_sub = 0;
												if ($request_awal[$row->barang_id] > 0) {
													$persen_by_sub = $sub[$row->barang_id]/$request_awal[$row->barang_id];
													# code...
												}
												?>
												<!-- #35aa47 ijo -->
												...
												<tr style="<?=($idx%5 == 0 ? 'border-bottom:1.5px solid #aaa' : '')?>" >
                                                    ...
                                                    <td class='bg-total text-right'>
														<?=str_replace(',00','',number_format($sub[$row->barang_id],'2',',','.')) ;?> <small hidden>(<?=$persen_non_request*100;?>%)
													</td>
													<td class='bg-total text-center'><?=str_replace('.00','',number_format($persen_by_sub*100,'2',".","")) ;?></td>
													...
													<td class='bg-awal text-center'><?=str_replace('.00','',number_format($persen_awal[$row->barang_id]*100,'2',".","")) ;?></td>
													...
													<td class='bg-lain text-center'><?=str_replace(".00","", number_format($persen_lain[$row->barang_id]*100,'2',".","")) ;?></td>
													...
													<td class='bg-nr text-right' hidden>
														<?=str_replace(',00','',number_format($sub[$row->barang_id],'2',',','.')) ;?> <small hidden>(<?=$persen_non_request*100;?>%)
													</td>
												</tr>
											<?$idx++;}?>
										...
										<tfooter>
											<tr style='font-size:1.1em; border-top:2px solid #777'>
												...
												?>
												<th>TOTAL</th>
												<th class='bg-request text-right'><?=number_format($request_awal_total,'0',',','.'); ?></th>
												<th class='bg-total text-right' ><?=str_replace(',00','',number_format($stotal,'2',',','.')) ;?></th>
												<th class='bg-total text-right'><?=number_format(($stotal/$request_awal_total)*100,'2',',','.'); ?>%</th>
												...
												<th class='bg-nr text-right' hidden><?=str_replace(',00','',number_format($stotal,'2',',','.')) ;?></th>
											</tr>
										</tfooter>
									</table>
							</div>
							<div class="col-xs-12 col-md-4">
								<div class="text-center">
									...
									<?$di = 150;?>
									<div id="venn-diagram" style='width:<?=$di*2?>px; height:<?=$di+20;?>px'>
										<?
											$diameter = ($request_awal_total > $total_datang_all ? $request_awal_total : $total_datang_all);
											$part_request = round($request_awal_total / $diameter,2);
											$part_datang = round($total_datang_all / $diameter,2);
											$part_datang_pertotaldatang = round($datang_awal_total / $total_datang_all,2);
											$l_request = 0;
											$r_datang = 0;
											
											
											$t_luas = 3.14 * pow(($di/2),2);
											//width
											$w_request = $part_request * $di;
											$w_datang = $part_datang * $di;
											if ($part_request < 1) {
												$area = $part_request*$t_luas;
												$r = round(sqrt($area/3.14));
												$w_request = ceil($r) * 2;
											}

											if ($part_datang < 1) {
												$area = $part_datang*$t_luas;
												$r = round(sqrt($area/3.14));
												$w_datang = ceil($r) * 2;
											}

											//top margin
											$t_request = 20 + (($di-$w_request)/2);
											$t_datang = 20 + (($di-$w_datang)/2);
											
											//left/right
											//klo request kiri klo datang kanan
											
											// $r_datang = $di * $part_datang_pertotaldatang * 0.8;

											$mrgn = ($w_datang + $w_request)/2;
											$l_request = $di - $mrgn;
											$r_datang = $di - $mrgn;

											if (is_posisi_id() == 1) {

												$offset = ($w_datang < $w_request ? $w_datang : $w_request) * $part_datang_pertotaldatang*1.2;
												$l_request += $offset/2;
												$r_datang += $offset/2;

												// echo $datang_awal_total .'/'. $total_datang_all.'<br/>';
												// echo $w_request.' '.$w_datang.'<br/>';
												// echo $part_datang_pertotaldatang.' '.$offset.'<br/>';
												// echo $part_datang_pertotaldatang.' '.($w_datang * $part_datang_pertotaldatang);
												

												
											}else{
												// $l_request = $di - $w_request;
												// $r_datang = $di - $w_datang;

												$offset = ($persen_total*$di)/2;
												$l_request += $offset;
												$r_datang += $offset;
											}
										...
										<div style="position:absolute; text-align:left; top:<?=$di/3?>px;height:100%;width:75px;left:<?=$l_request*0.6?>px;font-size:0.9em; "><b>Request<br/> Total </b><hr style='padding:0px;margin:2px; border-top:1px solid black'/> <b><?=number_format($request_awal_total,'0',',','.');?></b></div>
										<div style="position:absolute; text-align:right; top:<?=$di/3?>px;height:100%;width:75px;right:<?=$r_datang*0.6;?>px;font-size:0.9em; "><b>Pengiriman<br/> Total </b><hr style='padding:0px;margin:2px; border-top:1px solid black'/> <b><?=number_format($total_datang_all,'0',',','.');?></b></div>
										<div style="position:absolute; text-align:<?=($w_request > $w_datang ? 'center': 'right')?>; top:<?=$di/1.5?>px;height:100%;width:70px;right:<?=($r_datang+50);?>px;margin:auto;font-size:0.9em; "><b>Pengiriman<br/> Tepat</b> <hr style='padding:0px;margin:2px; border-top:1px solid black'/><b><?=number_format($datang_awal_total,'0',',','.');?></b></div>
										<?if (is_posisi_id() == 1) {?>
											<div style="position:absolute; ">
												<div class="triangle-left" style="width: 0;
													height: 0;
													border-top: <?=$di/2?>px solid transparent;
													border-right: <?=$di/3?>px solid #555;
													border-bottom: <?=$di/2?>px solid transparent;
												">
												<?=$di/2?>
												<?=$di/3?>
												</div>
									...
									<div style='position:relative; width:250px; height:65px; margin:auto' hidden>
										<div style="display:inline-block;text-align:center; top:0px;height:100%;width:75px;left:0px;background-color:rgba(255,0,0,0.3)">Request<br/> Total <hr style='padding:0px;margin:2px'/> <b><?=number_format($request_awal_total,'0',',','.');?></b> </div>
										<div style="position:absolute; width:82px; left:84px;height:100%;top:0px; background-color:rgba(255,0,0,0.3); color:transparent">Y</div>
										<div style="position:absolute; width:82px; left:84px;height:100%;top:0px; background-color:rgba(0,0,255,0.3); color:transparent">Y</div>
										<div style="display:inline-block;text-align:center; top:0px;height:100%;width:75px;left:75px; background:transparent;"><b>Pengiriman<br/> Tepat</b> <hr style='padding:0px;margin:2px'/><b><?=number_format($datang_awal_total,'0',',','.');?></b></div>
										<div style="display:inline-block;text-align:center; top:0px;height:100%;width:75px;right:0px;background-color:rgba(0,0,255,0.3);">Pengiriman<br/> Total <hr style='padding:0px;margin:2px'/> <b><?=number_format($total_datang_all,'0',',','.');?></b></div>
									</div>
						...
						<div class='row'>
								...
								<table class="table table-striped table-bordered table-hover" id="general_table">
									<thead>
										...
										<tr class='tr-1'>
											...
											<th scope="col" class='text-center' style="border-bottom:none" hidden >
												TOTAL
											</th>
											<th scope="col" class='text-center' style="border-bottom:none" >
										...
										<tr class='tr-2' style='font-size:1.1em; border-bottom:2px solid #777'>
											...
											<th class="text-center">REQUEST</th>
											<th class="text-center">PENGIRIMAN<br/>TOTAL</th>
											<th class="text-center">%</th>
											<th class="text-center">PENGIRIMAN<br/>TEPAT</th>
											<th class="text-center">%</th>
											<th class="text-center">REQUEST</th>
											<th class="text-center">DATANG</th>
											<th class="text-center">%</th>
											<th class="text-center">NON REQUEST</th>
											<th class="text-center" hidden>DATANG</th>
											...
											<?foreach ($barang_id as $key => $value) {
												...
												if ($request_awal[$row->barang_id] > 0) {
													foreach ($wid as $key2 => $value2) {
														...
														$persen_total = 0;
														if ($request_awal_warna[$value][$value2] > 0) {
															// $persen_total = round($sub / $request_awal_warna[$value][$value2],4);
															$persen_total = $sub / $request_awal_warna[$value][$value2];
														}
														...
														<tr style="<?=(($idx_warna+1)%5 == 0 ? 'border-bottom:2px solid #aaa;' : '')?>; "  >
															...
															<td class='bg-total text-center'><?=(float)number_format($persen_total*100,'2',".","");?>%</td>
															<td class="bg-awal text-right">
															...
															<td hidden class='bg-lain text-center'><?=number_format($sub,'0',',','.') ;?></td>
															<?if ($idx_warna == 0) {?>
																<td hidden class='text-center' rowspan="<?=count($wid)+count($wid_nr)+1;?>" style="border-left:1px solid #888;">
													...
													foreach ($wid_nr as $key2 => $value2) {
														...
														if ($isShow[$bid][$value2] == false) {
															...
															<tr>
																...
																<td class='text-center row-nr'> - </td>
																<td class='text-center row-nr'> - </td>
																<td hidden class='text-right row-nr'><?=number_format($qd_nr[$key2],'0',',','.');?></td>
																<?if (count($wid) == 0 && $widx == 0) {?>
												...
												if ($request_awal[$value] != 0) {
													$persen_awal_total = $datang_awal[$value]/$request_awal[$value];
													$persen_total = $total_sub/$request_awal[$value];
												}
												...
												?>
													<tr style='font-size:1.1em; border-top:2px solid #aaa;font-weight:bold;'>
														...
														<td class='text-center'><?=number_format($persen_awal_total*100,'2',',','.');?>%</td>
							...
							<div class="col-xs-12">
								...
								<form action="<?=base_url()?>report/po_request_list_export_excel" target='_blank' method="POST" class='text-right'>
									...
									<textarea type="text" name="data_barang" hidden><?=json_encode($dt_barang);?></textarea>
