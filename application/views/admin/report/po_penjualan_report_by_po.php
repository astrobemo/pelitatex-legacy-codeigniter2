...
function getPOList(){
	...
	var data = {};
	var url = `report/data_po_penjualan?customer_id=${customerId}&barang_id=${barangId}&warna_id=${warnaId}`;
		...
		if (jmlBarang > 0 ) {
			...
			for (let i = 0; i < jmlBarang; i++) {
				...
				for (let j = -1; j < jmlWarna[i]; j++) {
					...
					if(j==-1){
						tRow += `<td style="${(i==0 ? 'border-top:2px solid #ccc' : '')}; background:rgba(125,125,125,0.1)" colspan='3'>
